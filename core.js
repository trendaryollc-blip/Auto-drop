...
const Logger = (() => {
  ...
  return {
    /** @param {LogLevel} level */
    setLevel(level) { _minLevel = level; },
    /** @param {string} context */
    enableContext(context) { _enabledContexts.add(context); },
    /** @param {string} context */
    disableContext(context) { _enabledContexts.delete(context); },
    /** @returns {LogEntry[]} */
    getBuffer() { return [..._buffer]; },
    clearBuffer() { _buffer.length = 0; },
    /** @param {string} message @param {*} [data] */
    debug(message, data, context) { log('debug', message, context, data); },
    /** @param {string} message @param {*} [data] */
    info(message, data, context) { log('info', message, context, data); },
    /** @param {string} message @param {*} [data] */
    warn(message, data, context) { log('warn', message, context, data); },
    /** @param {string} message @param {*} [data] */
    error(message, data, context) { log('error', message, context, data); }
  };
})();
...
const EventBus = (() => {
  ...
  return {
    ...
    once(e, cb, opts={}) {
      const self = this;
      const w = (...args) => { self.off(e, w); cb.apply(opts.context || null, args); };
    ...
    async emit(e, data) {
      ...
      if (parallelListeners.length > 0) {
        await Promise.allSettled(parallelListeners.map(async ({eventName, entry}) => {
          try { await entry.cb.call(entry.ctx, data); }
          catch(err) { Logger.error(`Error in parallel listener "${eventName}"`, err, 'EventBus'); }
        }));
      }
      // Run sequential listeners in priority order
      for(const {eventName, entry} of sequentialListeners){
        try{await entry.cb.call(entry.ctx,data);}
        catch(err){Logger.error(`Error in "${eventName}"`, err, 'EventBus');}
      }
    },
    /** @param {string} e @returns {boolean} */
    has(e){for(const [eventName,listeners] of _l.entries()){if(matchesEventPattern(e,eventName)&&listeners.length>0)return true;}return false;},
    /** @returns {string[]} */
...
const PluginRegistry = (() => {
  ...
  return {
    ...
    async mount(id){
      ...
      var beforeChildren = document.body ? Array.from(document.body.childNodes) : [];
      try{
        ...
        var afterChildren = document.body ? Array.from(document.body.childNodes) : [];
        for (var i = afterChildren.length - 1; i >= 0; i--) {
...
const ComponentRegistry = (() => {
  ...
  const captureFocusState = (el) => {
    ...
    if(tag==='input' || tag==='textarea'){
      return {tag, value: el.value, selectionStart: el.selectionStart, selectionEnd: el.selectionEnd, id: el.id, name: el.name, type: el.type};
    }
    if(tag==='select'){return {tag, value: el.value, id: el.id, name: el.name};}
    if(tag==='button'){return {tag, id: el.id, name: el.name};}
    return null;
  };
  const restoreFocusState = (container, state) => {
    ...
    const match = focusables.find((el)=>{
      const tag=el.tagName.toLowerCase();
    ...
    if(state.selectionStart !== undefined && typeof match.setSelectionRange === 'function'){
      match.setSelectionRange(state.selectionStart, state.selectionEnd !== undefined ? state.selectionEnd : state.selectionStart);
    }
  };
  return {
    register(type, def) {
    ...
    update(id,newProps){
      ...
      const activeElement = document.activeElement && inst.container.contains(document.activeElement) ? document.activeElement : null;
...
// ===== 4. CONFIG MANAGER =====
const Config = (() => {
  const _d={}; const _v=new Map(); const _s=new Map();
  ...
  return {
    ...
    registerSchema(path,schema){_s.set(path,schema);},
    get(path,fallback){
    ...
    set(path,value){
      ...
      if(schema && !validateAgainstSchema(value,schema,last)){t[last]=old;return false;}
      t[last]=value;
      ...
      for(const[name,adapter]of _adapters){
        ...
        promises.push(
          adapter.search(query,filters)
            ...
            .catch(e=>{console.error(`[DataLayer] Search "${name}":`,e);return [];})
        );
      }
...
const UI = (() => {
  ...
  return {
    ...
    toast(msg,type='info',duration=3000){
      ...
      var maxToasts = 5;
      var currentToasts = _toastContainer.querySelectorAll('.hd-toast');
      while (currentToasts.length >= maxToasts) {
        var oldest = currentToasts[0];
        if (oldest) { oldest.style.opacity = '0'; oldest.style.transform = 'translateX(20px)'; setTimeout(function() { if (oldest.parentNode) oldest.remove(); }, 300); }
        currentToasts = _toastContainer.querySelectorAll('.hd-toast');
      }
      var colors={info:'var(--accent-cyan)',success:'var(--accent-green)',warning:'var(--accent-orange)',error:'var(--accent-red)'};
      var t=this.create('div',{className:'hd-toast hd-toast-' + type,innerHTML:msg,style:{padding:'10px 18px',borderRadius:'10px',background:'var(--bg-card)',border:'1px solid ' + (colors[type]||colors.info),color:'var(--text-primary)',fontSize:'13px',fontFamily:'var(--font-body)',backdropFilter:'blur(10px)',animation:'fadeUp 0.3s ease',boxShadow:'0 0 20px ' + ((colors[type]||colors.info) + '22')}});
      _toastContainer.appendChild(t);
      setTimeout(function(){t.style.opacity='0';t.style.transform='translateX(20px)';setTimeout(function(){if(t.parentNode)t.remove();},300);},duration);
    },
    /**
    ...
    async modal(content,onClose){
      ...
      var controller = Store.createController('modal');
      var signal = controller.signal;

      var overlay=document.querySelector('.hd-modal-overlay');
      var focusTrapBefore = null;
      var focusTrapAfter = null;
      
      if(!overlay){
        overlay=this.create('div',{className:'hd-modal-overlay',innerHTML:'<div tabindex="0" class="hd-modal-focus-start"></div><div class="hd-modal"><button class="hd-modal-close">&times;</button><div class="hd-modal-body"></div></div><div tabindex="0" class="hd-modal-focus-end"></div>',style:{position:'fixed',inset:'0',background:'rgba(0,0,0,0.82)',backdropFilter:'blur(10px)',zIndex:'1000',display:'flex',alignItems:'center',justifyContent:'center',opacity:'0',pointerEvents:'none',transition:'opacity 0.3s'}});
        document.body.appendChild(overlay);
        overlay.querySelector('.hd-modal-close').addEventListener('click',function(){ UI.closeModal(); },{signal});
        overlay.addEventListener('click',function(e){if(e.target===overlay) UI.closeModal();},{signal});
        ...
        if (focusTrapBefore) {
          focusTrapBefore.addEventListener('focus', function() {
            var modal = overlay.querySelector('.hd-modal');
            if (modal) {
     ...
     */
    closeModal(){
      var o=document.querySelector('.hd-modal-overlay');
      if(o){
        o.style.opacity='0';o.style.pointerEvents='none';document.body.style.overflow='';
        // Restore focus to previous element
        var prevFocus = Store.get('modal.previousFocus');
        ...
        var onClose = Store.get('modal.onClose');
        if(onClose){
          try{onClose();}catch(e){Logger.error('Modal onClose error',e,'UI');}
    ...
    escapeUrl(val){
      if(val==null)return '';
      return String(val).replace(/&/g,'&amp;').replace(/"/g,'%22').replace(/'/g,'%27').replace(/</g,'%3C').replace(/>/g,'%3E');
    }
  };
})();
...
// ===== GLOBAL EXPORT =====
window.HuntDrop = {EventBus,PluginRegistry,ComponentRegistry,Config,DataLayer,UI,FeatureFlags,Router,Store,Logger,normalizeImageUrl, getOptimizedImageAttributes};
