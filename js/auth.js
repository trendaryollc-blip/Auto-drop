...
(function () {
  ...
  function waitForFirebase(retries) {
    return new Promise((resolve, reject) => {
      function check() {
        if (window.HuntDrop && window.HuntDrop.firebase && window.HuntDrop.firebase.auth) {
          resolve(window.HuntDrop.firebase);
        } else if (retries > 0) {
  ...
  waitForFirebase(50)
    ...
    .catch((err) => {
      ...
      document.body.innerHTML =
        '<div style="color:#fff;text-align:center;padding:80px 20px;font-family:sans-serif"><h2>Failed to load authentication.</h2><p>Please refresh the page.</p></div>';
    });
