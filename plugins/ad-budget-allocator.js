...
const AdBudgetAllocatorPlugin = {
    ...
    section.innerHTML = `
      <div class="section-inner">
        ...
        ${window.HuntDrop.renderRelatedTools([
          ...
          { section:'section-personas', name:'Customer Persona', desc:'Target the right audience', icon:'👤', color:'#ec4899' },
          { section:'section-profit-lab', name:'Profit Calculator', desc:'Track ROI', icon:'💰', color:'#00ff88' },
          { section:'section-calendar', name:'Content Calendar', desc:'Schedule campaigns', icon:'📅', color:'#f97316' }
        ])}
      </div>`;
  ...
  allocate(){
    ...
    const scored=products.map(p=>{
      const score=p.score*0.3+(p.margin/100)*100*0.25+(100-p.marketSaturation)*0.25+(p.demand*0.2);
    ...
    allocations.forEach(a=>{a.platforms.forEach(p=>{platformTotals[p.name]=(platformTotals[p.name]||0)+p.amount;});});
    ...
    el.innerHTML=`
      <div class="aba-output">
            ...
            ${[1,2,3,4].map(w=>`
              <div class="aba-week">
                ...
                <div class="aba-week-split">
                  ${allocations.slice(0,3).map(a=>`<div class="aba-week-item"><span class="aba-week-dot" style="background:${a.signal==='SCALE NOW'?'var(--accent-green)':a.signal==='PAUSE'?'var(--accent-red)':'var(--accent-orange)'}"></span>${a.product.title.split('—')[0].trim().substring(0,15)}: $${a.weeklyBudget}</div>`).join('')}
                </div>
              </div>
            `).join('')}
          </div>
        ...
        <div class="aba-allocation-grid">
          ${allocations.map((a,i)=>{
            const signalClass=a.signal==='SCALE NOW'?'aba-signal-scale':a.signal==='PAUSE'?'aba-signal-pause':'aba-signal-test';
        ...
        <div class="aba-recommend-card">
          ...
          <div class="aba-rec-grid">
            ...
            <div class="aba-rec-item aba-rec-green">
              ...
              <div><div class="aba-rec-title">Scale These Products</div><div class="abc-rec-desc">${allocations.filter(a=>a.signal==='SCALE NOW').map(a=>a.product.title.split('—')[0].trim()).join(', ')} — high score, low competition. Increase budget 20-30% weekly.</div></div>
            </div>`:''}
            ${allocations.filter(a=>a.signal==='PAUSE').length>0?`
            <div class="aba-rec-item aba-rec-red">
              <div class="aba-rec-icon">&#9208;&#65039;</div>
              <div><div class="aba-rec-title">Pause or Re-evaluate</div><div class="abc-rec-desc">${allocations.filter(a=>a.signal==='PAUSE').map(a=>a.product.title.split('—')[0].trim()).join(', ')} — high competition or low score. Cut spend and test new creatives.</div></div>
            </div>`:''}
