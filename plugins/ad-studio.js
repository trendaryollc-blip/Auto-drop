...
const esc = function(s) { try { return UI.escapeHtml(String(s || '')); } catch { return ''; } };
...
const AdAI = {
  async generate(systemPrompt, userPrompt, opts) {
    opts = opts || {};
    try {
      const featureKey = await (window.HuntDrop.APIKeyManager ? window.HuntDrop.APIKeyManager.getFeatureKey('ad-studio') : { provider: null, key: null });
...
const Prompts = {
  adCopy: function(product, framework) {
    return "You are an expert direct-response copywriter specializing in dropshipping ads.\n\n" +
      "Generate ad copy for this product using the " + framework + " framework.\n\n" +
      "Product: " + product.title + "\n" +
      "Price: $" + product.price + "\n" +
      "Category: " + product.category + "\n" +
      "Keywords: " + (product.keywords || []).join(", ") + "\n" +
      "Rating: " + product.rating + "/5 (" + product.reviews + " reviews)\n" +
      "Target Audience: " + (product.audience ? product.audience.age + " " + product.audience.gender : "General") + "\n" +
      "Interests: " + (product.audience ? (product.audience.interests || []).join(", ") : "") + "\n\n" +
  ...
  hooks: function(product) {
    return "You are a viral video ad hook specialist. Generate 15 attention-grabbing hooks for the first 1-3 seconds of a video ad.\n\n" +
      "Product: " + product.title + "\nPrice: $" + product.price + "\nCategory: " + product.category + "\n" +
  ...
  compliance: function(copy) {
    return "You are a Facebook/TikTok/Instagram advertising policy expert. Review this ad copy for compliance.\n\n" +
      "AD COPY:\n" + copy + "\n\n" +
  ...
  variations: function(product, count) {
    return "Generate " + count + " unique ad copy variations for this product. Each must be meaningfully different.\n\n" +
      "Product: " + product.title + "\nPrice: $" + product.price + "\nCategory: " + product.category + "\n" +
      "Keywords: " + (product.keywords || []).join(", ") + "\n" +
      "Rating: " + product.rating + "/5 (" + product.reviews + " reviews)\n\n" +
  ...
  ugcScript: function(product, type) {
    return "Write a complete UGC video script for a dropshipping ad.\n\n" +
      "Product: " + product.title + "\nPrice: $" + product.price + "\nCategory: " + product.category + "\n" +
      "Script Type: " + type + "\nAudience: " + (product.audience ? product.audience.age + " " + product.audience.gender : "General") + "\n\n" +
  ...
  abTestPlan: function(product) {
    return "Create a comprehensive A/B testing plan for advertising this product.\n\n" +
      "Product: " + product.title + "\nPrice: $" + product.price + "\nCategory: " + product.category + "\n" +
      "Avg CPA: $" + (product.cpaAvg || "5") + "\nAvg Ad Spend: $" + (product.adSpendAvg || "20") + "\n\n" +
  ...
  retargeting: function(product) {
    return "Create a complete retargeting ad sequence for this product.\n\n" +
      "Product: " + product.title + "\nPrice: $" + product.price + "\nCategory: " + product.category + "\n" +
      "Keywords: " + (product.keywords || []).join(", ") + "\n" +
      "Rating: " + product.rating + "/5 (" + product.reviews + " reviews)\n\n" +
  ...
  fatigue: function(product) {
    return "Analyze creative fatigue risk and generate refresh suggestions.\n\n" +
      "Product: " + product.title + "\nCategory: " + product.category + "\n" +
      "Competition: " + product.competition + "\nMarket Saturation: " + (product.marketSaturation || "medium") + "\n\n" +
  ...
  swipeLibrary: function(product) {
    return "Generate a competitor ad swipe library for this niche.\n\n" +
      "Product: " + product.title + "\nCategory: " + product.category + "\nPrice: $" + product.price + "\n" +
      "Keywords: " + (product.keywords || []).join(", ") + "\n\n" +
  ...
  roas: function(product, budget) {
    return "Calculate ROAS predictions and ad spend simulation.\n\n" +
  ...
  audienceMatch: function(product) {
    return "Generate audience-specific ad copy for different customer segments.\n\n" +
      "Product: " + product.title + "\nPrice: $" + product.price + "\nCategory: " + product.category + "\n\n" +
  ...
  creativeBrief: function(product, adType) {
    return "Create a detailed visual creative brief for a " + adType + " ad.\n\n" +
      "Product: " + product.title + "\nPrice: $" + product.price + "\nCategory: " + product.category + "\n" +
      "Audience: " + (product.audience ? product.audience.age + " " + product.audience.gender : "General") + "\n\n" +
  ...
  seasonal: function(product, event) {
    return "Create seasonal/event-specific ad copy for " + event + ".\n\n" +
      "Product: " + product.title + "\nPrice: $" + product.price + "\nCategory: " + product.category + "\n\n" +
      "OUTPUT: " + event.toUpperCase() + " CAMPAIGN with: Campaign Theme, " +
  ...
  lpMatcher: function(product, adCopy) {
    return "Generate landing page copy that matches this ad for message consistency.\n\n" +
      "Product: " + product.title + "\nPrice: $" + product.price + "\nCategory: " + product.category + "\n" +
      "Rating: " + product.rating + "/5 (" + product.reviews + " reviews)\n\n" +
      "AD COPY TO MATCH:\n" + adCopy + "\n\n" +
...
const Fallback = {
  ...
  adCopy: function(product, framework) {
    ...
    if (framework === "PAS") {
      fbCopy = "Stop wasting money on " + kw + " that don't work.\n\nThis " + title + " has helped " + reviews + "+ customers solve their " + kw + " problems \u2014 and it's " + disc + "% off today.\n\n\u2705 " + rating + "/5 stars\n\u2705 Free shipping\n\u2705 30-day money-back guarantee\n\nDon't wait \u2014 this deal won't last.";
      ...
      fbCopy = "\uD83D\uDD25 Introducing the " + title + " \u2014 the " + kw + " that's taking the internet by storm!\n\n\u2728 Why " + reviews + "+ customers love it:\n\u2192 Premium quality construction\n\u2192 " + rating + "/5 average rating\n\u2192 Results in just days\n\u2192 Free express shipping\n\n\u26A1 Limited time: " + disc + "% OFF + Free bonuses\n\n\uD83D\uDC46 Tap Shop Now before this deal expires!";
      headline = "The " + kw + " That " + reviews + "+ People Are Obsessed With";
      desc = "Rated " + rating + "/5 by " + reviews + "+ happy customers. Order risk-free with our 30-day guarantee.";
    } else if (framework === "Before/After Bridge") {
      fbCopy = "Before: Struggling with " + kw + " that never delivered.\nAfter: Life-changing results with the " + title + ".\n\nBridge? Just one order.\n\n" + reviews + "+ verified buyers can't be wrong:\n\u2B50\u2B50\u2B50\u2B50\u2B50 This changed everything!\n\u2B50\u2B50\u2B50\u2B50\u2B50 Best purchase I've made all year\n\nGet yours " + disc + "% off \u2192 Limited stock!";
      ...
      fbCopy = "\u26A1 URGENT: " + disc + "% OFF ends at midnight!\n\n\u2705 USEFUL: The " + title + " that actually works\n\u2705 UNIQUE: Premium " + kw + " you won't find anywhere else\n\u2705 ULTRA-SPECIFIC: " + rating + "/5 from " + reviews + "+ reviews\n\u2705 URGENT: Only " + Math.floor(Math.random()*50+20) + " left at this price\n\nDon't regret missing this. \uD83D\uDC47";
      headline = title + " \u2014 " + disc + "% Off (Ends Tonight!)";
      desc = "Only " + Math.floor(Math.random()*50+20) + " left. " + rating + "/5 stars. Free shipping + 30-day guarantee.";
    } else {
      fbCopy = "Meet " + (["Sarah","Emma","Jessica","Lisa","Rachel"][Math.floor(Math.random()*5)]) + " \uD83D\uDC4B\n\nShe was tired of " + kw + " that promised everything and delivered nothing.\n\nThen she tried the " + title + "...\n\n\u2728 " + rating + "/5 star rating\n\u2728 " + reviews + "+ verified reviews\n\u2728 " + disc + "% off this week only\n\nHer story could be your story. Try it risk-free \u2192";
      headline = title + " \u2014 See Why Everyone's Switching";
      desc = "Join " + reviews + "+ happy customers. 30-day money-back guarantee.";
    }

    const tiktok = "## TIKTOK\n" +
      "### Hook (0-3s)\n\"POV: You finally found the " + kw + " that actually works...\"\n" +
      "### Script (3-15s)\nShow product in use \u2192 highlight key feature \u2192 show reviews counter \u2192 urgency (limited stock)\n" +
      "### CTA\n\"Link in bio \uD83D\uDC46 \u2014 " + disc + "% off ends tonight!\"\n" +
      "### Caption\nThe " + kw + " everyone's been asking about \uD83E\uDD29 #trending #musthave";

    const ig = "## INSTAGRAM REELS\n" +
      "### Caption\nThe " + kw + " that's breaking the internet \uD83D\uDD25\u2728 Link in bio! #" + kw.replace(/\s/g,"") + " #viral #fyp\n" +
      "### Hashtags\n#" + kw.replace(/\s/g,"") + " #trending #viral #shopping #deal #musthave";
  ...
  hooks: function(product) {
    ...
    lines.push("6. \"I found the " + kw + " that Amazon doesn't want you to know about...\" [Score: 7/10 | YouTube/FB]");
    lines.push("");
    lines.push("## SOCIAL PROOF");
    lines.push("7. " + reviews + "+ people bought this " + kw + " last week. Here's why... [Score: 9/10 | All]");
    lines.push("8. \"This " + title + " has " + product.rating + " stars from " + reviews + " reviews...\" [Score: 8/10 | FB/IG]");
    ...
    lines.push("14. \"This " + kw + " is better than brands charging 3x more. Fight me.\" [Score: 8/10 | TikTok/Twitter]");
  ...
  variations: function(product, count) {
    ...
    const rating = product.rating;
    const angles = [
      { angle: "Fear of Missing Out", tone: "Urgent", copy: "\u23F0 Only " + Math.floor(Math.random()*30+10) + " left! The " + title + " is selling out FAST. " + reviews + "+ people grabbed yours this week. Don't be the one who waits too long \u2192" },
      { angle: "Social Proof", tone: "Confident", copy: "\u2B50\u2B50\u2B50\u2B50\u2B50 " + reviews + "+ five-star reviews can't be wrong. The " + title + " is the #1 rated " + kw + " of 2026. See why everyone's switching \u2192" },
      { angle: "Value/Deal", tone: "Excited", copy: "\uD83D\uDD25 " + disc + "% OFF the " + title + " \u2014 premium quality, factory price. Limited time only \u2192" },
      { angle: "Problem/Solution", tone: "Empathetic", copy: "Tired of " + kw + " that don't deliver? Same. That's why we found the " + title + " \u2014 the last " + kw + " you'll ever need. " + rating + "/5 stars. 30-day guarantee." },
      { angle: "Lifestyle/Aspiration", tone: "Premium", copy: "Upgrade your " + product.category + " game \u2728 The " + title + " is what " + reviews + "+ people chose when they got serious about quality. Premium materials. Stunning design. Free shipping." },
      { angle: "Comparison", tone: "Direct", copy: "Brand " + kw + ": $80+. " + title + ": Same quality, fraction of the price. " + reviews + "+ customers already made the smart switch. " + disc + "% off this week only." },
      { angle: "Curiosity", tone: "Intriguing", copy: "There's a " + kw + " that 500+ people are ordering every day... and no one's talking about it yet. The " + title + ". Get it before everyone else does \u2192" },
      { angle: "Urgency + Value", tone: "Excited", copy: "\u26A1 FLASH SALE: " + title + " is " + disc + "% off for the next 24 hours. " + rating + "/5 stars. Free shipping. Risk-free with 30-day guarantee. This won't come around again." },
      { angle: "Testimonial", tone: "Authentic", copy: "\"I was skeptical at first, but this " + title + " literally changed my daily routine. 10/10 would recommend!\" \u2014 Verified Buyer \u2B50\u2B50\u2B50\u2B50\u2B50\n\nJoin " + reviews + "+ happy customers \u2192" },
      { angle: "Anti-Competitor", tone: "Bold", copy: "We tested 20+ " + kw + " brands. The " + title + " won by a landslide. " + rating + "/5 from " + reviews + " real reviews. No gimmicks \u2014 just the best product at the best price." }
    ];
    const out = [];
  ...
  ugcScript: function(product, _type) {
    ...
    lines.push("Dialogue: \"Okay first impressions \u2014 this feels amazing. Look at this quality... and it's only $" + price + "?!\"");
    ...
    lines.push("Dialogue: \"Link in bio if you want to grab one \u2014 but honestly don't wait, these sell out fast\"");
  ...
  abTestPlan: function(product) {
    ...
    lines.push("Systematic A/B testing for " + product.title + ". One variable at a time, minimum $" + dailyBudget + "/day per variation.");
    lines.push("");
    ...
    lines.push("## KEY METRICS: CTR >2% (FB) >1% (TikTok) | CPA <$" + (product.cpaAvg || "5") + " | Hook rate | Conversion rate");
    return lines.join("\n");
  },
  ...
  retargeting: function(product) {
    ...
    lines.push(reviews + "+ people bought the " + title + " this month alone. With a " + rating + "/5 star rating, it's the most trusted " + kw + " of 2026. See what the hype is about \u2192");
    ...
    lines.push("Still thinking? " + rating + "/5 star quality | Free shipping | 30-day guarantee | " + disc + "% off (ending soon)");
    ...
    lines.push("### Facebook/Instagram Copy");
    lines.push("\u23F0 FINAL NOTICE: The " + title + " deal expires in 48 hours. " + disc + "% off + free shipping ends Friday.");
  ...
  fatigue: function(product) {
    ...
    lines.push("Reasoning: " + kw + " niche has " + saturation + " saturation with " + product.competition + " competition.");
    lines.push("");
    ...
    lines.push("### Variation 2: Visual Refresh \u2014 UGC-style instead of product-focused. Expected: 20-30% CPM reduction");
    lines.push("### Variation 3: CTA Swap \u2014 \"Get Yours Before It's Gone\". Expected: 10-20% conversion lift");
    lines.push("### Variation 4: Emotional Pivot \u2014 Problem-to-solution approach. Expected: 20-40% CTR recovery");
  ...
  swipeLibrary: function(product) {
    ...
    lines.push("### Pattern 2: The Problem-Solution [Conversion] \u2014 Pain point question, Frustration \u2192 Relief");
  ...
  roas: function(product, budget) {
    ...
    lines.push("### Conservative: " + (product.price / (cpa * 1.5)).toFixed(1) + "x ROAS | ~" + conservSales + " sales/day | $" + ((conservSales * profitPerSale) - dailyBudget).toFixed(2) + " profit");
    lines.push("### Expected: " + (product.price / cpa).toFixed(1) + "x ROAS | ~" + expectSales + " sales/day | $" + ((expectSales * profitPerSale) - dailyBudget).toFixed(2) + " profit");
    lines.push("### Optimistic: " + (product.price / (cpa * 0.7)).toFixed(1) + "x ROAS | ~" + optimSales + " sales/day | $" + ((optimSales * profitPerSale) - dailyBudget).toFixed(2) + " profit");
    lines.push("");
    lines.push("## BUDGET RECOMMENDATIONS");
    lines.push("Facebook: $" + Math.max(dailyBudget, 10) + "/day | Scale: ROAS >2x for 3 days | Kill: ROAS <1x after $" + (dailyBudget * 3));
    lines.push("TikTok: $" + Math.max(Math.floor(dailyBudget * 0.6), 10) + "/day | Scale: CTR >1.5% | Kill: CPA >$" + (profitPerSale * 1.5).toFixed(2));
    lines.push("");
  ...
  audienceMatch: function(product) {
    ...
    lines.push("FB/IG: ok this " + kw + " is actually fire \uD83D\uDD25 " + reviews + "+ people copped this already. " + rating + "/5?? say less. link in bio fr");
    ...
    lines.push("FB/IG: Looking for a reliable " + kw + "? The " + title + " is rated " + rating + "/5 by " + reviews + "+ verified customers. Premium quality. 30-day guarantee. " + disc + "% off this week.");
    ...
    lines.push("FB/IG: I don't usually post about products, but the " + title + " earned it. " + reviews + "+ verified reviews. " + rating + "/5 stars. 30-day guarantee. If you've been on the fence, this is your sign.");
  ...
  creativeBrief: function(product, adType) {
    const kw = product.keywords[0] || product.category;
    const ratio = adType === "Story" ? "9:16 (1080x1920)" : adType === "Feed" ? "1:1 (1080x1080)" : "4:5 (1080x1350)";
    const lines = [];
  ...
  seasonal: function(product, event) {
    ...
    lines.push("FB/IG: Something big is coming... \uD83D\uDC08 The " + title + " drops to its LOWEST price ever. " + event + " exclusive: Up to " + disc + "% off.");
    lines.push("TikTok: \"Plot twist: the " + kw + " is about to go ON SALE...\" Turn on notifications.");
    lines.push("");
    lines.push("### LAUNCH DAY");
    lines.push("FB/IG: \uD83C\uDF89 IT'S HERE! " + event + " deals are LIVE! " + title + " \u2014 NOW " + disc + "% OFF! " + rating + "/5 stars. " + reviews + "+ happy customers. Free shipping + guarantee.");
    lines.push("TikTok: \"" + event + " drop just went LIVE and this " + kw + " is " + disc + "% off!!!\" Link in bio.");
    lines.push("");
    lines.push("### MID-EVENT URGENCY");
    lines.push("FB/IG: \u26A0\uFE0F " + event + " update: " + title + " is 60% claimed! " + disc + "% off ends in [X] hours.");
    ...
    lines.push("TikTok: \"FINAL HOURS. " + event + " deal expires at midnight. Link in bio \u2014 I can't help you after tonight.\"");
    lines.push("");
    lines.push("### POST-EVENT");
    lines.push("" + event + " is over but the " + title + " is still a bestseller. Sign up for alerts for next sale.");
    lines.push("");
...
const EVENTS = ["Black Friday", "Cyber Monday", "Back to School", "Valentine's Day", "Christmas", "New Year", "Summer Sale", "Prime Day", "Mother's Day", "Father's Day"];
...
const AdStudioPlugin = {
  ...
  mount: function(_ctx) {
    ...
    } catch(e) { console.error("[AdStudio] mount error:", e); }
  },

  unmount: function(_ctx) {
    try {
      (AdStudioPlugin._cleanups||[]).forEach(function(fn){ try{fn();}catch{/* ignored */} });
      AdStudioPlugin._cleanups = [];
      if (AdStudioPlugin.section) { AdStudioPlugin.section.remove(); AdStudioPlugin.section = null; }
    } catch {/* ignored */}
  },
  ...
  renderHTML: function() {
    ...
    products.forEach(function(p) {
      opts += "<option value=\"" + esc(p.id) + "\">" + esc(p.title.split("\u2014")[0].trim()) + " ($" + esc(p.price) + ")</option>";
    });
    let tabsHtml = "";
    ...
    return '<div class="section-inner">' +
      ...
      '<div class="ads-product-bar"><div class="ads-product-select-wrap">' +
        ...
        '<select id="adsProductSelect" class="ads-product-select">' + opts + '</select>' +
      '</div><div class="ads-product-info" id="adsProductInfo"></div></div>' +
      '<div class="ads-tabs" id="adsTabs">' + tabsHtml + '</div>' +
  ...
  renderInputArea: function(tabId) {
    ...
    const btn = function(action, label) {
      return '<button class="ads-generate-btn" data-action="' + action + '"><span class="ai-sparkle">\u2728</span> ' + label + '</button>';
    };
    switch(tabId) {
      case "copy":
        h += "<h3>\uD83E\uDDE0 AI Ad Copy Generator</h3><p class=\"ads-input-desc\">Generate platform-specific ad copy using proven direct-response frameworks</p>";
        h += '<div class="ads-input-row"><select id="adsFramework" class="ads-select">';
        FRAMEWORKS.forEach(function(f) { h += '<option value="' + f + '">' + f + '</option>'; });
        h += '</select>' + btn("generateCopy", "Generate Ad Copy") + '</div>';
        break;
      case "platforms":
        h += "<h3>\uD83C\uDFAF Multi-Platform Ad Generator</h3><p class=\"ads-input-desc\">Generate platform-native ad sets for Facebook, TikTok, Instagram, YouTube, and Pinterest</p>";
        h += '<div class="ads-input-row">' + btn("generatePlatforms", "Generate All Platforms") + '</div>';
        break;
      case "hooks":
        h += "<h3>\uD83D\uDD25 Winning Hook Generator</h3><p class=\"ads-input-desc\">Generate 15 attention-grabbing hooks for the first 1-3 seconds</p>";
        h += '<div class="ads-input-row">' + btn("generateHooks", "Generate Hooks") + '</div>';
        break;
      case "compliance":
        h += "<h3>\u2705 Ad Compliance Checker</h3><p class=\"ads-input-desc\">Check ad copy against Facebook, TikTok, and Instagram policies</p>";
        h += '<div class="ads-input-row"><textarea id="adsComplianceInput" class="ads-textarea" placeholder="Paste your ad copy here..." rows="5"></textarea></div>';
        h += '<div class="ads-input-row">' + btn("checkCompliance", "Check Compliance") + '</div>';
        break;
      case "variations":
        h += "<h3>\uD83E\uDDEA Ad Copy Variation Matrix</h3><p class=\"ads-input-desc\">Generate unique ad variations across emotional angles, tones, and lengths</p>";
        h += '<div class="ads-input-row"><label class="ads-label">Count:</label><select id="adsVariationCount" class="ads-select"><option value="5">5</option><option value="10" selected>10</option><option value="15">15</option><option value="20">20</option></select>';
        h += btn("generateVariations", "Generate Variations") + '</div>';
        break;
      case "ugc":
        h += "<h3>\uD83C\uDFAC UGC Script Generator</h3><p class=\"ads-input-desc\">Generate complete UGC-style video scripts with dialogue and production notes</p>";
        h += '<div class="ads-input-row"><select id="adsUGCType" class="ads-select">';
        UGC_TYPES.forEach(function(t) { h += '<option value="' + t + '">' + t + '</option>'; });
        h += '</select>' + btn("generateUGC", "Generate Script") + '</div>';
        break;
      case "abtest":
        h += "<h3>\uD83D\uDCCA A/B Test Plan Generator</h3><p class=\"ads-input-desc\">Create a structured 4-week testing plan with budget allocation</p>";
        h += '<div class="ads-input-row">' + btn("generateABTest", "Generate Test Plan") + '</div>';
        break;
      case "retarget":
        h += "<h3>\uD83D\uDD04 Retargeting Sequence Builder</h3><p class=\"ads-input-desc\">Generate a complete 4-stage retargeting funnel</p>";
        h += '<div class="ads-input-row">' + btn("generateRetarget", "Generate Sequence") + '</div>';
        break;
      case "fatigue":
        h += "<h3>\uD83D\uDCA1 Creative Fatigue Detector</h3><p class=\"ads-input-desc\">Analyze fatigue risk and get 5 fresh creative variations</p>";
        h += '<div class="ads-input-row">' + btn("generateFatigue", "Analyze & Refresh") + '</div>';
        break;
      case "swipe":
        h += "<h3>\uD83C\uDFC6 Competitor Ad Swipe Library</h3><p class=\"ads-input-desc\">Discover winning ad patterns, hooks, and copy angles for your niche</p>";
        h += '<div class="ads-input-row">' + btn("generateSwipe", "Build Swipe Library") + '</div>';
        break;
      case "roas":
        h += "<h3>\uD83D\uDCC8 ROAS Prediction & Ad Spend Simulator</h3><p class=\"ads-input-desc\">Calculate break-even CPA, predict ROAS, and model ad spend scenarios</p>";
        h += '<div class="ads-input-row"><label class="ads-label">Daily Budget ($):</label><input type="number" id="adsBudget" class="ads-input-num" value="20" min="5" max="10000">';
        h += btn("generateROAS", "Calculate ROAS") + '</div>';
        break;
      case "audience":
        h += "<h3>\uD83C\uDFAF Audience-Ad Match Optimizer</h3><p class=\"ads-input-desc\">Generate different ad copy for Gen Z, Millennials, and Gen X</p>";
        h += '<div class="ads-input-row">' + btn("generateAudience", "Generate Per Audience") + '</div>';
        break;
      case "briefs":
        h += "<h3>\uD83D\uDCF1 Creative Brief Generator</h3><p class=\"ads-input-desc\">Generate visual direction, shot lists, and AI image prompts</p>";
        h += '<div class="ads-input-row"><select id="adsAdType" class="ads-select">';
        AD_TYPES.forEach(function(t) { h += '<option value="' + t + '">' + t + '</option>'; });
        h += '</select>' + btn("generateBrief", "Generate Brief") + '</div>';
        break;
      case "seasonal":
        h += "<h3>\u23F0 Seasonal Ad Templates</h3><p class=\"ads-input-desc\">Generate event-specific ad campaigns with urgency tactics</p>";
        h += '<div class="ads-input-row"><select id="adsEvent" class="ads-select">';
        EVENTS.forEach(function(e) { h += '<option value="' + e + '">' + e + '</option>'; });
        h += '</select>' + btn("generateSeasonal", "Generate Campaign") + '</div>';
        break;
      case "lp":
        h += "<h3>\uD83D\uDCCB Landing Page Copy Matcher</h3><p class=\"ads-input-desc\">Generate landing page copy that matches your ad</p>";
        h += '<div class="ads-input-row"><textarea id="adsLPInput" class="ads-textarea" placeholder="Paste your ad copy to match (or leave empty)..." rows="4"></textarea></div>';
  ...
  bindEvents: function() {
    ...
    this.section.querySelectorAll(".ads-tab").forEach(function(tab) {
      tab.addEventListener("click", function() { self.switchTab(tab.getAttribute("data-tab")); });
    });
    const ps = this.section.querySelector("#adsProductSelect");
    if (ps) ps.addEventListener("change", function() { self.updateProductInfo(ps.value); });
    const ia = this.section.querySelector("#adsInputArea");
    if (ia) ia.addEventListener("click", function(e) {
      const btn = e.target.closest("[data-action]");
      if (btn) self.handleAction(btn.getAttribute("data-action"));
    });
    try {
      const unsub = EventBus.on("aikeys:changed", function() { self.updateAIStatus(); });
  ...
  updateProductInfo: function(productId) {
    const products = window.HuntDrop.ALL_PRODUCTS || [];
    this.currentProduct = products.find(function(p) { return String(p.id) === String(productId); }) || null;
    const info = this.section ? this.section.querySelector("#adsProductInfo") : null;
    if (!info) return;
    if (!this.currentProduct) { info.innerHTML = ""; return; }
    const p = this.currentProduct;
    info.innerHTML = '<span class="ads-pi-badge">Score: ' + p.score + '/100</span>' +
      '<span class="ads-pi-badge">Margin: ' + p.margin + '%</span>' +
      '<span class="ads-pi-badge">Rating: ' + p.rating + '\u2B50</span>' +
      '<span class="ads-pi-badge">' + p.reviews + ' reviews</span>';
  },
  ...
  updateAIStatus: function() {
    ...
    try {
      ...
      if (status && status.hasKey) {
        el.innerHTML = '<div class="ads-ai-connected"><span class="ads-ai-dot ads-ai-dot-green"></span>AI Connected: <strong>' + esc(status.providerName) + '</strong> \u00B7 ' + esc(status.model) + '</div>';
        el.className = "ads-ai-status ads-ai-connected-wrap";
      } else {
        el.innerHTML = '<div class="ads-ai-disconnected"><span class="ads-ai-dot ads-ai-dot-yellow"></span>No AI key configured \u2014 using template mode. <a href="#" id="adsAISettingsLink">Add API key in Settings \u2192</a></div>';
        el.className = "ads-ai-status ads-ai-disconnected-wrap";
        const link = el.querySelector("#adsAISettingsLink");
        if (link) link.addEventListener("click", function(e) { e.preventDefault(); try { EventBus.emit("navigate", { section: "section-ai-settings" }); } catch {/* ignored */} });
      }
    } catch { el.innerHTML = ""; }
  },
    ...
    try {
      ...
      switch(action) {
        ...
        case "checkCompliance": {
          ...
          if (!copy) { this.showResults('<div class="ads-error">\u26A0\uFE0F Please paste ad copy to check.</div>'); break; }
  ...
  generateFeature: async function(feature, product, extra, extraNum) {
    ...
    const sys = "You are an expert dropshipping ad copywriter. Generate detailed, actionable, platform-specific ad content with clear markdown headers.";
    let up;
    switch(feature) {
      case "adCopy": up = pb(product, extra || "PAS"); break;
      case "hooks": up = pb(product); break;
      case "compliance": up = pb(extra || ""); break;
      case "variations": up = pb(product, extraNum || 10); break;
      case "ugc": up = pb(product, extra || "Unboxing"); break;
      case "abTest": up = pb(product); break;
      case "retarget": up = pb(product); break;
      case "fatigue": up = pb(product); break;
      case "swipeLibrary": up = pb(product); break;
      case "roas": up = pb(product, extra || "20"); break;
      case "audienceMatch": up = pb(product); break;
      case "creativeBrief": up = pb(product, extra || "Feed"); break;
      case "seasonal": up = pb(product, extra || "Black Friday"); break;
      case "lpMatcher": up = pb(product, extra || ""); break;
      default: return "Unknown feature.";
    }
    const ai = await AdAI.generate(sys, up, { maxTokens: 4000 });
    if (ai.ok && ai.text) return '<div class="ads-ai-badge">Generated by ' + esc(ai.provider) + '</div>\n\n' + ai.text;
    if (fb) {
      try {
        let ft;
        switch(feature) {
          case "adCopy": ft = fb(product, extra || "PAS"); break;
          case "hooks": ft = fb(product); break;
          case "compliance": ft = fb(extra || ""); break;
          case "variations": ft = fb(product, extraNum || 10); break;
          case "ugc": ft = fb(product, extra || "Unboxing"); break;
          case "abTest": ft = fb(product); break;
          case "retarget": ft = fb(product); break;
          case "fatigue": ft = fb(product); break;
          case "swipeLibrary": ft = fb(product); break;
          case "roas": ft = fb(product, extra || "20"); break;
          case "audienceMatch": ft = fb(product); break;
          case "creativeBrief": ft = fb(product, extra || "Feed"); break;
          case "seasonal": ft = fb(product, extra || "Black Friday"); break;
          case "lpMatcher": ft = fb(product, extra || ""); break;
          default: ft = "Feature not available.";
        }
        return '<div class="ads-fallback-badge">Template mode \u2014 Add AI key in Settings for personalized generation</div>\n\n' + ft;
      } catch(e2) { return '<div class="ads-error">Fallback error: ' + esc(e2.message) + '</div>'; }
    }
  ...
  renderMarkdown: function(text) {
    ...
    } catch { return '<div class="ads-md"><pre>' + esc(text) + '</pre></div>'; }
  },

  showLoading: function(show) {
    try { const el = this.section ? this.section.querySelector("#adsLoading") : null; if (el) el.style.display = show ? "flex" : "none"; } catch {/* ignored */}
  },

  showResults: function(html) {
    try { const el = this.section ? this.section.querySelector("#adsResults") : null; if (el) el.innerHTML = html || ""; } catch {/* ignored */}
  }
};
...
PluginRegistry.register("ad-studio", AdStudioPlugin);
  ...
  try {
    window.HuntDrop.PluginRegistry.register("ad-studio", {
      id: "ad-studio", name: "Ad Creative Studio", version: "2.0.0",
      init: function(){}, mount: function(){}, unmount: function(){}
    });
    } catch {/* ignored */}
}
})();