...
(function () {
        ...
        for (var w = 0; w < words.length; w++) {
          ...
          if (p.keywords) {
            for (var k2 = 0; k2 < p.keywords.length; k2++) {
              if (p.keywords[k2].toLowerCase().indexOf(words[w]) !== -1) {
                matchCount++;
                break;
              }
            }
  ...
  function calculateProfit(product) {
    ...
    return {
      ...
      roi: roi,
      breakEven: be,
      profitMargin: sp > 0 ? +((net / sp) * 100).toFixed(0) : 0,
    };
  }

  function getRiskLevel(s) {
    return s < 25 ? 'low' : s < 50 ? 'med' : 'high';
  }
  function getRiskColor(s) {
    return s < 25 ? 'var(--accent-green)' : s < 50 ? 'var(--accent-yellow)' : 'var(--accent-red)';
  }
  function getDotColor(l) {
    return l === 'low' ? 'green' : l === 'med' ? 'yellow' : 'red';
  }
  function cap(s) {
  ...
  function generateKeywordData(product) {
    ...
    return kws
      .map(function (kw) {
        return {
          word: kw,
          volume: Math.floor(base * (0.2 + Math.random() * 1.8)),
  ...
  async function getAIInsight(product, profit, riskAnalysis) {
    ...
    try {
      var prompt =
        'Analyze this dropshipping product and give a concise 2-3 sentence verdict. ' +
        'Product: ' +
        product.title +
        ' | Category: ' +
        product.category +
        ...
        '%' +
        ' | Win Probability: ' +
        (riskAnalysis ? riskAnalysis.winProbability : 'N/A') +
        '%' +
        ' | Orders: ' +
        product.orders +
        ' | Rating: ' +
  ...
  function buildFallbackInsight(product, profit, riskAnalysis) {
    ...
    return (
      ...
      '. Key risks: ' +
      (risks.length > 0 ? risks.join(', ') : 'minimal') +
      '.'
    );
  }
  ...
  async function searchWebCompetitors(product) {
    ...
    try {
      ...
      console.warn('[AIAnalyst] Web competitor search failed:', e);
    }
    return null;
  }
  ...
  async function searchWebKeywords(product) {
    ...
    try {
      ...
      console.warn('[AIAnalyst] Web keyword search failed:', e);
    }
    return null;
  }
  ...
  function renderLoading(text, sub) {
    return (
      ...
      '<div class="aa-loading-text">' +
      UI.escapeHtml(text) +
      '</div>' +
      '<div class="aa-loading-sub">' +
      UI.escapeHtml(sub) +
      '</div>' +
      '</div>'
    );
  ...
  function renderResults(match, profit, riskAnalysis, competitors, keywordData, webComps, webKeywords, aiInsight) {
    ...
    var relatedProducts = (window.HuntDrop.ALL_PRODUCTS || [])
      .filter(function (p) {
    ...
    return (
      ...
      '<span class="aa-product-platform">' +
      esc(match.platform) +
      '</span>' +
      '<span class="aa-product-price-tag">$' +
      profit.sellPrice.toFixed(2) +
      '</span>' +
      '<span class="aa-product-sold">' +
      fmtN(match.orders) +
      ' orders \u00b7 ' +
      match.rating +
      '\u2605 \u00b7 ' +
      ...
      '</div>' +
      '</div>' +
      '<div class="aa-verdict ' +
      (verdict ? 'aa-verdict-go' : 'aa-verdict-no') +
      '">' +
      (verdict ? 'RECOMMENDED' : 'HIGH RISK') +
      '</div>' +
      '</div>' +
      '<div class="aa-grid-4">' +
      '<div class="aa-stat-card"><div class="aa-stat-value" style="color:var(--accent-green)">' +
      match.score +
      '/100</div><div class="aa-stat-label">AI Score</div></div>' +
      '<div class="aa-stat-card"><div class="aa-stat-value" style="color:var(--accent-cyan)">' +
      match.demand +
      '/100</div><div class="aa-stat-label">Demand</div></div>' +
      '<div class="aa-stat-card"><div class="aa-stat-value" style="color:' +
      getRiskColor(match.riskScore) +
      '">' +
      match.riskScore +
      '/100</div><div class="aa-stat-label">Risk</div></div>' +
      '<div class="aa-stat-card"><div class="aa-stat-value" style="color:var(--accent-orange)">' +
      match.marketSaturation +
      '%</div><div class="aa-stat-label">Saturation</div></div>' +
      '</div>' +
  ...
  function renderRiskCard(match, profit) {
    var items = [
      {
        ...
        level: getRiskLevel(match.marketSaturation < 40 ? 15 : match.marketSaturation < 65 ? 40 : 70),
        text:
          match.marketSaturation < 40
            ? 'Low competition \u2014 room to enter'
            : match.marketSaturation < 65
              ? 'Moderate \u2014 differentiate your offer'
              : 'High \u2014 crowded market',
      },
      {
        label: 'Product Risk',
        level: getRiskLevel(match.riskScore),
        text:
          match.riskScore < 25
            ? 'Stable product with proven demand'
            : match.riskScore < 50
              ? 'Some volatility \u2014 monitor trends'
              : 'Trendy \u2014 may fade quickly',
      },
      {
        label: 'Ad Cost Risk',
        level: getRiskLevel(match.adSpendAvg < 5 ? 15 : match.adSpendAvg < 10 ? 40 : 70),
        text:
          match.adSpendAvg < 5
            ? 'Low CPA \u2014 affordable to advertise'
            : match.adSpendAvg < 10
              ? 'Moderate CPA \u2014 requires optimization'
              : 'High CPA \u2014 need strong conversion',
      },
      {
        label: 'Supplier Risk',
        level: getRiskLevel(match.suppliers.length >= 3 ? 15 : match.suppliers.length >= 2 ? 40 : 70),
        text:
          match.suppliers.length >= 3
            ? 'Multiple verified suppliers'
            : match.suppliers.length >= 2
              ? 'Limited suppliers \u2014 backup recommended'
              : 'Few options \u2014 negotiate carefully',
      },
      {
        label: 'Demand Stability',
        level: getRiskLevel(match.orders > 500 ? 15 : match.orders > 100 ? 40 : 70),
        text:
          match.orders > 500
            ? 'High volume \u2014 proven demand'
            : match.orders > 100
              ? 'Moderate sales \u2014 growing'
              : 'Lower volume \u2014 test before scaling',
      },
      {
        label: 'Margin Risk',
        level: getRiskLevel(profit.netProfit > 15 ? 15 : profit.netProfit > 8 ? 40 : 70),
        text:
          profit.netProfit > 15
            ? 'Healthy margins \u2014 strong buffer'
            : profit.netProfit > 8
              ? 'Acceptable margins'
    ...
    var html =
      '<div class="aa-risk-card"><div class="aa-risk-header"><span class="aa-card-icon">\ud83d\udee1</span>Risk Assessment</div><div class="aa-risk-grid">';
    for (var i = 0; i < items.length; i++) {
      html +=
        '<div class="aa-risk-item risk-' +
        items[i].level +
        '">' +
        '<div class="aa-risk-dot dot-' +
        getDotColor(items[i].level) +
        '"></div>' +
        '<div><div class="aa-risk-label">' +
        items[i].label +
        '</div>' +
        '<div class="aa-risk-text">' +
        items[i].text +
        '</div></div></div>';
    }
    return html + '</div></div>';
  ...
  function renderOverviewPanel(match, profit, riskAnalysis, aiInsight) {
    ...
    return (
      ...
      (aiInsight
        ? '<p class="aa-card-text">' + esc(aiInsight) + '</p>'
        : '<p class="aa-card-text">' + esc(match.aiInsight || 'Analyzing...') + '</p>') +
      '</div>' +
      '<div class="aa-grid-4" style="margin-top:14px">' +
      '<div class="aa-stat-card"><div class="aa-stat-value" style="font-size:18px;color:var(--accent-green)">$' +
      profit.netProfit.toFixed(2) +
      '</div><div class="aa-stat-label">Net Profit / Sale</div></div>' +
      '<div class="aa-stat-card"><div class="aa-stat-value" style="font-size:18px;color:var(--accent-cyan)">' +
      profit.roi +
      '%</div><div class="aa-stat-label">ROI</div></div>' +
      '<div class="aa-stat-card"><div class="aa-stat-value" style="font-size:18px;color:var(--accent-orange)">' +
      fmtN(match.salesVelocity) +
      '/mo</div><div class="aa-stat-label">Sales Velocity</div></div>' +
      '<div class="aa-stat-card"><div class="aa-stat-value" style="font-size:18px;color:var(--accent-purple)">' +
      (riskAnalysis ? riskAnalysis.winProbability : winProb) +
      '%</div><div class="aa-stat-label">Win Probability</div></div>' +
      '</div>' +
  ...
  function renderRiskFactors(riskAnalysis) {
    ...
    if (riskAnalysis.factors.positive.length > 0) {
      html +=
        '<div><div style="font-size:12px;font-weight:600;color:var(--accent-green);margin-bottom:6px">Strengths</div><ul style="list-style:none;padding:0;margin:0">';
      for (var i = 0; i < riskAnalysis.factors.positive.length; i++) {
        html +=
          '<li style="font-size:12px;color:var(--text-secondary);padding:3px 0">\u2192 ' +
          UI.escapeHtml(riskAnalysis.factors.positive[i]) +
          '</li>';
      }
      html += '</ul></div>';
    }
    if (riskAnalysis.factors.negative.length > 0) {
      html +=
        '<div><div style="font-size:12px;font-weight:600;color:var(--accent-red);margin-bottom:6px">Risks</div><ul style="list-style:none;padding:0;margin:0">';
      for (var j = 0; j < riskAnalysis.factors.negative.length; j++) {
        html +=
          '<li style="font-size:12px;color:var(--text-secondary);padding:3px 0">\u2192 ' +
          UI.escapeHtml(riskAnalysis.factors.negative[j]) +
          '</li>';
      }
      html += '</ul></div>';
    }
    if (riskAnalysis.factors.neutral.length > 0) {
      html +=
        '<div style="grid-column:1/-1"><div style="font-size:12px;font-weight:600;color:var(--accent-cyan);margin-bottom:6px">Neutral</div><ul style="list-style:none;padding:0;margin:0">';
      for (var k = 0; k < riskAnalysis.factors.neutral.length; k++) {
        html +=
          '<li style="font-size:12px;color:var(--text-secondary);padding:3px 0">\u2192 ' +
          UI.escapeHtml(riskAnalysis.factors.neutral[k]) +
          '</li>';
      }
      html += '</ul></div>';
    }
  ...
  function renderProfitPanel(match, profit) {
    ...
    var html =
      ...
      '<div class="aa-profit-item"><span class="aa-profit-label">Sell Price (Amazon)</span><span class="aa-profit-value aa-profit-pos">$' +
      profit.sellPrice.toFixed(2) +
      '</span></div>' +
      '<div class="aa-profit-item"><span class="aa-profit-label">Product Cost</span><span class="aa-profit-value aa-profit-neg">-$' +
      profit.productCost.toFixed(2) +
      '</span></div>' +
      '<div class="aa-profit-item"><span class="aa-profit-label">Shipping</span><span class="aa-profit-value aa-profit-neg">-$' +
      profit.shippingCost.toFixed(2) +
      '</span></div>' +
      '<div class="aa-profit-item"><span class="aa-profit-label">Platform Fee (15%)</span><span class="aa-profit-value aa-profit-neg">-$' +
      profit.platformFee.toFixed(2) +
      '</span></div>' +
      '<div class="aa-profit-item"><span class="aa-profit-label">Ad Cost (avg)</span><span class="aa-profit-value aa-profit-neg">-$' +
      profit.adCost.toFixed(2) +
      '</span></div>' +
      '<div class="aa-profit-item"><span class="aa-profit-label">Refund Buffer (3%)</span><span class="aa-profit-value aa-profit-neg">-$' +
      profit.refundBuffer.toFixed(2) +
      '</span></div>' +
      '<div class="aa-profit-item aa-profit-total"><span class="aa-profit-label">Net Profit Per Sale</span><span class="aa-profit-value" style="color:var(--accent-green)">$' +
      profit.netProfit.toFixed(2) +
      '</span></div>' +
      '</div>' +
      '</div>' +
      ...
      '<div class="aa-stat-card"><div class="aa-stat-value" style="font-size:18px;color:var(--accent-cyan)">' +
      profit.profitMargin +
      '%</div><div class="aa-stat-label">Profit Margin</div></div>' +
      '<div class="aa-stat-card"><div class="aa-stat-value" style="font-size:18px;color:var(--accent-orange)">' +
      (profit.breakEven > 0 && profit.breakEven < 1000 ? profit.breakEven : '\u2014') +
      '</div><div class="aa-stat-label">Break-Even Units</div></div>' +
      '<div class="aa-stat-card"><div class="aa-stat-value" style="font-size:18px;color:var(--accent-purple)">$' +
      (profit.netProfit * 100).toFixed(0) +
      '</div><div class="aa-stat-label">Profit / 100 Sales</div></div>' +
      '</div>';
    if (match.platformPrices) {
      ...
      for (var i = 0; i < platforms.length; i++) {
        ...
        html +=
          '<div class="aa-profit-item"><span class="aa-profit-label">' +
          esc(cap2(platforms[i])) +
          '</span>' +
          '<span class="aa-profit-value" style="color:' +
          (isCurrent ? 'var(--accent-cyan)' : 'var(--text-primary)') +
          '">$' +
          price.toFixed(2) +
          (isCurrent ? ' \u2190 you' : '') +
          '</span></div>';
    ...
    return (
      ...
      match.seasonality
        .map(function (val) {
          ...
          var bg =
            intensity > 0.7 ? 'rgba(0,255,136,0.35)' : intensity > 0.4 ? 'rgba(0,229,255,0.2)' : 'rgba(85,85,112,0.15)';
      ...
      months
        .map(function (m) {
          return '<div class="aa-heatmap-label">' + m + '</div>';
        })
        .join('') +
      '</div>' +
      '</div>' +
      ...
      '<div class="aa-stat-card"><div class="aa-stat-value" style="font-size:18px;color:var(--accent-cyan)">' +
      fmtN(match.salesVelocity) +
      '</div><div class="aa-stat-label">Monthly Sales</div></div>' +
      '<div class="aa-stat-card"><div class="aa-stat-value" style="font-size:18px;color:var(--accent-green)">' +
      match.demand +
      '/100</div><div class="aa-stat-label">Demand Score</div></div>' +
      '<div class="aa-stat-card"><div class="aa-stat-value" style="font-size:18px;color:var(--accent-orange)">' +
      match.marketSaturation +
      '%</div><div class="aa-stat-label">Saturation</div></div>' +
      '<div class="aa-stat-card"><div class="aa-stat-value" style="font-size:18px;color:var(--accent-purple)">' +
      fmtN(match.orders) +
      '</div><div class="aa-stat-label">Total Orders</div></div>' +
      '</div>' +
      '</div>'
    );
  }
  ...
  function renderAudiencePanel(match) {
    ...
    return (
      ...
      '<div class="aa-grid-4">' +
      '<div class="aa-stat-card"><div class="aa-stat-value" style="font-size:15px">' +
      esc(match.audience.age) +
      '</div><div class="aa-stat-label">Age Range</div></div>' +
      '<div class="aa-stat-card"><div class="aa-stat-value" style="font-size:15px">' +
      esc(match.audience.gender) +
      '</div><div class="aa-stat-label">Gender</div></div>' +
      '<div class="aa-stat-card"><div class="aa-stat-value" style="font-size:12px">' +
      esc(match.audience.countries.slice(0, 3).join(', ')) +
      '</div><div class="aa-stat-label">Top Countries</div></div>' +
      '<div class="aa-stat-card"><div class="aa-stat-value" style="font-size:15px">$' +
      match.cpaAvg.toFixed(2) +
      '</div><div class="aa-stat-label">Est. CPA</div></div>' +
      '</div>' +
      '<div class="aa-card" style="margin-top:14px">' +
      ...
      match.audience.interests
        .map(function (i) {
          return '<span class="aa-tag">' + esc(i) + '</span>';
        })
        .join('') +
      '</div>' +
      '</div>' +
  ...
  function renderCompetitionPanel(competitors, webComps) {
    ...
    for (var i = 0; i < competitors.length; i++) {
      ...
      html +=
        ...
        '<div class="aa-comp-rank">#' +
        (i + 1) +
        '</div>' +
        '<div class="aa-comp-thumb">\ud83c\udfea</div>' +
        '<div class="aa-comp-info"><div class="aa-comp-name">' +
        esc(c.name) +
        '</div>' +
        '<div class="aa-comp-detail">' +
        c.rating +
        '\u2605 \u00b7 ' +
        fmtN2(c.sales) +
        ' sales</div></div>' +
        '<div class="aa-comp-price">$' +
        c.price.toFixed(2) +
        '</div>' +
        '<div class="aa-comp-bar-wrap"><div class="aa-comp-bar-bg"><div class="aa-comp-bar-fill" style="width:' +
        c.saturation +
        '%;background:' +
        (c.saturation > 60 ? 'var(--accent-red)' : c.saturation > 35 ? 'var(--accent-yellow)' : 'var(--accent-green)') +
        '"></div></div>' +
        '<div class="aa-comp-bar-label">' +
        c.saturation +
        '%</div></div></div>';
    }
    html += '</div></div>';
    ...
    if (webComps && webComps.length > 0) {
      ...
      for (var j = 0; j < webComps.length; j++) {
        var wc = webComps[j];
        html +=
          '<div style="padding:10px 14px;background:var(--bg-secondary);border-radius:var(--radius-sm);display:flex;align-items:center;gap:12px">' +
          '<div style="font-size:11px;color:var(--text-muted);min-width:24px">#' +
          wc.rank +
          '</div>' +
  ...
  function renderSWOTPanel(competitors) {
    ...
    return (
      ...
      '<li>' +
      (match.score >= 70 ? 'High AI score validates quality' : 'Moderate market fit') +
      '</li>' +
      '<li>' +
      (profit.netProfit > 10 ? 'Strong profit margins' : 'Competitive pricing possible') +
      '</li>' +
      '<li>' +
      (match.demand >= 70 ? 'Proven consumer demand' : 'Growing niche interest') +
      '</li>' +
      '</ul></div>' +
      '<div class="aa-swot-card swot-w"><div class="aa-swot-title" style="color:var(--accent-red)">Weaknesses</div><ul class="aa-swot-list">' +
      '<li>' +
      (match.competition === 'high' ? 'Saturated competitive space' : 'Need brand differentiation') +
      '</li>' +
      '<li>' +
      (profit.adCost > 8 ? 'High customer acquisition cost' : 'Ad creative testing required') +
      '</li>' +
      '<li>' +
      (match.riskScore > 50 ? 'Trend-dependent demand' : 'Standard product lifecycle') +
      '</li>' +
      '</ul></div>' +
      ...
      '<li>' +
      (match.marketSaturation < 40 ? 'Low saturation = early mover advantage' : 'Bundle/upsell potential') +
      '</li>' +
      '<li>' +
      (match.audience.countries.length > 2 ? 'Multi-market expansion' : 'Untapped geographic markets') +
      '</li>' +
      ...
      '<li>' +
      (match.competition === 'high'
        ? 'Established competitors with reviews'
        : 'New entrants may increase competition') +
      '</li>' +
      '<li>' +
      (match.riskScore > 40 ? 'Demand could shift with trends' : 'Price war risk from low-cost sellers') +
      '</li>' +
  ...
  function renderSuppliersPanel(match) {
    ...
    for (var i = 0; i < match.suppliers.length; i++) {
      ...
      html +=
        ...
        '<div class="aa-supplier-avatar">' +
        esc(s.name.charAt(0)) +
        '</div>' +
        '<div class="aa-supplier-info"><div class="aa-supplier-name">' +
        esc(s.name) +
        '</div>' +
        '<div class="aa-supplier-loc">' +
        esc(s.location) +
        '</div></div>' +
        '<div class="aa-supplier-stats">' +
        '<span class="aa-supplier-stat"><span style="color:var(--accent-yellow)">' +
        s.rating +
        '\u2605</span></span>' +
        '<span class="aa-supplier-stat">' +
        fmtN2(s.orders) +
        ' orders</span>' +
        '<span class="aa-supplier-stat">' +
        esc(s.responseTime) +
  ...
  function renderAdsPanel(match, profit) {
    ...
    for (var i = 0; i < hooks.length; i++) {
      html +=
        '<div class="aa-ad-hook"><div class="aa-ad-hook-type">' +
        esc(hooks[i].type) +
        '</div><div>"' +
        esc(hooks[i].text) +
        '"</div></div>';
    ...
    html +=
      ...
      '</div>' +
      '<div class="aa-ad-platforms">' +
      '<div class="aa-ad-platform-card"><div class="aa-ad-platform-name" style="color:var(--accent-cyan)">Facebook</div><div class="aa-ad-platform-budget">40% \u00b7 $' +
      (profit.adCost * 0.4 * 30).toFixed(0) +
      '/mo</div><div class="aa-ad-platform-type">Carousel + Video</div></div>' +
      '<div class="aa-ad-platform-card"><div class="aa-ad-platform-name" style="color:var(--accent-purple)">TikTok</div><div class="aa-ad-platform-budget">35% \u00b7 $' +
      (profit.adCost * 0.35 * 30).toFixed(0) +
      '/mo</div><div class="aa-ad-platform-type">In-Feed + Spark</div></div>' +
      '<div class="aa-ad-platform-card"><div class="aa-ad-platform-name" style="color:var(--accent-orange)">Instagram</div><div class="aa-ad-platform-budget">25% \u00b7 $' +
      (profit.adCost * 0.25 * 30).toFixed(0) +
      '/mo</div><div class="aa-ad-platform-type">Reels + Stories</div></div>' +
      '</div></div>' +
      '<div class="aa-card" style="margin-top:14px"><div class="aa-card-header"><span class="aa-card-icon">\u270d\ufe0f</span>Sample Ad Copy</div>' +
      '<div class="aa-ad-copy-block"><div class="aa-ad-copy-label">Facebook Carousel</div><div class="aa-ad-copy-text">Tired of ' +
      esc(match.category) +
      " that don't deliver? \ud83d\udd25 Our " +
      esc(title0) +
      ' is rated ' +
      match.rating +
      '\u2605 by ' +
      fmtN(match.reviews) +
      ' happy customers. Shop now and see the difference.</div></div>' +
      '<div class="aa-ad-copy-block"><div class="aa-ad-copy-label">TikTok In-Feed (15s)</div><div class="aa-ad-copy-text">POV: You finally found the perfect ' +
      esc(match.category) +
      ' \u2728 ' +
      fmtN(match.orders) +
      ' people already ordered theirs. Link in bio before it sells out \ud83d\udc40</div></div>' +
      '<div class="aa-ad-copy-block"><div class="aa-ad-copy-label">Instagram Reel (30s)</div><div class="aa-ad-copy-text">This ' +
      esc(match.category) +
      ' changed everything for me. ' +
      match.rating +
      '\u2605 rating, ' +
      fmtN(match.orders) +
      " orders, and it's still flying under the radar. Grab yours before everyone else catches on \ud83d\ude80</div></div>" +
      '</div></div></div>';
    return html;
  }
  ...
  function renderKeywordsPanel(keywordData, webKeywords, match) {
    ...
    for (var i = 0; i < primary.length; i++) {
      var kw = primary[i];
      html +=
        '<div class="aa-kw-row"><span class="aa-kw-word">' +
        esc(kw.word) +
        '</span>' +
        '<span class="aa-kw-vol">' +
        fmtN(kw.volume) +
        '/mo</span>' +
        '<div class="aa-kw-comp"><div class="aa-kw-comp-bar"><div class="aa-kw-comp-fill" style="width:' +
        kw.competition +
        '%;background:' +
        (kw.competition > 60
          ? 'var(--accent-red)'
    ...
    html += '</div></div>';
    html +=
      '<div class="aa-card" style="margin-top:14px"><div class="aa-card-header"><span class="aa-card-icon">\ud83c\udff7</span>All Tags</div>' +
      '<div class="aa-tags">' +
      match.keywords
        .map(function (k) {
          return '<span class="aa-tag">' + esc(k) + '</span>';
        })
        .join('') +
      '</div></div>';
    html +=
      '<div class="aa-card" style="margin-top:14px"><div class="aa-card-header"><span class="aa-card-icon">\ud83d\udcdd</span>Suggested Listing Title</div>' +
      '<div class="aa-kw-listing"><div class="aa-kw-listing-label">Optimized Title</div>' +
      '<div class="aa-kw-listing-text">' +
      esc(match.title.split('\u2014')[0].trim()) +
      ' - ' +
      keywordData
        .slice(0, 3)
      ...
      ' | ' +
      match.rating +
      '\u2605 Rated | Free Shipping</div></div></div>';
    if (webKeywords && webKeywords.length > 0) {
      html +=
        '<div class="aa-card" style="margin-top:14px"><div class="aa-card-header"><span class="aa-card-icon">\ud83c\udf10</span>Web Research Insights</div>' +
        '<div style="display:flex;flex-direction:column;gap:8px">';
      for (var k = 0; k < webKeywords.length; k++) {
        var wk = webKeywords[k];
        html +=
          '<div style="padding:10px 14px;background:var(--bg-secondary);border-radius:var(--radius-sm)">' +
  ...
  function renderRelatedProducts(relatedProducts) {
    ...
    for (var i = 0; i < relatedProducts.length; i++) {
      ...
      html +=
        '<div class="aa-related-card" data-related-query="' +
        esc(rp.title.split('\u2014')[0].trim()) +
        '">' +
        '<img class="aa-related-img" src="' +
        esc(rp.image) +
        '" alt="' +
        esc(rp.title) +
        '" onerror="this.style.display=\'none\'">' +
        ...
        esc(rp.title.split('\u2014')[0].trim()) +
        '</div>' +
        '<div class="aa-related-meta"><span class="aa-related-score">' +
        rp.score +
        '/100</span>' +
        '<span class="aa-related-price">$' +
        rp.price.toFixed(2) +
        '</span></div></div>';
    }
    return html + '</div></div>';
  ...
  const AIAnalystPlugin = {
    ...
    mount(_ctx) {
      ...
      if (btn)
        btn.addEventListener('click', function () {
          AIAnalystPlugin.runAnalysis(input ? input.value : '');
        });
      if (input)
        input.addEventListener('keypress', function (e) {
          if (e.key === 'Enter') AIAnalystPlugin.runAnalysis(input.value);
        });
      ...
      _cleanups.push(
        EventBus.on('ai-analyst:run', function (data) {
    ...
    async runAnalysis(query) {
      ...
      resultsEl.innerHTML = renderLoading(
        ...
        'Matching products across all platforms'
      );
      ...
      resultsEl.innerHTML = renderLoading(
        'Analyzing ' + esc(match.title.split('\u2014')[0].trim()) + '...',
        'Running AI risk analysis & web research'
      );
      await new Promise(function (r) {
        setTimeout(r, 300);
      });
      ...
      try {
        ...
        console.warn('[AIAnalyst] Risk analysis failed:', e);
      }

      resultsEl.innerHTML = renderLoading(
        'Deep-scanning market data...',
        'Fetching competitor intelligence & keyword data'
      );
      ...
      try {
        ...
        console.warn('[AIAnalyst] Web search failed:', e);
      }
      ...
      resultsEl.innerHTML =
        renderAPIKeyBanner() +
        renderResults(match, profit, riskAnalysis, competitors, keywordData, webComps, webKeywords, aiInsight);
