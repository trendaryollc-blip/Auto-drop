...
const COMMANDS = {
  research: {
    section: 'Research',
    items: [
      { cmd: '/search', icon: '🔍', name: 'Web Search', desc: 'Search the web for anything', prompt: 'Search the web for: ' },
      { cmd: '/product', icon: '📦', name: 'Product Research', desc: 'Deep research on a product', prompt: 'Research this product from the web: ' },
      { cmd: '/trend', icon: '📈', name: 'Trend Analysis', desc: 'Check if a product is trending', prompt: 'Analyze trends for: ' },
      { cmd: '/niche', icon: '⭐', name: 'Niche Discovery', desc: 'Find untapped niches', prompt: 'Find me untapped niches in: ' }
    ]
  },
  intelligence: {
    section: 'Intelligence',
    items: [
      { cmd: '/competitor', icon: '⚔️', name: 'Competitor Spy', desc: 'Analyze competitor stores', prompt: 'Analyze competitors for: ' },
      { cmd: '/price', icon: '💰', name: 'Price Intel', desc: 'Check live prices across platforms', prompt: 'Check prices across platforms for: ' },
      { cmd: '/market', icon: '🌐', name: 'Market Analysis', desc: 'Market size and opportunity', prompt: 'Analyze the market for: ' },
      { cmd: '/supplier', icon: '🏭', name: 'Supplier Check', desc: 'Verify supplier reliability', prompt: 'Verify this supplier: ' }
    ]
  },
  strategy: {
    section: 'Strategy',
    items: [
      { cmd: '/analyze', icon: '🧠', name: 'Full Analysis', desc: 'Complete product analysis with AI', prompt: 'Give me a full analysis of: ' },
      { cmd: '/ad', icon: '📢', name: 'Ad Strategy', desc: 'Create ad campaigns and copy', prompt: 'Create an ad strategy for: ' },
      { cmd: '/seo', icon: '🔎', name: 'SEO Keywords', desc: 'Keyword research and suggestions', prompt: 'Find SEO keywords for: ' },
      { cmd: '/social', icon: '📱', name: 'Social Intel', desc: 'Social media trends and hashtags', prompt: 'Analyze social media trends for: ' }
    ]
  },
  tools: {
    section: 'Tools',
    items: [
      { cmd: '/compare', icon: '⚖️', name: 'Compare', desc: 'Compare products side by side', prompt: 'Compare these products: ' },
      { cmd: '/forecast', icon: '📊', name: 'Forecast', desc: 'Sales and profit forecasting', prompt: 'Forecast sales for: ' },
      { cmd: '/risk', icon: '⚠️', name: 'Risk Assessment', desc: 'Evaluate business risks', prompt: 'Assess the risks of: ' },
      { cmd: '/export', icon: '📋', name: 'Export Chat', desc: 'Download conversation as file', prompt: '__EXPORT__' }
    ]
  }
};

const CAPABILITIES = [
  { icon: '🔍', title: 'Web Search', desc: 'Search the live web for products, prices, trends, and market data', color: 'cyan', prompt: 'Search the web for trending dropshipping products in 2025' },
  { icon: '📦', title: 'Product Research', desc: 'Deep-dive research any product with live pricing and demand data', color: 'green', prompt: 'Research wireless earbuds — pricing, competition, and trends' },
  { icon: '⚔️', title: 'Competitor Intel', desc: 'Spy on competitor stores, pricing strategies, and ad campaigns', color: 'purple', prompt: 'Analyze my top competitors in the pet niche' },
  { icon: '📈', title: 'Trend Detection', desc: 'Real-time trend scanning from Google, TikTok, and social media', color: 'orange', prompt: 'What products are trending on TikTok right now?' },
  { icon: '💰', title: 'Price Intelligence', desc: 'Compare live prices across 10+ platforms instantly', color: 'yellow', prompt: 'Compare prices for LED galaxy projectors across all platforms' },
  { icon: '🧠', title: 'Market Analysis', desc: 'Market size, saturation levels, and opportunity scoring', color: 'red', prompt: 'Analyze the home fitness equipment market opportunity' },
  { icon: '📢', title: 'Ad Strategy', desc: 'Create high-converting ad copy and campaign strategies', color: 'cyan', prompt: 'Create a TikTok ad campaign for posture correctors' },
  { icon: '🏭', title: 'Supplier Verify', desc: 'Background-check suppliers using web intelligence', color: 'green', prompt: 'Verify TechGear Direct supplier reliability' },
  { icon: '🔎', title: 'SEO & Keywords', desc: 'Keyword research, search volume, and content suggestions', color: 'purple', prompt: 'Find SEO keywords for my pet water fountain product' }
];
...
const INTEL_FEED = [
  { badge: 'trend', title: 'TikTok Viral: LED Sunset Lamps', desc: 'Searches up 340% this week. Low competition, high margin potential.', time: '2m', action: 'Research this' },
  { badge: 'opportunity', title: 'Market Gap: Eco-Friendly Pet Products', desc: 'Only 12 suppliers on AliExpress. Demand growing 25% MoM.', time: '15m', action: 'Explore niche' },
  { badge: 'alert', title: 'Price Drop: Bluetooth Speakers on DHgate', desc: 'Average cost down 18%. Good time to stock inventory.', time: '32m', action: 'Check prices' },
  { badge: 'info', title: 'New Supplier: Verified 5-star in Shenzhen', desc: 'Electronics specialist, 200K+ orders, <1h response time.', time: '1h', action: 'View supplier' },
  { badge: 'trend', title: 'Google Trends: "posture corrector" spiking', desc: 'Search volume doubled in 7 days. WFH season peak.', time: '2h', action: 'Analyze trend' }
];
...
const AICoachPlugin = {
  ...
  buildHTML() {
    ...
    return '<div class="coach-grid-overlay"></div>' +
      '<div class="coach-ticker">' +
        '<div class="coach-ticker-status ' + (connected ? 'live' : 'paper') + '">' +
          '<span class="coach-status-dot"></span>' +
          (connected ? 'LIVE' : 'PAPER') + ' · ' + products.length + ' tracked' +
        '</div>' +
        '<div class="coach-ticker-track">' + ticker + ticker + '</div>' +
      '</div>' +
  ...
  buildTickerHTML(products) {
    ...
    for (var i = 0; i < products.length; i++) {
      ...
      html += '<span class="coach-ticker-item">' +
        ...
        '<span class="coach-ticker-chg ' + (up ? 'up' : 'down') + '">' + chgStr + '</span>' +
      '</span>';
    }
    return html;
  },
  ...
  buildSidebarHTML() {
    return '<aside class="coach-sidebar" id="coachSidebar">' +
  ...
  bindEvents() {
    ...
    if (el.input) {
      el.input.addEventListener('keydown', function(e) {
        if (e.key === 'Enter' && !e.shiftKey) { e.preventDefault(); AICoachPlugin.sendMessage(); }
      });
    ...
    if (el.toggleSidebar) el.toggleSidebar.addEventListener('click', function() { AICoachPlugin.toggleSidebar(); });

    if (_state.section) {
      _state.section.querySelectorAll('.coach-sb-tab').forEach(function(tab) {
        tab.addEventListener('click', function() { AICoachPlugin.switchTab(tab.dataset.tab); });
      });
    }

    if (el.exportBtn) el.exportBtn.addEventListener('click', function() { AICoachPlugin.exportChat(); });
    if (el.clearBtn) el.clearBtn.addEventListener('click', function() { AICoachPlugin.clearChat(); });
    if (el.settingsBtn) el.settingsBtn.addEventListener('click', function() { window.HuntDrop.navigateTo('section-ai-settings'); });
    if (el.voiceBtn) el.voiceBtn.addEventListener('click', function() { AICoachPlugin.toggleVoiceInput(); });
  },
    ...
    INTEL_FEED.forEach(function(intel) {
      html += '<div class="coach-intel-card">' +
        '<div class="coach-intel-header">' +
          '<span class="coach-intel-badge ' + intel.badge + '">' + intel.badge + '</span>' +
          '<span class="coach-intel-time">' + intel.time + ' ago</span>' +
        '</div>' +
        '<div class="coach-intel-title">' + intel.title + '</div>' +
        '<div class="coach-intel-desc">' + intel.desc + '</div>' +
        '<div class="coach-intel-actions">' +
          '<button class="coach-intel-action" data-query="' + intel.title + '">' + intel.action + '</button>' +
        '</div>' +
      '</div>';
    });
    el.innerHTML = html;
    ...
    products.forEach(function(p, i) {
      ...
      html += '<div class="coach-trending-item" data-prompt="Analyze ' + p.title + ' — is it worth selling?">' +
        ...
        '<div class="coach-trending-info">' +
          '<div class="coach-trending-name">' + p.title + '</div>' +
          '<div class="coach-trending-meta"><span>' + p.platform + '</span><span>$' + p.price.toFixed(2) + '</span><span>' + p.margin + '% margin</span></div>' +
        '</div>' +
        '<div class="coach-trending-change up">' + change + '</div>' +
      '</div>';
    });
    el.innerHTML = html;
  ...
  renderStatusBar() {
    ...
    el.innerHTML =
      ...
      '<div class="coach-status-row"><span class="coach-status-label">Health</span><span class="coach-status-value" style="color:' + (health.score >= 80 ? 'var(--accent-green)' : 'var(--accent-yellow)') + '">' + health.score + '/100</span></div>' +
      '<div class="coach-status-row"><span class="coach-status-label">Messages</span><span class="coach-status-value">' + history.length + '</span></div>';
  },
  ...
  updateContextBar() {
    ...
    el.innerHTML =
      ...
      '<div class="coach-ctx-pill"><div class="coach-ctx-ring" style="--progress:' + Math.min(100, health.score) + '%;--ring-color:' + hc + '"><span class="coach-ctx-val">' + health.score + '</span></div><span class="coach-ctx-label">HEALTH</span></div>' +
      '<div class="coach-ctx-pill"><div class="coach-ctx-ring" style="--progress:' + Math.min(100, _state.msgCount * 5) + '%;--ring-color:#00ff88"><span class="coach-ctx-val">' + _state.msgCount + '</span></div><span class="coach-ctx-label">MSGS</span></div>';
  },
  ...
  async sendMessage() {
    ...
    if (text === '/clear') { AICoachPlugin.clearChat(); input.value = ''; return; }
  ...
  buildFallbackWithSearch(query, searchResult) {
    ...
    if (matchedProduct) {
      ...
      response += '- **Score:** ' + matchedProduct.score + '/100\n- **Price:** $' + matchedProduct.price.toFixed(2) + ' (margin: ' + matchedProduct.margin + '%)\n';
      response += '- **Competition:** ' + matchedProduct.competition + '\n- **Risk Score:** ' + matchedProduct.riskScore + '/100\n';
      response += '- **Sales Velocity:** ' + matchedProduct.salesVelocity + '/mo\n\n';
      response += '**Verdict:** ' + (matchedProduct.score >= 80 ? '✅ PROCEED' : matchedProduct.score >= 60 ? '⚠️ CAUTION' : '❌ RECONSIDER') + '\n\n';
    }

    if (!searchResult && !matchedProduct) response = AICoachPlugin.getSmartFallback(query);
    if (searchResult && searchResult.fallback) response += '\n💡 Add a web search API key in AI Settings for live results. Supports Tavily, Serper, and Brave Search.';
