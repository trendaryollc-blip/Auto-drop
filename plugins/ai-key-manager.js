...
if (!_km || !_km.PluginRegistry) { console.error('[AIKeyManager] HuntDrop core not loaded'); return; }
...
const PROVIDERS = {
  ...
  huggingface: {
    ...
    models: ['mistralai/Mistral-7B-Instruct-v0.3','HuggingFaceH4/zephyr-7b-beta','microsoft/Phi-3-mini-4k-instruct'],
    keyPrefix: 'hf_',
  ...
  replicate: {
    ...
    models: ['meta/meta-llama-3-70b-instruct','mistralai/mixtral-8x7b-instruct-v0.1','google-deepmind/gemma-2-27b-it'],
    keyPrefix: '',
...
function openKeyDB() {
  return new Promise(function(resolve, reject) {
    ...
    let req;
    try { req = indexedDB.open(MASTER_KEY_DB_NAME, 1); }
    catch(e) { reject(e); return; }
    req.onupgradeneeded = function(e) {
      try { e.target.result.createObjectStore(MASTER_KEY_DB_STORE); } catch {/* ignored */}
    };
...
async function storeKeyInDB(keyBytes) {
  try {
    ...
    return new Promise(function(resolve, reject) {
      ...
      tx.onerror = function() { reject(new Error('IndexedDB store failed')); };
    });
  } catch { return false; }
}
...
async function loadKeyFromDB() {
  try {
    ...
    return new Promise(function(resolve, reject) {
      ...
      req.onerror = function() { reject(new Error('IndexedDB load failed')); };
    });
  } catch { return null; }
}
...
async function ensureReady() {
  ...
  if (_initPromise) { await _initPromise; return _masterKey; }
...
async function _decryptLegacyV2(str) {
  try {
    ...
    const passphrase = (window.HuntDrop && window.HuntDrop._legacyKeyConfig && window.HuntDrop._legacyKeyConfig.passphrase) || '';
    const salt = (window.HuntDrop && window.HuntDrop._legacyKeyConfig && window.HuntDrop._legacyKeyConfig.salt) || 'HuntDropAI_DefaultSalt';
...
function _decryptLegacyV1(b64Str) {
  try {
    ...
    const xorKey = (window.HuntDrop && window.HuntDrop._legacyKeyConfig && window.HuntDrop._legacyKeyConfig.xorKey) || 0;
...
const APIKeyManager = {
  ...
  init: function(_ctx) {
    ...
    if (!window.crypto || !window.crypto.subtle) {
      console.error('[AIKeyManager] CRITICAL: Web Crypto API (crypto.subtle) is not available. ' +
    ...
    console.warn('[AIKeyManager] SECURITY NOTICE: API keys are stored client-side with AES-GCM encryption. ' +
    ...
    _initPromise = ensureReady().then(async function() {
  ...
  saveKey: async function(provider, key) {
    ...
    await ensureReady();
    var encrypted = await encrypt(key);
  ...
  getKey: async function(provider) {
    ...
    await ensureReady();
    var plaintext = await decrypt(encrypted);
    if (!plaintext) return null;
  ...
  verifyKey: async function(provider, key) {
    ...
    try {
      ...
      if (provider === 'google') {
        ...
        if (!proxyUrl) {
          console.warn('[AIKeyManager] Google API key verification exposes key in URL. Configure window.HuntDrop._apiProxy for secure verification.');
        }
  ...
  getSecurityNotice: function() {
    return {
      ...
      message: 'API keys are encrypted with AES-GCM but remain accessible via browser DevTools. ' +
      ...
      recommendation: 'For production, deploy a backend proxy that holds API keys server-side ' +
  ...
  FEATURES: {
    ...
    'ai-business-coach': { name: 'AI Business Coach', desc: 'Personalized business mentorship and strategy', icon: '🧠' },
    'cb-intelligence-service': { name: 'Competitor Intelligence', desc: 'Analyze competitors, ads, and market data', icon: '🔍' }
  },
