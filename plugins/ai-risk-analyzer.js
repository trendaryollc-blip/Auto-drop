  ...
  const AIRiskAnalyzer = {
    ...
    analyzeProduct(product) {
      ...
      if (product.suppliers && product.suppliers.length > 0) {
        const verified = product.suppliers.filter(function (s) {
          return s.verified;
        });
      ...
      let winScore =
        product.score * 0.25 +
        product.margin * 0.2 +
        (100 - product.riskScore) * 0.2 +
        product.demand * 0.15 +
        (trend > 0 ? 10 : 0) +
        (product.competition === 'low' ? 10 : product.competition === 'medium' ? 5 : 0);
      ...
      return {
        ...
        recColor:
          winScore >= 70 ? 'var(--accent-green)' : winScore >= 50 ? 'var(--accent-orange)' : 'var(--accent-red)',
        factors: factors,
    ...
    calculateTrend(trendData) {
      ...
      const recent =
        trendData.slice(-3).reduce(function (a, b) {
          return a + b;
        }, 0) / 3;
      const early =
        trendData.slice(0, 3).reduce(function (a, b) {
          return a + b;
        }, 0) / 3;
      if (early === 0) return 0;
    ...
    formatAnalysisForAI(analysis) {
      ...
      output +=
        '  Conservative: ' +
        analysis.monthlyProjection.conservative.units +
        ' units = $' +
        analysis.monthlyProjection.conservative.profit +
        '\n';
      output +=
        '  Moderate: ' +
        analysis.monthlyProjection.moderate.units +
        ' units = $' +
        analysis.monthlyProjection.moderate.profit +
        '\n';
      output +=
        '  Aggressive: ' +
        analysis.monthlyProjection.aggressive.units +
        ' units = $' +
        analysis.monthlyProjection.aggressive.profit +
        '\n\n';
      output += 'POSITIVE FACTORS:\n';
      analysis.factors.positive.forEach(function (f) {
        output += '  + ' + f + '\n';
      });
      output += '\nNEGATIVE FACTORS:\n';
      analysis.factors.negative.forEach(function (f) {
        output += '  - ' + f + '\n';
      });
      if (analysis.factors.neutral.length > 0) {
        output += '\nNEUTRAL FACTORS:\n';
        analysis.factors.neutral.forEach(function (f) {
          output += '  ~ ' + f + '\n';
        });
      }
      return output;
