  ...
  const AISystemHealth = {
    ...
    checkAdapters() {
      ...
      return {
        ...
        pass: adapterCount >= 10,
        issues: issues,
        severity: adapterCount < 10 ? 'medium' : 'none',
      };
    },
    ...
    checkProfitData() {
      ...
      return {
        id: 'profit-data',
        name: 'Profit Data Integrity',
        pass: issues.length === 0,
        issues: issues.concat(warnings),
        severity: issues.length > 0 ? 'high' : warnings.length > 0 ? 'low' : 'none',
      };
    },
    ...
    getHealthSummary() {
      ...
      const highSeverity = failed.filter(function (r) {
    ...
    getProactiveWarnings() {
      ...
      if (health.highSeverity > 0) {
        warnings.push({
          ...
          issues: health.results
            .filter(function (r) {
              return r.severity === 'high' && !r.pass;
            })
            .map(function (r) {
      ...
      const products = window.HuntDrop.ALL_PRODUCTS || [];
      const lowMargin = products.filter(function (p) {
        return p.margin < 15;
      });
      if (lowMargin.length > 0) {
        warnings.push({
          ...
          products: lowMargin.map(function (p) {
            return p.title + ' (' + p.margin + '%)';
          }),
        });
      }
      return warnings;
