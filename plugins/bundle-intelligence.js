...
const BundleIntelligencePlugin = {
  ...
  init(ctx){Config.defaults('bundleIntelligence',{enabled:true});},
  ...
  mount(ctx){
    ...
    const self=BundleIntelligencePlugin;
    self._section=section;
    const btn=section.querySelector('#biGenerateBtn');
    const input=section.querySelector('#biInput');
    if(btn) btn.addEventListener('click',()=>self.analyze(input?.value||''));
    if(input) input.addEventListener('keypress',e=>{if(e.key==='Enter')self.analyze(input.value);});
    section.querySelectorAll('.bi-quick-btn').forEach(b=>{
      b.addEventListener('click',()=>{input.value=b.dataset.q;self.analyze(b.dataset.q);});
    });
    setTimeout(()=>self.analyze(''),500);
  },

  unmount(ctx){if(BundleIntelligencePlugin._section){BundleIntelligencePlugin._section.remove();BundleIntelligencePlugin._section=null;}},
  ...
  analyze(query){
    ...
    const el=this._section?this._section.querySelector('#biResults'):null;
    if(!el) return;
    ...
    el.innerHTML=`
      ...
      <div class="bi-base-product">
        ...
        <div class="bi-base-card">
          <div class="bi-base-img"><img src="${product.image}" alt=""></div>
          <div class="bi-base-info">
            <div class="bi-base-name">${product.title.split('—')[0].trim()}</div>
            <div class="bi-base-meta">${product.category} • Score: ${product.score}/100</div>
            <div class="bi-base-price">$${product.platformPrices.amazon.toFixed(2)}</div>
          </div>
          <div class="bi-base-stats">
            <div class="bi-base-stat"><div class="bi-base-stat-val">${product.orders.toLocaleString()}</div><div class="bi-base-stat-label">Orders</div></div>
            <div class="bi-base-stat"><div class="bi-base-stat-val">${product.rating}★</div><div class="bi-base-stat-label">Rating</div></div>
            <div class="bi-base-stat"><div class="bi-base-stat-val">${product.margin}%</div><div class="bi-base-stat-label">Margin</div></div>
          </div>
        </div>
      </div>
      ...
      <div class="bi-section">
        ...
        <div class="bi-insights-grid">
          <div class="bi-insight-card">
            ...
            <div class="bi-insight-text">73% of customers who buy <strong>${product.title.split('—')[0].trim()}</strong> also purchase complementary products within 48 hours.</div>
          </div>
          <div class="bi-insight-card">
    ...
    el.querySelectorAll('.bi-tab-btn').forEach(tab=>{
      tab.addEventListener('click',()=>{
        ...
        const content=self._section.querySelector('#biTabContent');
        if(!content) return;
        switch(tab.dataset.tab){
          case 'bundles': content.innerHTML=self.renderBundles(bundles); break;
          case 'copy': content.innerHTML=self.renderCopy(bundles); break;
          case 'abtest': content.innerHTML=self.renderABTests(); break;
          case 'patterns': content.innerHTML=self.renderPatterns(product,bundles); break;
        }
      });
    });
  },
        ...
        ${bundles.map((b,i)=>{
          ...
          return `<div class="bi-bundle-card">
            <div class="bi-bundle-header">
              ...
              <span class="bi-bundle-badge" style="background:${b.badgeColor}22;color:${b.badgeColor}">${b.badge}</span>
              <span class="bi-bundle-aov">+${b.aovIncrease}% AOV</span>
            </div>
            <div class="bi-bundle-products">
              ${b.products.map((p,j)=>`
                ${j>0?'<div class="bi-bundle-plus">+</div>':''}
                <div class="bi-bundle-product">
                  <div class="bi-bundle-product-img"><img src="${p.image}" alt=""></div>
                  <div class="bi-bundle-product-info">
                    <div class="bi-bundle-product-name">${p.title.split('—')[0].trim()}</div>
                    <div class="bi-bundle-product-price">$${p.platformPrices.amazon.toFixed(2)}</div>
                  </div>
                </div>
              `).join('')}
            </div>
            <div class="bi-bundle-stats">
              <div class="bi-bundle-stat"><div class="bi-bundle-stat-value" style="color:var(--accent-green)">${b.coPurchaseRate}%</div><div class="bi-bundle-stat-label">Co-Purchase</div></div>
              <div class="bi-bundle-stat"><div class="bi-bundle-stat-value" style="color:var(--accent-cyan)">+${b.aovIncrease}%</div><div class="bi-bundle-stat-label">AOV Increase</div></div>
              <div class="bi-bundle-stat"><div class="bi-bundle-stat-value" style="color:var(--accent-orange)">+${b.marginBoost}%</div><div class="bi-bundle-stat-label">Margin Boost</div></div>
            </div>
            ...
            <div class="bi-bundle-copy">
              ...
              <div class="bi-bundle-copy-text">${b.copy}</div>
            </div>
            <div class="bi-bundle-pricing">
              <div class="bi-bundle-pricing-row">
                <span class="bi-bundle-individual">${b.products.map(p=>'$'+p.platformPrices.amazon.toFixed(2)).join(' + ')}</span>
              </div>
              <div class="bi-bundle-pricing-row">
                <span class="bi-bundle-arrow-down">↓</span>
              </div>
              <div class="bi-bundle-pricing-row bi-bundle-pricing-final">
                <span class="bi-bundle-final">$${bundlePrice.toFixed(2)}</span>
                <span class="bi-bundle-savings">Save $${savings.toFixed(2)}</span>
              </div>
            </div>
  ...
  renderPatterns(product,bundles){
