...
(function () {
  ...
  function simulate(params) {
    ...
    for (let d = 1; d <= days; d++) {
      ...
      let cumRev = 0,
        cumProfit = 0,
        cumCost = 0;
      results.daily.forEach((dd) => {
        cumRev += dd.revenue;
        cumProfit += dd.profit;
        cumCost += dd.cost + dd.adCost;
      });
      cumRev += revenue;
      cumProfit += profit;
      cumCost += productCost + adCost;
    ...
    Object.entries(scenarios).forEach(([key, mult]) => {
      ...
      for (let d = 0; d < days; d++) {
        ...
        const baseProfit =
          (results.daily[d].netRevenue * (1 + marginAdj) - results.daily[d].cost - results.daily[d].adCost * cpaMult) *
          mult.growthMult;
        cumRev += baseRev;
    ...
    const d30 = results.daily.slice(0, 30),
      d60 = results.daily.slice(0, 60),
      d90 = results.daily;
    results.milestones = {
      day30: {
        revenue: sums(d30, 'revenue'),
        profit: sums(d30, 'profit'),
        sales: sums(d30, 'sales'),
        costs: sums(d30, 'cost') + sums(d30, 'adCost'),
        refunds: sums(d30, 'refunds'),
      },
      day60: {
        revenue: sums(d60, 'revenue'),
        profit: sums(d60, 'profit'),
        sales: sums(d60, 'sales'),
        costs: sums(d60, 'cost') + sums(d60, 'adCost'),
        refunds: sums(d60, 'refunds'),
      },
      day90: {
        revenue: sums(d90, 'revenue'),
        profit: sums(d90, 'profit'),
        sales: sums(d90, 'sales'),
        costs: sums(d90, 'cost') + sums(d90, 'adCost'),
        refunds: sums(d90, 'refunds'),
      },
    };
  ...
  const BusinessSimulatorPlugin = {
    ...
    init(_ctx) {
      Config.defaults('businessSim', { enabled: true });
    },
  ...
  function getInputs() {
    ...
    return {
      ...
      avgCpa: parseFloat(q('bsCPA')?.value) || 4.5,
      ...
      <div class="bs-output">
        ...
        <div class="bs-monthly-card">
          ...
          <div class="bs-monthly-grid">
            ${[
              { label: 'Month 1', data: d30 },
              {
                label: 'Month 2',
                data: {
                  revenue: d60.revenue - d30.revenue,
                  profit: d60.profit - d30.profit,
                  sales: d60.sales - d30.sales,
                },
              },
              {
                label: 'Month 3',
                data: {
                  revenue: d90.revenue - d60.revenue,
                  profit: d90.profit - d60.profit,
                  sales: d90.sales - d60.sales,
                },
              },
            ]
              .map(
    ...
    _chart = new Chart(canvas, {
      ...
      data: {
        ...
        datasets: [
          {
            label: 'Revenue',
            data: results.daily.filter((_, i) => i % 3 === 0).map((d) => d.revenue),
            borderColor: '#00ff88',
          ...
          {
            label: 'Profit',
            data: results.daily.filter((_, i) => i % 3 === 0).map((d) => d.profit),
            borderColor: '#00e5ff',
          ...
          {
            label: 'Costs',
            data: results.daily.filter((_, i) => i % 3 === 0).map((d) => d.cost + d.adCost),
            borderColor: '#ff3366',
      ...
      options: {
        ...
        scales: {
          x: {
            ...
            ticks: { color: '#555570', font: { family: 'JetBrains Mono', size: 9 }, maxTicksLimit: 10 },
          },
          y: {
            grid: { color: 'rgba(255,255,255,0.025)' },
            ticks: {
    ...
    _scenarioChart = new Chart(scCanvas, {
      ...
      data: {
        ...
        datasets: [
          {
            label: 'Best',
            data: results.scenarios.best.filter((_, i) => i % 6 === 0).map((s) => s.cumRevenue),
            borderColor: '#00ff88',
            borderWidth: 2,
            tension: 0.4,
            pointRadius: 0,
          ...
          {
            label: 'Likely',
            data: results.scenarios.likely.filter((_, i) => i % 6 === 0).map((s) => s.cumRevenue),
            borderColor: '#00e5ff',
            borderWidth: 2,
            tension: 0.4,
            pointRadius: 0,
          ...
          {
            label: 'Worst',
            data: results.scenarios.worst.filter((_, i) => i % 6 === 0).map((s) => s.cumRevenue),
            borderColor: '#ff3366',
            borderWidth: 2,
            tension: 0.4,
            pointRadius: 0,
      ...
      options: {
        ...
        plugins: {
          ...
          tooltip: { backgroundColor: '#111119', borderColor: '#2a2a3d', borderWidth: 1 },
        },
        scales: {
          x: {
            grid: { color: 'rgba(255,255,255,0.025)' },
