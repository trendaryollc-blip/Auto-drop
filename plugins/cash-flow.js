...
const PLATFORMS = [
  { id:'shopify', name:'Shopify', icon:'🛍️', payoutDelay:2, payoutFreq:'Daily', fees:2.9, fixedFee:0.30, color:'#96bf48' },
  { id:'amazon', name:'Amazon', icon:'📦', payoutDelay:14, payoutFreq:'Bi-weekly', fees:15, fixedFee:0, color:'#ff9900' },
  { id:'ebay', name:'eBay', icon:'🏷️', payoutDelay:3, payoutFreq:'Daily', fees:13.25, fixedFee:0.30, color:'#e53238' },
  { id:'tiktok', name:'TikTok Shop', icon:'🎵', payoutDelay:7, payoutFreq:'Weekly', fees:5, fixedFee:0, color:'#000000' },
  { id:'etsy', name:'Etsy', icon:'🎨', payoutDelay:3, payoutFreq:'Daily', fees:6.5, fixedFee:0.20, color:'#f1641e' },
  { id:'temu', name:'Temu', icon:'🔥', payoutDelay:14, payoutFreq:'Bi-weekly', fees:0, fixedFee:0, color:'#fb7701' }
];
...
function getStoredCashFlow(){
  try{ const s = JSON.parse(localStorage.getItem('hd_cashflow')); return s && s.cash ? s : getDefaultData(); }catch(e){ return getDefaultData(); }
}
function saveCashFlow(d){ try{ localStorage.setItem('hd_cashflow', JSON.stringify(d)); }catch(e){} }
...
function generateTimeline(data, days){
  ...
  for(let i=0; i<days; i++){
    ...
    const dayLabel = i === 0 ? 'Today' : i === 1 ? 'Tomorrow' : d.toLocaleDateString('en',{month:'short',day:'numeric'});
    let income = 0;
    let expenses = 0;
...
function getStats(timeline){
  ...
  const dailyExpenses = (d.dailyAdSpend + (d.ordersPerDay * d.avgProductCost) + (d.ordersPerDay * d.avgShippingCost) + d.monthlyFixedCosts/30);
  ...
  return { totalInflow, totalOutflow, daysOfRunway, pendingPayouts, payoutCount:d.payouts.length, healthScore, dailyExpenses };
}
...
function renderTimeline(timeline){
  ...
  el.innerHTML = `<div class="cf-tl-chart">
    ...
    <div class="cf-tl-grid">
      ${timeline.slice(0,14).map(t=>{
        const cashPct = Math.max(2, (Math.abs(t.cash) / maxCash) * 100);
        const barColor = t.isNegative ? 'var(--accent-red)' : t.isLow ? 'var(--accent-yellow)' : 'var(--accent-green)';
...
function renderCalcResults(){
  ...
  const dailyTotalCosts = d.dailyAdSpend + dailyProductCost + dailyShipCost + dailyPlatformFees + d.monthlyFixedCosts/30;
...
function renderDelays(){
  ...
  el.innerHTML = `<div class="cf-delay-grid">
    ${PLATFORMS.map(p=>`<div class="cf-delay-card" data-section="section-supplier-hub" role="button" tabindex="0" style="cursor:pointer">
...
function renderReinvest(){
  ...
  const d = _data;
  const dailyRevenue = d.ordersPerDay * d.avgOrderValue;
  const dailyCosts = d.dailyAdSpend + (d.ordersPerDay * d.avgProductCost) + (d.ordersPerDay * d.avgShippingCost) + d.monthlyFixedCosts/30;
...
function renderAlerts(timeline){
  ...
  if(lowDays.length > 0){
    alerts.push({ icon:'🚨', title:'Cash Crunch Predicted', desc:`${lowDays.length} day(s) in the next 2 weeks will have less than $500. Nearest: ${lowDays[0].dayLabel} ($${Math.round(lowDays[0].cash)})`, severity:'high', section:'section-budget' });
  }
  ...
  if(nextPayout && nextSupplier && new Date(nextPayout.date) > new Date(nextSupplier.date)){
    alerts.push({ icon:'⚡', title:'Payment Timing Risk', desc:`Supplier payment of $${nextSupplier.amount} due ${nextSupplier.date} but next payout ($${nextPayout.amount}) arrives ${nextPayout.date}. Ensure enough cash reserve.`, severity:'medium', section:'section-supplier-hub' });
  }
  const dailyExpenses = d.dailyAdSpend + (d.ordersPerDay * d.avgProductCost) + (d.ordersPerDay * d.avgShippingCost) + d.monthlyFixedCosts/30;
  if(d.cash < dailyExpenses * 3){
    alerts.push({ icon:'⚠️', title:'Low Cash Reserve', desc:`Current cash ($${Math.round(d.cash)}) covers only ${Math.floor(d.cash/dailyExpenses)} days of expenses. Aim for 14+ days.`, severity:'medium', section:'section-business-sim' });
  }
  if(dailyExpenses > d.ordersPerDay * d.avgOrderValue){
    alerts.push({ icon:'📉', title:'Expenses Exceed Revenue', desc:`Daily costs ($${dailyExpenses.toFixed(0)}) exceed daily revenue ($${(d.ordersPerDay*d.avgOrderValue).toFixed(0)}). You're operating at a loss.`, severity:'high', section:'section-profit-lab' });
  }
  if(alerts.length === 0){
    alerts.push({ icon:'✅', title:'All Clear', desc:'No cash flow concerns detected. Your financial position is healthy.', severity:'low', section:'section-dashboard' });
  }
...
PluginRegistry.register('cash-flow', {
  ...
  name: 'Cash Flow',
  version: '1.0.0',
  description: 'Cash flow command center — track platform payouts, supplier payments, working capital, and when you can scale',
    ...
    section.innerHTML = `
      <div class="section-inner">
        ...
        ${window.HuntDrop.renderRelatedTools ? window.HuntDrop.renderRelatedTools([
          {section:'section-profit-lab', name:'Profit Calculator', desc:'Per-unit profit margins with all costs', icon:'💰', color:'var(--accent-green)'},
          {section:'section-budget', name:'Budget Planner', desc:'Allocate ad spend with cash constraints', icon:'💳', color:'var(--accent-purple)'},
          {section:'section-order-tracker', name:'Order Tracker', desc:'Track orders generating revenue', icon:'📦', color:'var(--accent-cyan)'},
          {section:'section-business-sim', name:'Business Simulator', desc:'Model growth scenarios with real cash data', icon:'🎯', color:'var(--accent-orange)'},
          {section:'section-shipping-calc', name:'Shipping Calculator', desc:'Optimize shipping costs per order', icon:'🚚', color:'var(--accent-yellow)'},
          {section:'section-refund-shield', name:'Refund Shield', desc:'Refund losses affect cash position', icon:'🛡', color:'var(--accent-red)'}
        ]) : ''}
      </div>`;
