    ...
    return new Promise(function(resolve) {
      ...
      var timeoutId = setTimeout(function() {
        if (attempt < MAX_RETRIES) {
          ...
          console.error('[DataAdapters] Load timeout after ' + (LOAD_TIMEOUT/1000) + 's x' + MAX_RETRIES + ' - falling back to inline products');
          var inline = getInlineProducts();
          if (inline.length > 0) {
      ...
      fetch('mock-products.json')
        ...
        .catch(function(e) {
          ...
          if (attempt < MAX_RETRIES) {
            ...
            var fallback = getInlineProducts();
...
function validateFilters(filters) {
  ...
  var validPlatforms = ['all','aliexpress','amazon','shopify','ebay','temu','tiktok','etsy','cjdropshipping','dhgate','wish'];
...
function mockSearch(allProducts, platformName, query, filters) {
  ...
  const results = allProducts.filter(p => {
    ...
    if (query) {
      ...
      let match = p.title.toLowerCase().indexOf(q) !== -1
        || (p.keywords && p.keywords.some(function(k){return k && k.toLowerCase().indexOf(q) !== -1;}))
        || (p.category && p.category.toLowerCase().indexOf(q) !== -1);
      if (!match && fuzzy) {
        match = fuzzy(p.title, query)
          || (p.keywords && p.keywords.some(function(k){return k && fuzzy(k, query);}))
  ...
  if (filters && filters.sort) {
    switch(filters.sort) {
      ...
      case 'competition': results.sort(function(a,b){var compOrder={low:0,medium:1,high:2};return (compOrder[a.competition]||1)-(compOrder[b.competition]||1);}); break;
  ...
  return {
    search: async (rawQuery, rawFilters = {}) => {
      ...
      if (PC && platformName === 'all') {
        const connectedPlatforms = Object.keys(PC.configs).filter(function(p) { return PC.isConnected(p); });
        if (connectedPlatforms.length > 0) {
          ...
          if (merged.length > 0) {
            ...
            if (filters && filters.sort) {
              switch(filters.sort) {
                case 'score': merged.sort(function(a,b){return b.score-a.score;}); break;
                case 'trending': merged.sort(function(a,b){return b.salesVelocity-a.salesVelocity;}); break;
                case 'profit': merged.sort(function(a,b){return b.margin-a.margin;}); break;
                case 'velocity': merged.sort(function(a,b){return b.salesVelocity-a.salesVelocity;}); break;
                case 'competition': merged.sort(function(a,b){var compOrder={low:0,medium:1,high:2};return (compOrder[a.competition]||1)-(compOrder[b.competition]||1);}); break;
                case 'price-low': merged.sort(function(a,b){return a.price-b.price;}); break;
                case 'price-high': merged.sort(function(a,b){return b.price-a.price;}); break;
              }
            }
            console.debug('[DataAdapters] Merged search: ' + realResultsAll.length + ' real + ' + mockResults.length + ' mock = ' + merged.length + ' total');
            return merged;
          }
        }
      }
    ...
    getProduct: async (id) => {
      await loadProducts();
      const p = getAllProducts().find(function(x){return x.id === id;});
      return p || null;
    },
    getTrends: async (id) => {
      await loadProducts();
