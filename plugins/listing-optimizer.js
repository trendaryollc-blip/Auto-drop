  ...
  try {
    ...
    const PLATFORMS = [
      {
        ...
        hasAPlus: true,
        rules: {
          titlePattern: '[Brand] [Product] [Key Feature] [Size/Qty] [Color]',
          requiresBrand: true,
          requiresASIN: false,
          imageMin: 1,
          imageMax: 9,
      ...
      },
      {
        ...
        hasAPlus: false,
        rules: {
          titlePattern: '[Product Name] — [Benefit]',
          requiresBrand: false,
          requiresASIN: false,
          imageMin: 1,
          imageMax: 20,
      ...
      },
      {
        ...
        hasAPlus: false,
        rules: {
          titlePattern: '[Brand] [Product] [Model] [Key Feature]',
          requiresBrand: false,
          requiresASIN: false,
          imageMin: 1,
          imageMax: 12,
      ...
      },
      {
        ...
        hasAPlus: false,
        rules: {
          titlePattern: '[Product] [Style] [Material] [Occasion]',
          requiresBrand: false,
          requiresASIN: false,
          imageMin: 1,
          imageMax: 10,
      ...
      },
      {
        ...
        hasAPlus: false,
        rules: {
          titlePattern: '[Hook] [Product] [Benefit]',
          requiresBrand: false,
          requiresASIN: false,
          imageMin: 1,
          imageMax: 9,
      ...
      },
      {
        ...
        hasAPlus: false,
        rules: {
          titlePattern: '[Product] [Feature] [Size] [Color]',
          requiresBrand: false,
          requiresASIN: false,
          imageMin: 1,
          imageMax: 10,
    ...
    const IMAGE_STRATEGIES = {
      default: [
        {
          slot: 1,
          type: 'Main White Background',
          desc: 'Clean product on pure white (RGB 255,255,255). No text, no watermarks. Fill 85%+ of frame.',
          required: true,
        },
        {
          slot: 2,
          type: 'Lifestyle In-Use',
          desc: 'Product being used by target audience in natural setting. Show scale and context.',
          required: true,
        },
        {
          slot: 3,
          type: 'Feature Callout',
          desc: 'Annotated image highlighting 3-5 key features with text overlays.',
          required: false,
        },
        {
          slot: 4,
          type: 'Size/Scale Reference',
          desc: 'Product next to common object or person for size reference.',
          required: false,
        },
        {
          slot: 5,
          type: 'Packaging/Unboxing',
          desc: 'What customer receives — box, accessories, manual. Sets expectations.',
          required: false,
        },
        {
          slot: 6,
          type: 'Comparison Chart',
          desc: 'Visual comparison vs competitors or before/after results.',
          required: false,
        },
        {
          slot: 7,
          type: 'Detail Close-Up',
          desc: 'Macro shot of material, stitching, texture, or key differentiator.',
          required: false,
        },
        {
          slot: 8,
          type: 'Social Proof/Review',
          desc: 'Screenshot of top review or user-generated content (with permission).',
          required: false,
        },
        {
          slot: 9,
          type: 'Infographic',
          desc: 'Size chart, care instructions, or technical specifications visual.',
          required: false,
        },
      ],
    };
    ...
    const SEASONAL_CALENDAR = [
      {
        ...
        tips: ['New Year resolution angle', 'Winter clearance positioning', 'Back-to-routine messaging'],
      },
      {
        month: 'Feb',
        season: "Valentine's Day",
        tips: ['Gift-giving angle', 'Couples/bundle positioning', 'Self-love messaging'],
      },
      {
        month: 'Mar',
        season: 'Spring Break',
        tips: ['Travel-friendly angle', 'Outdoor/refresh theme', 'Spring cleaning positioning'],
      },
      {
        month: 'Apr',
        season: 'Easter / Tax Season',
        tips: ['Spring refresh', 'Budget-conscious angle', 'Gift baskets'],
      },
      {
        month: 'May',
        season: "Mother's Day",
        tips: ['Gift guide positioning', 'Premium wrapping option', 'Bundle deals'],
      },
      {
        month: 'Jun',
        season: "Summer / Father's Day",
        tips: ['Outdoor/gadget angle', 'Dad gifts', 'Summer essentials'],
      },
      {
        month: 'Jul',
        season: 'Mid-Year / Prime Day',
        tips: ['Flash sale angle', 'Amazon Prime Day prep', 'Summer peak'],
      },
      ...
      {
        month: 'Oct',
        season: 'Halloween / Pre-Holiday',
        tips: ['Costume/party angle', 'Early bird deals', 'Holiday prep'],
      },
      {
        month: 'Nov',
        season: 'Black Friday / Cyber Monday',
        tips: ['Deal urgency', 'Bundle savings', 'Gift lists', 'BF/CM specific copy'],
      },
      {
        month: 'Dec',
        season: 'Holiday / Christmas',
        tips: ['Gift-giving focus', 'Express shipping urgency', 'Last-minute deals', 'New Year prep'],
      },
    ];
    ...
    function analyzeListing(text, platform) {
      ...
      READABILITY_WORDS.easy.forEach((w) => {
        if (allText.includes(w)) readScore += 2;
      });
      READABILITY_WORDS.hard.forEach((w) => {
        if (allText.includes(w)) readScore -= 3;
      });
    ...
    function generateBullets(title, description, platform) {
      ...
      descSentences.slice(0, 5).forEach((s, i) => {
        features.push({
          label:
            ['✅ Premium Quality', '🚀 Fast Results', '💎 Built to Last', '🎯 Perfect Fit', '🛡️ Risk-Free'][i] ||
            '✨ Feature',
      ...
      while (features.length < 5) {
        features.push({
          label: '⭐ Benefit',
          text: 'Designed for everyday use with premium materials and craftsmanship.',
        });
      }
    ...
    function predictPerformance(scores) {
      const ctr = Math.min(
        15,
        1.5 + (scores.titleSeo / 100) * 4 + (scores.emotionalImpact / 100) * 3 + (scores.keywordCoverage / 100) * 2
      );
          ...
          scores.overall >= 80
            ...
            : scores.overall >= 65
              ...
              : scores.overall >= 50
                ...
                : scores.overall >= 35
                  ? 'Below Average — major rewrite recommended'
                  : 'Poor — complete overhaul needed',
      };
    }
    ...
    function generateABVariants(original, scores) {
      ...
      if (scores.emotionalImpact < 50) {
        ...
        variants.push({
          ...
          title: pick + ' ' + original,
          hypothesis: 'Adding urgency word increases CTR by 10-20%',
          changeType: 'urgency',
        });
      }
      if (scores.titleSeo < 60) {
        variants.push({
          name: 'Benefit-First Variant',
          title: words
            .slice(Math.floor(words.length / 2))
            .concat(words.slice(0, Math.floor(words.length / 2)))
            .join(' '),
          hypothesis: 'Rearranging to lead with benefits improves scan-ability',
          changeType: 'reorder',
        });
      }
      ...
      variants.push({
        ...
        title: shortTitle,
        hypothesis: 'Shorter titles improve mobile readability and CTR',
        changeType: 'shorten',
      });
      const powerWords = ['Premium', 'Pro', 'Ultra', 'Elite', 'Advanced'];
      const pw = powerWords[Math.floor(Math.random() * powerWords.length)];
      variants.push({
        name: 'Power Word Variant',
        title: pw + ' ' + original,
        hypothesis: 'Power words increase perceived value and conversion',
        changeType: 'power',
      });
      return variants;
    }
    ...
    const ListingOptimizerPlugin = {
      ...
      version: '1.0.0',
      description:
        'AI-powered listing intelligence — optimize titles, descriptions, images, and keywords across 6 platforms',
      _section: null,
      _cleanups: [],
            ...
            ? window.HuntDrop.renderRelatedTools([
                {
                  ...
                  name: 'Ad Creator',
                  desc: 'Generate ads from your optimized listing',
                  icon: '📢',
                ...
                },
                {
                  ...
                  desc: 'Calculate margins with optimized pricing',
                  icon: '💰',
    ...
    function bindEvents(cfg) {
      ...
      UI.$('loAnalyzeBtn')?.addEventListener('click', () => {
        ...
        const backendTerms = backendText
          .split(',')
          .map((t) => t.trim())
          .filter(Boolean);
    ...
    function renderHeatmap(title, keywords) {
      ...
      const words = title.split(/\s+/).filter(Boolean);
      el.innerHTML = `<div class="lo-heat-words">${words
        .map((w) => {
    ...
    console.error('[ListingOptimizer] error:', e);
  }
})();
