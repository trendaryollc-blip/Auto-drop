...
(function () {
  ...
  const CATEGORY_COLORS = ['#00e5ff', '#00ff88', '#a855f7', '#ff3366', '#ff8a00', '#fbbf24', '#34d399', '#60a5fa', '#f472b6', '#c084fc'];
  ...
  function buildDataFromProducts(products) {
    ...
    Object.entries(grouped).forEach(([category, catProducts]) => {
      ...
      const avgSaturation = Math.round(catProducts.reduce((s, p) => s + (p.marketSaturation || 50), 0) / catProducts.length);
      ...
      const trend = trends.filter((t) => t === 'rising').length > trends.length / 2 ? 'rising'
        : trends.filter((t) => t === 'declining').length > trends.length / 2 ? 'declining'
        : 'stable';
      ...
      if (!bestArb) {
        ...
        bestArb = { buy: Math.round(avgPrice * 0.4), sell: avgPrice, buyPlatform: 'aliexpress', sellPlatform: 'amazon', margin: 50, profit: +(avgPrice * 0.6).toFixed(2) };
      }
      ...
      // Opportunity text
      const opportunity = avgDemand > avgSupply
      ...
      catProducts.forEach((p) => {
        ...
        if (pTrend === 'rising' && pDemand > 40) {
          ...
          const growth = td.length >= 2
            ? Math.round(((td[td.length - 1] - td[0]) / (td[0] || 1)) * 100)
            : 0;
          const platPrices = Object.entries(p.platformPrices || {}).filter(([, v]) => v > 0).map(([k]) => k);
          DATA.emerging.push({
            id: emId++,
      ...
      chip.addEventListener('click', () => {
        ...
        if (kw) {
          ...
          setTimeout(() => {
            ...
            if (si) { si.value = kw; si.dispatchEvent(new Event('input', { bubbles: true })); }
  ...
  function _renderGaps() {
    ...
    if (!gaps.length) return '<div class="mg-empty">No market gaps found. Run a search to analyze products and discover opportunities.</div>';
  ...
  function _renderArbitrage() {
    ...
    if (!arb.length) return '<div class="mg-empty">No arbitrage opportunities found. Run a search to discover cross-platform price gaps.</div>';
  ...
  function _renderEmerging() {
    ...
    if (!em.length) return '<div class="mg-empty">No emerging niches found. Run a search to identify trending products.</div>';
  ...
  function renderActionZone() {
    ...
    el.querySelector('#mgAZCompare')?.addEventListener('click', () => {
      ...
      if (catFilter) { catFilter.value = 'all'; catFilter.dispatchEvent(new Event('change')); }
    });
  ...
  function calculate() {
    ...
    if (!recommended.length) {
      res.innerHTML = '<div class="mg-calc-empty">No opportunities match your criteria. Try increasing budget or risk tolerance.</div>';
      return;
    }
  ...
  function renderInsights() {
    ...
    if (!el) return;
    if (!DATA.gaps.length) {
      el.innerHTML = '<div class="mg-insights-section"><div class="mg-section-header"><h3 class="mg-section-title">&#x1F4A1; AI Insights</h3></div><div class="mg-empty">No data to generate insights. Run a search to analyze products.</div></div>';
      return;
    }
    ...
    el.innerHTML = `<div class="mg-insights-section">
      ...
      <div class="mg-insights-grid">
        ...
        <div class="mg-insight-card fastest"><div class="mg-insight-icon" style="background:var(--accent-purple-dim);color:var(--accent-purple)">&#x1F680;</div><div class="mg-insight-label">Rising Trends</div><div class="mg-insight-title">${rising.length} niches</div><div class="mg-insight-list">${rising.slice(0, 3).map((g, i) => `<div class="mg-insight-item"><span class="mg-insight-item-rank">#${i + 1}</span><span class="mg-insight-item-name">${esc(g.category)}</span><span class="mg-insight-item-val" style="color:var(--accent-green)">Rising</span></div>`).join('')}</div></div>
      </div>
    </div>`;
  }
  ...
  function renderCharts() {
    ...
    if (!el) return;
    if (!DATA.gaps.length) { el.innerHTML = ''; return; }
  ...
  function renderCategories() {
    ...
    el.innerHTML = `<div class="mg-categories-section">
      ...
      <div class="mg-category-grid">${DATA.categories.map((c) => `<div class="mg-category-card" data-cat-filter="${esc(c.name)}">
  ...
  function _render() {
    const fns = [
      ...
      renderCharts, renderCategories, renderTabs, renderBattlefield, renderNicheRadar,
      renderLifecycle, renderProductHunt, renderActionZone, renderCalculator,
    ];
    fns.forEach(function (fn) {
      try { fn(); } catch (e) { console.error('[MG] render error:', e); }
    });
  }
  ...
  const P = {
    ...
    unmount(_ctx) {
      ...
      _charts = {};
      if (_unsubSearch) { _unsubSearch(); _unsubSearch = null; }
