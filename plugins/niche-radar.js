...
function renderHeatmap(seasonality) {
  ...
  return `<div class="nr-heatmap">${seasonality.map((v,i) => {
    const intensity = v / max;
    const color = intensity > 0.9 ? 'var(--accent-red)' : intensity > 0.75 ? 'var(--accent-orange)' : intensity > 0.6 ? 'var(--accent-yellow)' : 'var(--accent-cyan)';
...
const NicheRadarPlugin = {
  ...
  mount(ctx){
    ...
    for(let i = 0; i < 4; i++) {
      ...
      seasonalHtml.push(`<div class="nr-seasonal-card" style="animation-delay:${i*0.08}s"><div class="nr-seasonal-month">${esc(s.month)}</div><div class="nr-seasonal-niches">${s.niches.map(n=>'<span class="nr-seasonal-niche">'+esc(n)+'</span>').join('')}</div><div class="nr-seasonal-reason">${esc(s.reason)}</div></div>`);
    }
...
window.HuntDrop._nicheExplore = function(idOrName) {
  ...
  if(searchInput) { searchInput.value = niche.name; searchInput.dispatchEvent(new Event('input')); }
...
window.HuntDrop._nicheDetail = function(id) {
  ...
  const heatVar = n.heat==='hot'?'var(--accent-red)':n.heat==='warm'?'var(--accent-orange)':'var(--accent-cyan)';
  const lifecycleColor = n.lifecycle==='emerging'?'var(--accent-cyan)':n.lifecycle==='growing'?'var(--accent-green)':'var(--accent-orange)';
  const riskColor = n.riskScore<30?'var(--accent-green)':n.riskScore<50?'var(--accent-yellow)':'var(--accent-red)';
  ...
  const tW = 800, tH = 160, tPad = 10;
  const tMin = Math.min(...n.trendData), tMax = Math.max(...n.trendData);
  ...
  const trendArea = trendPath + ' L'+trendPoints[trendPoints.length-1].x.toFixed(1)+','+tH+' L'+trendPoints[0].x.toFixed(1)+','+tH+' Z';
        ...
        <div class="nd-section">
          ...
          <div class="nd-platform-bars">
            ${n.platforms.map(p => {
              const barW = Math.min((p.sellers / 50) * 100, 100);
              const c = p.sellers < 8 ? 'var(--accent-green)' : p.sellers < 20 ? 'var(--accent-yellow)' : 'var(--accent-red)';
              return '<div class="nd-plat-row"><div class="nd-plat-header"><span class="nd-plat-name">'+esc(p.name)+'</span><span class="nd-plat-count">'+p.sellers+' sellers</span></div><div class="nd-plat-bar-wrap"><div class="nd-plat-bar" style="width:'+barW+'%;background:'+c+'"></div></div><span class="nd-plat-avg">avg $'+p.avgPrice+'</span></div>';
            }).join('')}
          </div>
        </div>
        <div class="nd-section">
          ...
          <div class="nd-seasonal-grid">
            ${n.seasonality.map((v,i) => {
              const intensity = v / maxSeason;
              const h = n.heat==='hot'?'0,100%,60%':n.heat==='warm'?'30,100%,55%':'180,100%,55%';
              return '<div class="nd-seasonal-cell" style="--cell-opacity:'+(0.2+intensity*0.8)+';--cell-hue:'+h+'"><div class="nd-seasonal-val">'+v+'</div><div class="nd-seasonal-month">'+months[i]+'</div></div>';
            }).join('')}
          </div>
        </div>
      </div>
