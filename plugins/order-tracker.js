...
(function () {
  try {
    ...
    const SAMPLE_ORDERS = [
      ...
      },
      {
        ...
        status: 'shipped',
        carrier: 'epacket',
        tracking: 'EP9876543210',
        cost: 6.5,
        sellPrice: 18.99,
        shipFrom: 'China',
        destination: 'USA',
        orderedAt: '2026-07-11',
        shippedAt: '2026-07-13',
        estDelivery: '2026-07-25',
        notes: '',
      },
      {
        id: 'ORD-003',
      ...
      },
      {
        ...
        estDelivery: '2026-07-20',
        notes: 'Customer reports wrong color sent',
      },
      {
        id: 'ORD-005',
      ...
      },
      {
        ...
        estDelivery: '2026-07-19',
        notes: '',
      },
      {
        id: 'ORD-007',
        product: 'Kitchen Timer Set',
        customer: 'Nina P.',
        date: '2026-07-14',
        status: 'ordered',
        carrier: '',
      ...
      },
      {
        ...
        estDelivery: '2026-07-19',
        notes: 'Item damaged in transit — full refund issued',
      },
      {
        id: 'ORD-009',
    ...
    function getStoredOrders() {
      try {
        return JSON.parse(localStorage.getItem('hd_orders')) || SAMPLE_ORDERS;
      } catch (e) {
    ...
    function renderSupplierGrid() {
      ...
      _orders.forEach((o) => {
        ...
        if (isLate(o)) s.late++;
        if (!['refunded', 'returned'].includes(o.status)) {
          s.revenue += o.sellPrice;
          s.cost += o.cost;
        }
      ...
      el.innerHTML = sorted
        .map((s) => {
          const grade =
            s.fulfillmentRate >= 90 ? 'A' : s.fulfillmentRate >= 75 ? 'B' : s.fulfillmentRate >= 60 ? 'C' : 'D';
          const gradeColor =
            grade === 'A'
              ? 'var(--accent-green)'
              : grade === 'B'
      ...
      _orders.forEach((o) => {
        if (isLate(o))
          alerts.push({
            ...
            icon: '⏰',
            title: `${o.id} is LATE`,
            desc: `${o.product} for ${o.customer} — expected ${o.estDelivery}`,
            severity: 'high',
            order: o,
        ...
        if (o.status === 'issue')
          alerts.push({
            type: 'issue',
            icon: '⚠️',
            title: `${o.id} has an ISSUE`,
            desc: `${o.notes || 'No details provided'}`,
            severity: 'high',
            order: o,
            section: 'section-refund-shield',
          });
        if (o.shippedAt && !o.tracking && ['shipped', 'in_transit'].includes(o.status))
          alerts.push({
            type: 'no-track',
            icon: '🔍',
            title: `${o.id} — No tracking number`,
            desc: `${o.product} shipped but no tracking number entered`,
            severity: 'medium',
            order: o,
            section: 'section-supplier-intel',
          });
        if (!o.shippedAt && o.status === 'ordered' && daysSince(o.orderedAt) > 2)
          alerts.push({
            type: 'unshipped',
            icon: '📦',
            title: `${o.id} — Not yet shipped`,
            desc: `Ordered ${daysSince(o.orderedAt)} days ago, still processing`,
            severity: 'medium',
            order: o,
  ...
  `
      ...
      <pre class="ot-wismo-text">${esc(reply)}</pre>
        ...
        out.querySelector('.ot-copy-btn')?.addEventListener('click', function () {
          navigator.clipboard.writeText(reply).then(() => {
            UI.toast && UI.toast('Copied to clipboard!', 'success');
          });
        });
      });
    }
    ...
    PluginRegistry.register('order-tracker', {
      ...
      version: '1.0.0',
      description:
        'Complete order fulfillment command center — track shipments, manage WISMO tickets, monitor supplier performance',
      ...
      <div class="section-inner">
        ...
        <div class="ot-section">
          ...
          <div class="ot-pipeline" id="otPipeline">
            ${STATUSES.filter((s) => !['completed', 'refunded', 'returned'].includes(s.id))
              .map((s) => {
        ...
        <div class="ot-section">
          ...
          <div class="ot-filters">
            ...
            ${STATUSES.map((s) => {
              const c = _orders.filter((o) => o.status === s.id).length;
              return c > 0
                ? `<button class="ot-filter-btn" data-filter="${s.id}">${s.icon} ${s.name} (${c})</button>`
                : '';
            }).join('')}
          </div>
            ...
            <div class="ot-wismo-select">
              ...
              <select id="otWismoOrder" class="ot-select">
                ...
                ${_orders
                  ...
                  .map((o) => `<option value="${o.id}">${o.id} — ${o.product} (${o.customer})</option>`)
                  .join('')}
              </select>
            </div>
          ...
          window.HuntDrop.renderRelatedTools
            ? window.HuntDrop.renderRelatedTools([
                ...
                },
                {
                  section: 'section-supplier-intel',
                  name: 'Supplier Check',
                  desc: 'Verify supplier reliability scores',
                  icon: '🛡',
                ...
                },
                {
                  ...
                  name: 'Store Health',
                  desc: 'Fulfillment data feeds store scoring',
                  icon: '❤',
                ...
                },
                {
                  ...
                  desc: 'Compare shipping methods and costs',
                  icon: '🚚',
                ...
                },
                {
                  section: 'section-refund-shield',
                  name: 'Refund Shield',
                  desc: 'Track refund patterns and root causes',
                  icon: '🛡',
                ...
                },
                {
                  ...
                  name: 'Cash Flow',
                  desc: 'Monitor cash position from order revenue',
                  icon: '💳',
                  color: 'var(--accent-cyan)',
                },
              ])
        ...
        section.addEventListener('click', (e) => {
          ...
          if (!card) return;
          if (e.target.closest('select, button, a, .ot-status-select, .ot-wismo-btn, .ot-delete-btn, .ot-track-link'))
            return;
          e.preventDefault();
    ...
    console.error('[OrderTracker] error:', e);
  }
})();
