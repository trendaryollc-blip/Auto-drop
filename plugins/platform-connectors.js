...
(function () {
  ...
  function normalizeEbayProduct(p) {
    ...
    return {
      ...
      images: p.image && p.image.imageUrl ? [p.image.imageUrl] : [],
      platform: 'ebay',
      price: price,
      originalPrice: price,
      margin: 0,
  ...
  function normalizeRedditPost(p) {
    return {
      ...
      images: p.thumbnail && p.thumbnail.startsWith('http') ? [p.thumbnail] : [],
      platform: 'reddit',
      price: 0,
      originalPrice: 0,
      margin: 0,
  ...
  function normalizePinterestPin(p) {
    const media = p.media || {};
    const origUrl = media.images && media.images.orig && media.images.orig.url ? media.images.orig.url : '';
    return {
      id: 'pt_' + (p.id || Math.random().toString(36).substr(2, 9)),
      title: p.title || p.note || '',
      image: origUrl,
      images: origUrl ? [origUrl] : [],
      platform: 'pinterest',
      price: 0,
      originalPrice: 0,
      margin: 0,
  ...
  function getProxyUrl() {
    ...
    return 'https://backend-psi-five-60.vercel.app/api/platform';
  }
