...
function onProductSelect(id){
  if(!id){_selectedProduct=null;return;}
  const products=window.HuntDrop.ALL_PRODUCTS||[];
...
function matchCompetitor(){
  ...
  if(slider){slider.value=Math.round(target*100);simulate();}
...
const PriceElasticityPlugin = {
  ...
  init(_ctx){ Config.defaults('elasticity',{defaultProduct:null}); },
  ...
  unmount(_ctx){
    ...
    if(_demandChart){_demandChart.destroy();_demandChart=null;}
