...
(function () {
  ...
  let _chart = null;
  let _demandChart = null;
  let _section = null;

  function populateProductDropdown() {
    const sel = _section ? _section.querySelector('#peProductSelect') : null;
    if (!sel) return;
    const products = window.HuntDrop.ALL_PRODUCTS || [];
    sel.innerHTML =
      '<option value="">Select a product...</option>' +
      products.map((p) => `<option value="${esc(p.id)}">${esc(p.title.split('—')[0].trim())}</option>`).join('');
  }
  ...
  function onProductSelect(id) {
    ...
    const products = window.HuntDrop.ALL_PRODUCTS || [];
    _selectedProduct = products.find((p) => p.id === parseInt(id));
  ...
  function renderFull() {
    ...
    if (!p) return;
    const el = _section ? _section.querySelector('#peResults') : null;
    if (!el) return;
        ...
        <div class="pes-price-tiers">
          ${priceRanges
            .map((r) => {
              ...
              const tierDemand = Math.max(
                100,
                Math.round(
                  p.salesVelocity *
                    Math.pow(r.mult, p.competition === 'high' ? -2 : p.competition === 'medium' ? -1.7 : -1.5)
                )
              );
    ...
    if (slider) slider.addEventListener('input', () => simulate());
    const matchBtn = el.querySelector('#peMatchCompetitorBtn');
    if (matchBtn) matchBtn.addEventListener('click', () => matchCompetitor());
    simulate();
  }
  ...
  function simulate() {
    ...
    if (priceLabel) {
      ...
      priceLabel.style.color = newPrice >= p.price ? 'var(--accent-green)' : 'var(--accent-red)';
    }
    ...
    renderMetrics(newPrice, newDemand, demandChange, revenueChange, profitChange, newProfit, newMargin);
    renderChart(newPrice);
    renderDemandChart(newPrice);
    renderSweetSpot(p);
    renderStrategy(p, newPrice, newDemand, newProfit, costPerUnit);
    renderCompetitorOverlay(p, newPrice);
  }

  function renderMetrics(price, demand, demandChg, revChg, profitChg, profit, margin) {
    const el = _section ? _section.querySelector('#peMetrics') : null;
    if (!el) return;
    const fmtChg = (v) => {
  ...
  function renderChart(_newPrice) {
    const canvas = _section ? _section.querySelector('#peChart') : null;
    if (!canvas) return;
    ...
    _chart = new Chart(canvas, {
      ...
      options: {
        ...
        plugins: {
          legend: {
            position: 'bottom',
            labels: { color: '#8888a4', font: { family: 'Inter', size: 11 }, usePointStyle: true, padding: 16 },
          },
        },
        scales: {
          x: {
            grid: { color: 'rgba(255,255,255,0.025)' },
            ticks: { color: '#555570', font: { family: 'JetBrains Mono', size: 9 }, maxRotation: 45 },
          },
          y: {
            grid: { color: 'rgba(255,255,255,0.025)' },
            ticks: {
  ...
  function renderDemandChart(newPrice) {
    const canvas = _section ? _section.querySelector('#peDemandChart') : null;
    if (!canvas) return;
    if (_demandChart) _demandChart.destroy();
    const p = _selectedProduct;
    const elasticity = -1.5 - (p.competition === 'high' ? 0.5 : p.competition === 'medium' ? 0.2 : 0);
    const prices = [];
    const demands = [];
    ...
    _demandChart = new Chart(canvas, {
      type: 'bar',
      data: {
        labels: prices,
        datasets: [{ label: 'Demand', data: demands, backgroundColor: colors, borderRadius: 4 }],
      },
      options: {
        responsive: true,
        maintainAspectRatio: false,
        ...
        scales: {
          x: {
            grid: { display: false },
            ticks: { color: '#555570', font: { family: 'JetBrains Mono', size: 8 }, maxRotation: 45 },
          },
          y: {
            grid: { color: 'rgba(255,255,255,0.025)' },
  ...
  function renderSweetSpot(p) {
    const el = _section ? _section.querySelector('#peSweetSpot') : null;
    if (!el) return;
    const costPerUnit = p.price * (1 - p.margin / 100);
  ...
  function renderStrategy(p, price, demand, profit, costPerUnit) {
    const el = _section ? _section.querySelector('#peStrategy') : null;
    if (!el) return;
    const margin = price > 0 ? ((price - costPerUnit) / price) * 100 : 0;
    const avgPrice =
      Object.values(p.platformPrices).reduce((a, b) => a + b, 0) / Object.values(p.platformPrices).length;
    const strategies = [];

    if (margin >= 40)
      strategies.push({
        icon: '✅',
        title: 'High-Margin Pricing',
        text: `Your ${margin.toFixed(1)}% margin is excellent. You have room to invest in ads and still stay profitable. Consider reinvesting 20-30% of margin into scaling.`,
      });
    else if (margin >= 20)
      strategies.push({
        icon: '⚠️',
        title: 'Moderate Margin',
        text: `${margin.toFixed(1)}% margin is workable but tight. Focus on reducing ad costs or increasing perceived value to boost margin above 30%.`,
      });
    else
      strategies.push({
        icon: '🚨',
        title: 'Low Margin Warning',
        text: `${margin.toFixed(1)}% margin is dangerously low. One bad ad day could erase profits. Raise price or reduce costs immediately.`,
      });
    ...
    if (price < avgPrice * 0.8)
      strategies.push({
        ...
        text: `You're priced ${((1 - price / avgPrice) * 100).toFixed(0)}% below average. Unless you're intentionally going for volume, you're leaving money on the table.`,
      });
    else if (price > avgPrice * 1.3)
      strategies.push({
        icon: '💡',
        title: 'Premium Positioning',
        text: `You're priced ${((price / avgPrice - 1) * 100).toFixed(0)}% above average. Make sure your ad copy and product page justify the premium.`,
      });

    if (p.competition === 'high')
      strategies.push({
        icon: '⚔️',
        title: 'High Competition Strategy',
        text: `In a crowded market, differentiate on brand/story/guarantee rather than price. Competing on price alone will destroy margins.`,
      });

    strategies.push({
      icon: '📊',
      title: 'Revenue Mix',
      text: `At $${price.toFixed(2)}, you earn $${(price - costPerUnit).toFixed(2)} profit per sale × ${demand.toLocaleString()} sales = $${profit.toLocaleString(undefined, { maximumFractionDigits: 0 })}/mo. To hit $10K/mo, you need ${Math.ceil(10000 / (price - costPerUnit)).toLocaleString()} sales.`,
    });
  ...
  function renderCompetitorOverlay(p, currentPrice) {
    const el = _section ? _section.querySelector('#peCompetitorOverlay') : null;
    if (!el) return;
  ...
  function matchCompetitor() {
    ...
    const slider = _section ? _section.querySelector('#pePriceSlider') : null;
    if (slider) {
      slider.value = Math.round(target * 100);
      simulate();
    }
  ...
  const PriceElasticityPlugin = {
    ...
    init(_ctx) {
      Config.defaults('elasticity', { defaultProduct: null });
    },
    ...
    mount(_ctx) {
      ...
      if (sel) sel.addEventListener('change', () => onProductSelect(sel.value));
    },
