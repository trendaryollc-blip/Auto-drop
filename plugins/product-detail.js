...
function destroyCharts() {
  Object.keys(_currentChart).forEach(function(k){
    if(_currentChart[k]){try{_currentChart[k].destroy();}catch(e){/* ignored */} _currentChart[k]=null;}
  });
}
...
function renderDetail(p) {
  ...
  const platColorMap = {aliexpress:'#e62e04',amazon:'#ff9900',shopify:'#96bf48',ebay:'#e53238',temu:'#fb7701',tiktok:'#00f2ea',etsy:'#f1641e',cjdropshipping:'#40c351',dhgate:'#e62e04',wish:'#2fb7ec'};
  ...
  Object.keys(p.platformPrices||{}).forEach(function(pl){
    ...
    platformPricesHtml += '<div class="pd-price-card'+(isBest?' best':'')+'" data-platform="'+pl+'" data-price="'+pr+'" role="button" tabindex="0" style="cursor:pointer">'
  ...
  const suppliersHtml = (p.suppliers||[]).map(function(s){
    return '<div class="pd-supplier-card">'
      +'<div class="pd-supplier-header">'
      +'<div class="pd-supplier-avatar">'+esc((s.name||'U').charAt(0))+'</div>'
      +'<div><div class="pd-supplier-name">'+esc(s.name||'Unknown')+'</div><div class="pd-supplier-location">'+esc(s.location||'N/A')+'</div></div>'
      +'</div>'
      +'<div class="pd-supplier-stats">'
      +'<div class="pd-supplier-stat"><div class="pd-supplier-stat-label">Rating</div><div class="pd-supplier-stat-value" style="color:var(--accent-yellow)">'+(s.rating||0)+'&#9733;</div></div>'
      +'<div class="pd-supplier-stat"><div class="pd-supplier-stat-label">Orders</div><div class="pd-supplier-stat-value">'+esc(s.orders||'N/A')+'</div></div>'
      +'<div class="pd-supplier-stat"><div class="pd-supplier-stat-label">Response</div><div class="pd-supplier-stat-value">'+esc(s.responseTime||'N/A')+'</div></div>'
  ...
  const riskColor = p.riskScore<30?'var(--accent-green)':p.riskScore<50?'var(--accent-yellow)':'var(--accent-red)';
  const riskLabel = p.riskScore<30?'Low Risk':p.riskScore<50?'Medium Risk':'High Risk';
  const satColor = p.marketSaturation<40?'var(--accent-green)':p.marketSaturation<60?'var(--accent-yellow)':'var(--accent-red)';
  const satLabel = p.marketSaturation<40?'Unsaturated':p.marketSaturation<60?'Moderate':'Saturated';

  const keywordsHtml = (p.keywords||[]).slice(0,10).map(function(k){return '<span class="pd-keyword" data-keyword="'+esc(k)+'" role="button" tabindex="0" style="cursor:pointer">'+esc(k)+'</span>';}).join('');
  const interestsHtml = (p.audience&&p.audience.interests||[]).map(function(i){return '<span class="pd-audience-tag" data-keyword="'+esc(i)+'" role="button" tabindex="0" style="cursor:pointer">'+esc(i)+'</span>';}).join('');
  const countriesHtml = (p.audience&&p.audience.countries||[]).map(function(c){return '<span class="pd-audience-tag">'+esc(c)+'</span>';}).join('');
  ...
  return '<div class="pd-hero">'
    ...
    +'<div class="pd-audience-card"><span class="pd-audience-value">'+esc((p.audience&&p.audience.age)||'N/A')+'</span><span class="pd-audience-label">Age Range</span></div>'
    +'<div class="pd-audience-card"><span class="pd-audience-value">'+esc((p.audience&&p.audience.gender)||'N/A')+'</span><span class="pd-audience-label">Gender</span></div>'
...
function renderCharts(p) {
  ...
  if(pc){const labels=Object.keys(p.platformPrices).map(function(s){return cap(s);});_currentChart.price = new Chart(pc,{type:'bar',data:{labels:labels,datasets:[{data:Object.values(p.platformPrices),backgroundColor:'rgba(0,229,255,0.4)',borderColor:'#00e5ff',borderWidth:1,borderRadius:5}]},options:{...chartOpts,scales:{...chartOpts.scales,y:{...chartOpts.scales.y,ticks:{...chartOpts.scales.y.ticks,callback:function(v){return '$'+v.toFixed(0);}}}}}});}

  const prc = document.getElementById('pdProfitChart');
  if(prc){const sp=p.platformPrices.amazon||p.price,cost=p.price,ship=2.50,ads=sp*0.15,profit=sp-cost-ship-ads;_currentChart.profit = new Chart(prc,{type:'bar',data:{labels:['Sell Price','Cost','Shipping','Ads','Net Profit'],datasets:[{data:[sp,-cost,-ship,-ads,profit],backgroundColor:['rgba(0,229,255,0.6)','rgba(255,51,102,0.6)','rgba(255,138,0,0.6)','rgba(168,85,247,0.6)',profit>0?'rgba(0,255,136,0.6)':'rgba(255,51,102,0.6)'],borderColor:['#00e5ff','#ff3366','#ff8a00','#a855f7',profit>0?'#00ff88':'#ff3366'],borderWidth:1,borderRadius:5}]},options:{...chartOpts,scales:{...chartOpts.scales,y:{...chartOpts.scales.y,ticks:{...chartOpts.scales.y.ticks,callback:function(v){return '$'+v.toFixed(0);}}}}}});}
}
      ...
      EventBus.on('product:analyze', function(data) {
        ...
        if (keyword) {
          ...
          if (kw) {
            ...
            setTimeout(function() {
              const searchInput = document.getElementById('searchPageInput') || document.getElementById('searchInput');
              if (searchInput) {
        ...
        if (interest) {
          ...
          if (kw2) {
            window.HuntDrop.navigateTo('section-search');
            setTimeout(function() {
              const searchInput2 = document.getElementById('searchPageInput') || document.getElementById('searchInput');
              if (searchInput2) {
        ...
        if (insightTag) {
          ...
          if (kw3) {
            window.HuntDrop.navigateTo('section-search');
            setTimeout(function() {
              const searchInput3 = document.getElementById('searchPageInput') || document.getElementById('searchInput');
              if (searchInput3) {
