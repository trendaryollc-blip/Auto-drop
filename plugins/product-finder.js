...
(function() {
  ...
  PluginRegistry.register('product-finder', {
    ...
    performSearch(query) {
      ...
      // Perform search using DataLayer
      setTimeout(() => {
      ...
      resultsGrid.innerHTML = results.map((product, index) => `
        <div class="pf-product-card" data-id="${product.id}" style="animation: fadeUp 0.4s ease ${index * 0.05}s both;">
          ...
          <div class="pf-product-info">
            ...
            <button class="pf-select-btn" onclick="window.HuntDrop.Plugins['product-finder'].selectProduct('${product.id}')">
              Select Product →
            </button>
          </div>
        </div>
      ...
      if (selectedBanner) {
        selectedBanner.innerHTML = `
          ...
          <div class="product-selected-actions">
            <button class="unified-btn unified-btn-secondary" onclick="window.HuntDrop.Plugins['product-finder'].clearSelection()">Change Product</button>
          </div>
        `;
      }
