...
function renderCard(p) {
  ...
  const compColor = {low:'var(--accent-green)',medium:'var(--accent-yellow)',high:'var(--accent-red)'}[comp]||'var(--accent-yellow)';
  ...
  const fmtN = function(n){ return n>=1000?(n/1000).toFixed(1)+'K':n.toString(); };
  const badges = (p.badges || []).map(function(b){const safe=esc(b);const cls=safe.replace(/\s+/g,'-').replace(/[^a-zA-Z0-9_-]/g,'');return '<span class="badge badge-'+cls+'">'+(b==='ai'?'AI':safe)+'</span>';}).join('');
  ...
  const img = esc(UI.normalizeImageUrl ? UI.normalizeImageUrl(p.image, 'https://via.placeholder.com/300x200?text=Product') : (p.image || 'https://via.placeholder.com/300x200?text=Product'));
  const title = esc(p.title || 'Unknown Product');
  const margin = p.margin || 0;
  const marginClass = margin >= 50 ? 'margin-high' : margin >= 30 ? 'margin-med' : 'margin-low';

  var selectCb = window.HuntDrop.StoreConnect && window.HuntDrop.StoreConnect.isSelectMode() ? '<input type="checkbox" class="sc-select-cb" data-sel-id="'+esc(String(p.id))+'" style="position:absolute;top:8px;right:8px;z-index:5;width:20px;height:20px;cursor:pointer;accent-color:var(--accent-cyan)">' : '';
  return '<div class="product-card" data-product-id="'+esc(String(p.id))+'">'
    +'<div class="card-image">'
    +'<img src="'+img+'" alt="'+title+'" loading="lazy" decoding="async" fetchpriority="low">'
    +'<div class="card-badges">'+badges+'</div>'
    +'<div class="card-score '+scoreClass+'">'+score+'</div>'
    +'<div class="card-platform platform-'+esc(p.platform||'unknown')+'"><span class="platform-dot"></span>'+esc(cap(p.platform))+'</div>'
    +selectCb
    +'</div>'
    +'<div class="card-body">'
    ...
    +'<div class="metric"><span class="metric-label">Orders</span><span class="metric-value">'+fmtN(p.orders||0)+'</span><div class="metric-bar"><div class="metric-bar-fill bar-purple" style="width:'+Math.min(parseOrders(p.orders)/32,100)+'%"></div></div></div>'
    +'</div>'
    +'</div>'
    ...
    +'<button class="push-trendaryo-btn" data-push-id="'+p.id+'" title="Push to Trendaryo"><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><path d="M3 9l9-7 9 7v11a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2z"/><polyline points="9 22 9 12 15 12 15 22"/></svg> Store</button>'
...
function bindPushButtons(grid) {
  ...
  grid.querySelectorAll('.push-trendaryo-btn').forEach(function(btn) {
    ...
    btn.addEventListener('click', async function(e) {
      ...
      const p = _allResults.find(function(x){ return String(x.id) === String(id); });
      if (!p) return;
      this.disabled = true;
      this.textContent = '...';
      try {
        ...
        this.textContent = '✅';
        setTimeout(function(){ btn.textContent = 'Store'; btn.disabled = false; }, 2000);
      } catch(e) {
        this.textContent = '❌';
        setTimeout(function(){ btn.textContent = 'Store'; btn.disabled = false; }, 2000);
      }
...
function updateBulkBar() {
  ...
  if (!bar) return;
  var sc = window.HuntDrop.StoreConnect;
  if (!sc) return;
  var active = sc.isSelectMode();
  bar.classList.toggle('active', active);
  if (active) {
    var count = sc.getSelectedProducts().length;
...
function updateActiveFilters(filters, query) {
  ...
  if (query) {
    html += '<span class="sr-af-chip">Search: "' + esc(query) + '" <button class="sr-af-chip-remove" data-clear="query">&times;</button></span>';
  }
  if (filters.platform && filters.platform !== 'all') {
    html += '<span class="sr-af-chip">Platform: ' + esc(cap(filters.platform)) + ' <button class="sr-af-chip-remove" data-clear="platform">&times;</button></span>';
  }
  if (filters.margin && filters.margin !== 'all') {
    html += '<span class="sr-af-chip">Margin: ' + esc(filters.margin) + '%+ <button class="sr-af-chip-remove" data-clear="margin">&times;</button></span>';
  }
  if (filters.competition && filters.competition !== 'all') {
    html += '<span class="sr-af-chip">Competition: ' + esc(cap(filters.competition)) + ' <button class="sr-af-chip-remove" data-clear="competition">&times;</button></span>';
  }
  if (filters.minScore && filters.minScore > 0) {
    html += '<span class="sr-af-chip">Score: ' + esc(filters.minScore) + '+ <button class="sr-af-chip-remove" data-clear="score">&times;</button></span>';
  }
  ...
  mount(_ctx) {
    ...
    c.push(
      ...
      if (_visibleCount < _allResults.length) {
        html += '<div class="pg-load-more-container" style="grid-column:1/-1;text-align:center;padding:20px 0">'
          ...
          + 'Show More (' + _visibleCount + ' of ' + _allResults.length + ')'
          + '</button></div>';
      }
      ...
      var loadMoreSnapshot = data.results || [];
      var loadMoreSnapshotLen = loadMoreSnapshot.length;
      ...
      if (loadMoreBtn) {
        loadMoreBtn.addEventListener('click', function loadMoreHandler() {
          ...
          var currentResults = _allResults;
          var currentLen = currentResults.length;
          ...
          if (_visibleCount < currentLen) {
            moreHtml += '<div class="pg-load-more-container" style="grid-column:1/-1;text-align:center;padding:20px 0">'
              + '<button id="pgLoadMore" class="pg-load-more-btn" style="padding:12px 32px;border:1px solid rgba(0,255,136,0.3);border-radius:8px;background:rgba(0,255,136,0.08);color:var(--accent-green);font-size:14px;font-weight:600;cursor:pointer;transition:all 0.2s">'
              + 'Show More (' + _visibleCount + ' of ' + currentLen + ')'
              + '</button></div>';
          }
    ...
    c.push(
      EventBus.on('store:selectMode', function() {
        ...
        if (_visibleCount < _allResults.length) {
          html += '<div class="pg-load-more-container" style="grid-column:1/-1;text-align:center;padding:20px 0">'
            + '<button id="pgLoadMore" class="pg-load-more-btn" style="...">Show More (' + _visibleCount + ' of ' + _allResults.length + ')</button></div>';
        }
        grid.innerHTML = html;
        bindCardClicks(grid);
        bindPushButtons(grid);
    ...
    if (viewToggle) {
      viewToggle.addEventListener('click', function(e) {
        ...
        if (!btn) return;
        viewToggle.querySelectorAll('.sr-view-btn').forEach(function(b){ b.classList.remove('active'); });
    ...
    if (!document.getElementById('scBulkBar')) {
      ...
      bar.innerHTML = '<span class="sc-bulk-count">0 selected</span>'
      ...
      document.getElementById('scBulkPushBtn')?.addEventListener('click', async function() {
        ...
        var btn = this;
        var status = document.getElementById('scBulkStatus')?.value || 'active';
        btn.disabled = true;
        btn.textContent = 'Pushing...';
        try {
          var result = await window.HuntDrop.StoreConnect.pushSelected(window.HuntDrop.ALL_PRODUCTS || _allResults, status);
          btn.textContent = result.success ? '✅ Done!' : '❌ Failed';
          UI.toast && UI.toast(result.success ? result.created + ' products pushed to Trendaryo' : 'Push failed: ' + (result.error || 'unknown'), result.success ? 'success' : 'error');
        } catch(e) {
          btn.textContent = '❌ Error';
        }
        setTimeout(function(){ btn.disabled = false; btn.innerHTML = '<svg width="16" height="16" ...><path d="M3 9l9-7 9 7v11a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2z"/><polyline points="9 22 9 12 15 12 15 22"/></svg> Push to Trendaryo'; }, 3000);
      });
