  ...
  return `
    <div class="section-inner">
        ...
        <div class="ph-prompt-box">
          ...
          <div class="ph-prompt-footer">
            <div class="ph-prompt-left">
              ...
              ${PLATFORMS.slice(0,4).map(p => `<span class="ph-platform-toggle" data-plat="${p.id}"><span class="ph-pt-dot" style="background:${p.color}"></span>${p.name}</span>`).join('')}
      ...
      ${window.HuntDrop.renderRelatedTools([
        ...
        {section:'section-lifecycle',name:'Product Lifecycle Radar',desc:'Track maturity',icon:'📊',color:'#4ECDC4'},
        {section:'section-market-gaps',name:'Market Gap Finder',desc:'Find gaps',icon:'🔍',color:'#45B7D1'},
        {section:'section-battlefield',name:'Competitor Battlefield',desc:'See competition',icon:'⚔️',color:'#96CEB4'}
      ])}
    </div>`;
}
...
function showSupplierChat(product) {
  ...
  if (suppliers.length) {
    addChatMsg('ai', `<strong>Top Suppliers for ${esc(product.title.split('—')[0].trim())}:</strong><br><br>` +
      suppliers.map(s => `🏭 <strong>${esc(s.name)}</strong><br>` +
        `📍 ${esc(s.location)} | ⭐ ${s.rating}★ | 📦 ${s.orders} orders<br>` +
        `⏱ Response: ${esc(s.responseTime)} ${s.verified ? '| ✅ Verified' : ''}`).join('<br><br>'));
  } else {
...
function generateAdCopy(product) {
  ...
  addChatMsg('ai', `<strong>🎬 Ad Hooks for ${esc(title)}:</strong><br><br>` +
...
function sendChatMessage(text) {
  ...
  setTimeout(() => {
    if (lower.includes('supplier') || lower.includes('source')) {
      ...
      if (safe.length) {
        addChatMsg('ai', `Found ${safe.length} alternatives with better scores:<br>` +
          safe.slice(0,3).map(p => `• <strong>${esc(p.title)}</strong> (Score: ${p.score}, ${p.margin}% margin)`).join('<br>'));
        renderProducts(safe);
      } else {
      ...
      const top = _searchResults.filter(p => !p.killZone).sort((a,b) => b.margin - a.margin).slice(0,3);
      if (top.length) {
        addChatMsg('ai', `Top profit products:<br>` +
          top.map(p => `• <strong>${esc(p.title)}</strong><br>  Buy: $${p.price.toFixed(2)} → Sell: $${(p.platformPrices?.amazon || p.price*3).toFixed(2)}<br>  Profit: $${((p.platformPrices?.amazon || p.price*3) - p.price - 2.50).toFixed(2)} (${p.margin}%)`).join('<br><br>'));
      }
    } else {
      addChatMsg('ai', `I can help you with:<br>• <strong>Suppliers</strong> — "Show best suppliers"<br>• <strong>Ad Copy</strong> — "Generate TikTok ads"<br>• <strong>Alternatives</strong> — "Find similar products"<br>• <strong>Profit</strong> — "What are the margins?"<br><br>Or click any product card for full analysis!`);
    }
  }, 600);
}
...
function enrichProduct(p) {
  ...
  Object.entries(prices).forEach(([plat, price]) => {
    if (price < bestBuyPrice) { bestBuyPrice = price; bestBuyPlatform = plat; }
  });
  Object.entries(prices).forEach(([plat, price]) => {
    if (price > bestSellPrice && plat !== bestBuyPlatform) { bestSellPrice = price; bestSellPlatform = plat; }
  });

  const arbitrageProfit = bestSellPrice - bestBuyPrice - 2.50;
  ...
  return {
    ...p,
    competitorCount, adCount, tiktokCreators, trendSlope, trendDirection,
    killZone, killReasons, signals, verdict, confidence,
    arbitrage: { buyPlatform: bestBuyPlatform, buyPrice: bestBuyPrice, sellPlatform: bestSellPlatform, sellPrice: bestSellPrice, profit: arbitrageProfit, roi: arbitrageROI }
  };
}
...
function renderValidation() {
  ...
  const verdictReason = verdict === 'test'
...
function renderKillZone() {
  ...
  if (killProducts.length === 0) { el.classList.remove('active'); el.innerHTML = ''; return; }
...
function renderStats() {
  const el = UI.$('phStats');
  if (!el || !_searchResults.length) { el.innerHTML = ''; el.style.display = 'none'; return; }
  el.style.display = '';
...
function renderFilters() {
  ...
  if (!el) return;
  if (!_searchResults.length) { el.style.display = 'none'; return; }
  el.style.display = '';
  const filters = ['all', 'safe', 'killzone', 'rising', 'high-margin'];
  const labels = { all:'All Products', safe:'Safe to Test', killzone:'Kill Zone', rising:'Trending Up', 'high-margin':'High Margin' };
  el.innerHTML = `
...
function exportCSV() {
  ...
  const headers = ['Product','Platform','Score','Margin','Price','Competitors','Ads','TikTok Creators','Trend','Verdict','Buy Price','Sell Price','Profit','ROI'];
  const rows = items.map(p => [
    p.title.split('—')[0], p.platform, p.score, p.margin+'%', '$'+p.price.toFixed(2),
    p.competitorCount, p.adCount, p.tiktokCreators, p.trendDirection, p.verdict,
    '$'+(p.arbitrage?.buyPrice||p.price).toFixed(2), '$'+(p.arbitrage?.sellPrice||p.price*3).toFixed(2),
    '$'+(p.arbitrage?.profit||0).toFixed(2), (p.arbitrage?.roi||0)+'%'
  ]);
...
function exportJSON() {
  const items = getFilteredProducts().map(p => ({
    title: p.title.split('—')[0], platform: p.platform, score: p.score, margin: p.margin,
    price: p.price, competitors: p.competitorCount, trend: p.trendDirection, verdict: p.verdict,
    profit: p.arbitrage?.profit, roi: p.arbitrage?.roi, buyFrom: p.arbitrage?.buyPlatform, sellOn: p.arbitrage?.sellPlatform
  }));
...
function renderWatchlist() {
  ...
  if (!el) return;
  if (!_watchlist.length) { el.classList.remove('active'); el.innerHTML = ''; return; }
  el.classList.add('active');
  el.innerHTML = `
...
function addToWatchlist(product) {
  ...
  _watchlist.push({id:product.id, title:product.title, image:product.image, platform:product.platform, price:product.price, score:product.score, margin:product.margin});
...
function renderProducts(override) {
  ...
  if (!products.length) { grid.innerHTML = '<div style="text-align:center;padding:40px;color:var(--text-muted)">No products found. Try a different search.</div>'; return; }
...
async function startHunt(query) {
  ...
  addChatMsg('ai', `✅ Found <strong>${_searchResults.length} products</strong> across 10 platforms.<br><br>` +
...
function analyzeUrl(url) {
  if (!url?.trim()) {
    addChatMsg('ai', 'Please paste a product URL first. I support AliExpress, Amazon, CJ Dropshipping, eBay, Temu, TikTok Shop, Etsy, DHgate, and Wish.');
    return;
  }

  const detected = detectPlatform(url);
  if (detected) {
    addChatMsg('ai', `🔗 <strong>${detected.name} link detected!</strong><br>Analyzing product and finding it across all platforms...`);
  } else {
...
function mountChatSidebar() {
  ...
  if (clearBtn) {
    clearBtn.addEventListener('click', () => {
      const msgs = UI.$('phChatMessages');
      if (msgs) msgs.innerHTML = '<div class="ph-chat-msg ai">Chat cleared. Ask me anything about your products!</div>';
      _chatHistory = [];
    });
  }
  ...
  if (quickWrap) {
    quickWrap.addEventListener('click', e => {
      ...
      if (btn && chatInput) { chatInput.value = btn.dataset.q; sendChatMessage(btn.dataset.q); }
    });
  }
}
      ...
      urlInput.addEventListener('input', () => {
        ...
        if (urlDetected) {
          if (/aliexpress\.com/i.test(val)) {
            ...
            urlDetected.innerHTML = '<span class="ph-detected-badge web">URL detected — will search across all platforms</span>';
            urlDetected.style.display = 'flex';
          } else {
