...
const STAGES = {
  ...
  Peak:     {color:'var(--accent-yellow)', hex:'#fbbf24', emoji:'🟡', action:'Maximize profit — prepare to transition'},
  Declining:{color:'var(--accent-red)', hex:'#ff3366', emoji:'🔴', action:'Exit or pivot — find rising replacement'}
};
...
function analyzeProduct(p){
  ...
  const bestBuy=platforms.reduce((a,b)=>b.price<a.price?b:a,platforms[0]||{price:p.price,name:p.platform});
  const bestSell=platforms.reduce((a,b)=>b.price>a.price?b:a,platforms[0]||{price:p.price*3,name:'amazon'});
  const profit=(bestSell.price-bestBuy.price-2.50).toFixed(2);
  const roi=bestBuy.price>0?((profit/bestBuy.price)*100).toFixed(0):0;

  return {
    product:p, stage, confidence, daysToNext, reason, trend:t,
    avgGrowth: avgGrowth.toFixed(1),
    platforms, bestBuy, bestSell, profit, roi
  };
}
...
function buildHTML(){
  return `
    ...
    ${window.HuntDrop.renderRelatedTools([
      {section:'section-product-hunt',name:'Product Hunt Scout',desc:'Find new products',icon:'🔥',color:'#ff8a00'},
      {section:'section-market-gaps',name:'Market Gap Finder',desc:'Spot opportunities',icon:'🎯',color:'#a855f7'},
      {section:'section-time-machine',name:'Profit Time Machine',desc:'Forecast by stage',icon:'🔮',color:'#00e5ff'},
      {section:'section-bundles',name:'Bundle Intelligence',desc:'Bundle by lifecycle',icon:'📦',color:'#00ff88'}
    ])}`;
}
...
function renderSummary(){
  ...
  el.innerHTML=Object.entries(counts).map(([stage,count])=>`
...
function renderActions(){
  ...
  if(!el) return;
  el.innerHTML=Object.entries(ACTIONS).map(([stage,items])=>`
...
function renderExport(){
  ...
  if(allBtn) allBtn.addEventListener('click',()=>{_activeFilter='all';renderFilters();renderGrid();renderExport()});
}
    ...
    return `
      <div class="lc-card lc-card-${item.stage.toLowerCase()}" data-id="${p.id}">
        ...
        <div class="lc-card-audience">
          ...
          ${(p.audience?.interests||[]).slice(0,3).map(i=>`<span class="lc-aud-chip">🏷 ${i}</span>`).join('')}
        </div>
...
function renderSparklines(items){
  ...
  items.forEach(item=>{
    ...
    canvas._chart=new Chart(canvas,{
      type:'line',
      data:{labels:MONTHS,datasets:[{data:item.trend,borderColor:c,backgroundColor:grad,borderWidth:2,fill:true,tension:0.4,pointRadius:0}]},
...
function exportCSV(){
  const items=getFiltered();
  const headers=['Product','Platform','Stage','Confidence','Days to Next','Score','Margin','Sales/mo','Saturation','Competition','Best Buy Price','Best Sell Price','Profit','ROI'];
