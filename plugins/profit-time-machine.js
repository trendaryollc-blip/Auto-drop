...
(function () {
  ...
  function calculateConfidence(product) {
    let score = 50;
    if (product.score >= 80) score += 15;
    else if (product.score >= 60) score += 8;
    if (product.competition === 'low') score += 12;
    else if (product.competition === 'medium') score += 5;
    else score -= 10;
    if (product.margin >= 60) score += 10;
    else if (product.margin >= 40) score += 5;
    const trend = product.trendData;
    const last3 = (trend[9] + trend[10] + trend[11]) / 3;
    const first3 = (trend[0] + trend[1] + trend[2]) / 3;
    if (last3 > first3 * 1.2) score += 8;
    else if (last3 > first3) score += 4;
    else score -= 5;
    if (product.riskScore < 30) score += 5;
    else if (product.riskScore > 60) score -= 5;
  ...
  function getTimingRecommendation(confidence, trendData, seasonality) {
    ...
    if (confidence >= 80 && momentum > 0 && nextSeason >= currentSeason)
      return {
        ...
        icon: '🚀',
        urgency: 'high',
        reason: `Momentum is strong (+${Math.round(momentum)}%), seasonality rising, confidence ${confidence}%. Every day you wait is lost profit.`,
      };
    if (confidence >= 65 && momentum >= 0)
      return {
        ...
        icon: '✅',
        urgency: 'medium',
        reason: `Good confidence (${confidence}%) with stable trend. Start with a small test budget ($10-15/day) to validate before scaling.`,
      };
    if (confidence >= 50 && momentum < 0)
      return {
        ...
        icon: '⏳',
        urgency: 'low',
        reason: `Trend declining slightly. Wait 2 weeks to see if it stabilizes. Monitor competitor count and ad frequency.`,
      };
    return {
      action: 'WATCH',
      ...
      icon: '👀',
      urgency: 'none',
      reason: `Low confidence (${confidence}%) or declining trend. Add to watchlist. Look for products with 70+ score and upward momentum.`,
    };
  }
  ...
  function getHistory() {
    try {
      return JSON.parse(localStorage.getItem('ptm_history') || '[]');
    } catch {
      return [];
    }
  }
  ...
  function renderEmptyState() {
    ...
    chips.innerHTML = trending
      .map(
        (p) =>
          `<button class="ptm-chip" data-q="${esc(p.title.split('—')[0].trim())}"><img src="${esc(p.image)}" class="ptm-chip-img" alt=""><span class="ptm-chip-name">${esc(p.title.split('—')[0].trim())}</span><span class="ptm-chip-score">${p.score}</span></button>`
      )
      .join('');
        ...
        ${
          ...
          <div class="ptm-history-list">
            ${history
              ...
              .map(
                (h) =>
                  `<div class="ptm-history-item" data-q="${h.title}"><span class="ptm-history-name">${h.title}</span><span class="ptm-history-timing" style="color:${h.timingColor}">${h.timing}</span></div>`
              )
              .join('')}
          </div>
        </div>`
  ...
  function analyze(query) {
    ...
    const _profitDelta =
      currentMonthlyProfit > 0
        ? (
            (proj30profit / currentMonthlyProfit - ((proj30sales * profitPerSale) / currentMonthlyProfit / 30) * 30) *
              100 || 0
          ).toFixed(0)
        : 0;
  ...
  function bindResultEvents(product, projected, labels, sellPrice, cost, shipping, adSpend, _monthlySales) {
    ...
    if (csvBtn)
      csvBtn.addEventListener('click', () => exportCSV(product, projected, labels, sellPrice, cost, shipping, adSpend));
    ...
    el.innerHTML = `
      <div class="ptm-compare">
        ...
        <div class="ptm-compare-grid">
          ${products
            .map((p) => {
              const profit = p.platformPrices.amazon - p.price - 2.5 - (p.adSpendAvg || p.platformPrices.amazon * 0.15);
  ...
  function renderCharts(product, projected, labels, sellPrice, cost, shipping, adSpend, monthlySales, saturationRate) {
    setTimeout(() => {
      ...
      _chart = new Chart(canvas, {
        type: 'line',
        data: {
          labels: fullLabels.filter((_, i) => i % 6 === 0),
          datasets: [
            {
              label: 'Profit',
              data: profitData.filter((_, i) => i % 6 === 0),
              borderColor: '#00ff88',
            ...
            {
              label: 'Break-even',
              data: breakEven.filter((_, i) => i % 6 === 0),
        ...
        options: {
          ...
          scales: {
            x: {
              ...
              ticks: { color: '#555570', font: { family: 'JetBrains Mono', size: 9 }, maxTicksLimit: 12 },
            },
            y: {
              grid: { color: 'rgba(255,255,255,0.025)' },
  ...
  const ProfitTimeMachinePlugin = {
    ...
    init(_ctx) {
      Config.defaults('profitTimeMachine', { enabled: true });
    },
