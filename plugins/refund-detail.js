...
function renderDetail(refund){
  ...
  const related = allRefunds.filter(r=> r.id !== refund.id && (r.reason === refund.reason || r.supplier === refund.supplier)).slice(0,3);
  ...
  const timeline = [
    { step:'Order Placed', date:refund.date, icon:'🛒', active:true, desc:`Order ${refund.orderId} placed on ${refund.platform}` },
...
}catch(e){ console.error('[RefundDetail] error:', e); }
})();
