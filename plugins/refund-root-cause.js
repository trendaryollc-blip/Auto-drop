...
const REFUND_REASONS = [
  { id:'quality', name:'Product Quality', icon:'🔍', desc:'Item not as described, poor materials, defective', severity:'high', prevention:'Improve product selection, order samples before listing', tips:['Order samples before listing any product','Compare supplier photos with actual items','Read supplier reviews for quality patterns','Set up quality checkpoints at receiving','Create a quality scoring rubric for each supplier'] },
  { id:'damage', name:'Shipping Damage', icon:'📦', desc:'Item arrived broken or damaged in transit', severity:'high', prevention:'Use better packaging, switch to tracked/insured shipping', tips:['Upgrade to double-walled packaging','Add bubble wrap for fragile items','Switch to tracked shipping methods','Insure high-value shipments','Test packaging with drop tests'] },
  { id:'wrong_item', name:'Wrong Item Sent', icon:'❌', desc:'Customer received incorrect product or variant', severity:'medium', prevention:'Verify SKU matching with supplier, improve order accuracy', tips:['Implement SKU verification with suppliers','Create variant mapping documents','Use barcode scanning for fulfillment','Double-check orders before shipping','Add visual confirmation step'] },
  { id:'not_as_described', name:'Not As Described', icon:'📸', desc:'Product differs from listing photos/description', severity:'high', prevention:'Use accurate photos, honest descriptions, manage expectations', tips:['Use only actual product photos','Write honest, detailed descriptions','Include size charts and measurements','Add disclaimers for color variations','Show product from multiple angles'] },
  { id:'late_delivery', name:'Late Delivery', icon:'⏰', desc:'Package arrived after expected delivery date', severity:'medium', prevention:'Set realistic delivery estimates, use faster shipping methods', tips:['Set delivery windows instead of exact dates','Offer expedited shipping options','Use local warehouses when possible','Communicate proactively about delays','Track and alert on delayed shipments'] },
  { id:'buyer_remorse', name:'Buyer Remorse', icon:'🤷', desc:'Customer changed mind, no longer wants product', severity:'low', prevention:'Improve product-market fit, better listing targeting', tips:['Target ads more precisely','Use retargeting for warm audiences','Improve product descriptions','Add social proof and reviews','Implement post-purchase follow-up'] },
  { id:'fraud', name:'Suspected Fraud', icon:'🚨', desc:'Suspicious order pattern or chargeback', severity:'critical', prevention:'Implement fraud checks, verify high-value orders', tips:['Implement address verification','Use fraud detection tools','Verify high-value orders manually','Set up chargeback alerts','Monitor velocity of orders per IP'] },
  { id:'duplicate', name:'Duplicate Order', icon:'🔄', desc:'Customer accidentally ordered twice', severity:'low', prevention:'Add order confirmation step, clear cart UX', tips:['Add order confirmation dialog','Implement duplicate detection','Clear cart after purchase','Send order confirmation emails','Add quantity limit warnings'] }
];
...
function getActionPlan(){ try{ return JSON.parse(localStorage.getItem('rrc_action_plan')) || {}; }catch(e){ return {}; } }
function saveActionPlan(plan){ try{ localStorage.setItem('rrc_action_plan', JSON.stringify(plan)); }catch(e){} }
...
function buildFullPage(reason, refunds){
  ...
  const impactCards = [
    { icon:'💰', label:'Total Lost to This Reason', value:'-$'+totalLost.toFixed(2), color:'var(--accent-red)', sub:reasonCount+' refund'+(reasonCount!==1?'s':'') },
    { icon:'📊', label:'Avg Cost Per Refund', value:'$'+avgCost.toFixed(2), color:'var(--accent-orange)', sub:'Product + shipping + ads' },
    { icon:'📈', label:'% of All Refunds', value:reasonPct+'%', color:reasonPct>30?'var(--accent-red)':reasonPct>15?'var(--accent-yellow)':'var(--accent-green)', sub:reasonCount+' of '+totalRefunds+' total' },
    { icon:'💸', label:'Revenue Refunded', value:'-$'+totalRevenue.toFixed(2), color:'var(--accent-red)', sub:'Gross amount returned' }
  ];

  const severityLabels = { critical:'Critical — Immediate Action Required', high:'High — Needs Urgent Attention', medium:'Medium — Monitor Closely', low:'Low — Minor Issue' };
  const severityDescs = {
    critical:'This is a severe issue that can lead to chargebacks, payment processor penalties, and store bans. It must be addressed immediately to protect your business.',
    ...
    medium:'This issue is creating unnecessary friction and costs. While not urgent, addressing it will improve margins and customer satisfaction.',
          ...
          ${sortedSuppliers.map(s => {
            ...
            return `
              <div class="rrc-sup-card">
                ...
                <div class="rrc-sup-platforms">${Object.entries(s.platforms).map(([p,c])=>`<span class="rrc-platform-tag">${p} (${c})</span>`).join('')}</div>
              </div>`;
          }).join('')}
        </div>
...
function getTipExplanation(reasonId, tipIndex){
  ...
  return (explanations[reasonId] && explanations[reasonId][tipIndex]) || 'Implementing this tip will help reduce ' + (reasonId || 'this') + ' refunds over time.';
}
    ...
    _section.querySelectorAll('.rrc-action-cb').forEach(cb => {
      cb.addEventListener('change', function(){
        ...
        if(completed === total && total > 0){
          UI.toast && UI.toast('🎉 Action plan complete! Great work preventing ' + (REFUND_REASONS.find(r=>r.id===_reasonId)?.name || 'refunds') + '.', 'success');
        }
      });
    });
  },
...
}catch(e){ console.error('[RefundRootCause] error:', e); }
})();
