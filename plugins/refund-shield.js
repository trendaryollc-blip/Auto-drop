...
const REFUND_REASONS = [
  { id:'quality', name:'Product Quality', icon:'🔍', desc:'Item not as described, poor materials, defective', severity:'high', prevention:'Improve product selection, order samples before listing', tips:['Order samples before listing any product','Compare supplier photos with actual items','Read supplier reviews for quality patterns','Set up quality checkpoints at receiving'] },
  { id:'damage', name:'Shipping Damage', icon:'📦', desc:'Item arrived broken or damaged in transit', severity:'high', prevention:'Use better packaging, switch to tracked/insured shipping', tips:['Upgrade to double-walled packaging','Add bubble wrap for fragile items','Switch to tracked shipping methods','Insure high-value shipments'] },
  { id:'wrong_item', name:'Wrong Item Sent', icon:'❌', desc:'Customer received incorrect product or variant', severity:'medium', prevention:'Verify SKU matching with supplier, improve order accuracy', tips:['Implement SKU verification with suppliers','Create variant mapping documents','Use barcode scanning for fulfillment','Double-check orders before shipping'] },
  { id:'not_as_described', name:'Not As Described', icon:'📸', desc:'Product differs from listing photos/description', severity:'high', prevention:'Use accurate photos, honest descriptions, manage expectations', tips:['Use only actual product photos','Write honest, detailed descriptions','Include size charts and measurements','Add disclaimers for color variations'] },
  { id:'late_delivery', name:'Late Delivery', icon:'⏰', desc:'Package arrived after expected delivery date', severity:'medium', prevention:'Set realistic delivery estimates, use faster shipping methods', tips:['Set delivery windows instead of exact dates','Offer expedited shipping options','Use local warehouses when possible','Communicate proactively about delays'] },
  { id:'buyer_remorse', name:'Buyer Remorse', icon:'🤷', desc:'Customer changed mind, no longer wants product', severity:'low', prevention:'Improve product-market fit, better listing targeting', tips:['Target ads more precisely','Use retargeting for warm audiences','Improve product descriptions','Add social proof and reviews'] },
  { id:'fraud', name:'Suspected Fraud', icon:'🚨', desc:'Suspicious order pattern or chargeback', severity:'critical', prevention:'Implement fraud checks, verify high-value orders', tips:['Implement address verification','Use fraud detection tools','Verify high-value orders manually','Set up chargeback alerts'] },
  { id:'duplicate', name:'Duplicate Order', icon:'🔄', desc:'Customer accidentally ordered twice', severity:'low', prevention:'Add order confirmation step, clear cart UX', tips:['Add order confirmation dialog','Implement duplicate detection','Clear cart after purchase','Send order confirmation emails'] }
];

const BENCHMARKS = { ecommerce:3.0, dropshipping:5.0, amazon_fba:1.5, shopify:4.0 };

const SAMPLE_REFUNDS = [
  { id:'REF-001', orderId:'ORD-008', product:'Bluetooth Speaker Mini', customer:'Tom H.', date:'2026-07-14', amount:22.99, cost:7.50, shippingCost:3.50, adCost:4.00, reason:'damage', status:'completed', supplier:'AliExpress Supplier A', platform:'Shopify', notes:'Speaker arrived with cracked casing — shipping damage' },
  { id:'REF-002', orderId:'ORD-012', product:'LED Strip Lights 5m', customer:'Mike R.', date:'2026-07-13', amount:18.99, cost:6.50, shippingCost:2.80, adCost:3.00, reason:'quality', status:'completed', supplier:'CJ Dropshipping', platform:'Shopify', notes:'LEDs only half lit — product defect' },
  { id:'REF-003', orderId:'ORD-015', product:'Phone Case Set', customer:'Anna B.', date:'2026-07-12', amount:12.99, cost:3.20, shippingCost:1.50, adCost:2.50, reason:'wrong_item', status:'pending', supplier:'AliExpress Supplier B', platform:'Amazon', notes:'Ordered black, received blue — SKU mismatch' },
  { id:'REF-004', orderId:'ORD-018', product:'Smart Watch Band', customer:'Chris P.', date:'2026-07-11', amount:15.99, cost:4.50, shippingCost:2.00, adCost:2.80, reason:'not_as_described', status:'completed', supplier:'AliExpress Supplier A', platform:'Shopify', notes:'Buckle quality much lower than photos showed' },
  { id:'REF-005', orderId:'ORD-020', product:'Yoga Mat Premium', customer:'Dana W.', date:'2026-07-10', amount:24.99, cost:8.50, shippingCost:5.00, adCost:4.50, reason:'late_delivery', status:'completed', supplier:'CJ Dropshipping', platform:'TikTok Shop', notes:'Took 28 days — customer needed it sooner' },
  { id:'REF-006', orderId:'ORD-022', product:'Portable Charger 20000mAh', customer:'Sam L.', date:'2026-07-09', amount:34.99, cost:11.00, shippingCost:4.00, adCost:5.50, reason:'fraud', status:'disputed', supplier:'AliExpress Supplier C', platform:'Shopify', notes:'Chargeback filed — customer claims unauthorized purchase' }
];

function getStoredRefunds(){
  try{ return JSON.parse(localStorage.getItem('hd_refunds')) || SAMPLE_REFUNDS; }catch(e){ return SAMPLE_REFUNDS; }
}
function saveRefunds(r){ try{ localStorage.setItem('hd_refunds', JSON.stringify(r)); }catch(e){} }

function calcRefundCost(r){ return r.cost + r.shippingCost + r.adCost; }
function calcTrueLoss(r){ return r.amount - calcRefundCost(r); }
...
function getStats(){
  ...
  const byReason = {};
  refunds.forEach(r => { byReason[r.reason] = (byReason[r.reason]||0) + 1; });
  ...
  return { totalRefunds:total, totalLost, avgCostPerRefund, refundRate, shieldScore, topReason:topReason?topReason[0]:null, topSupplier:topSupplier?topSupplier[0]:null, totalRevenue };
}
...
function renderCauses(){
  ...
  if(!el) return;
  const counts = {};
  _refunds.forEach(r => { counts[r.reason] = (counts[r.reason]||0) + 1; });
  const total = _refunds.length || 1;
  el.innerHTML = REFUND_REASONS.map(r => {
    const count = counts[r.id] || 0;
    const pct = Math.round((count / total) * 100);
    const sevColor = r.severity==='critical'?'var(--accent-red)':r.severity==='high'?'var(--accent-orange)':r.severity==='medium'?'var(--accent-yellow)':'var(--accent-green)';
...
function renderSupplierGrid(){
  ...
  el.innerHTML = sorted.map(s=>{
    ...
    const riskLevel = s.totalLost > 30 ? 'critical' : s.totalLost > 15 ? 'high' : s.totalLost > 5 ? 'medium' : 'low';
    const riskColor = riskLevel==='critical'?'var(--accent-red)':riskLevel==='high'?'var(--accent-orange)':riskLevel==='medium'?'var(--accent-yellow)':'var(--accent-green)';
    return `<div class="rs-sup-card" data-supplier="${esc(s.name)}" tabindex="0" role="button" aria-label="View ${s.name} risk profile">
      ...
      <div class="rs-sup-platforms">${Object.entries(s.platforms).map(([p,c])=>`<span class="rs-sup-platform-tag">${p} (${c})</span>`).join('')}</div>
...
function renderRefundList(){
  ...
  el.innerHTML = filtered.map(r=>{
    ...
    const sevColor = reason.severity==='critical'?'var(--accent-red)':reason.severity==='high'?'var(--accent-orange)':reason.severity==='medium'?'var(--accent-yellow)':'var(--accent-green)';
...
function renderTrend(){
  ...
  el.innerHTML = `<div class="rs-trend-chart">
    ${data.map(d=>`<div class="rs-trend-bar-wrap" data-section="section-refund-shield" role="button" tabindex="0" style="cursor:pointer" aria-label="${d.month}: ${d.count} refunds">
...
function renderPlaybook(){
  ...
  const counts = {};
  _refunds.forEach(r => { counts[r.reason] = (counts[r.reason]||0) + 1; });
  const topReasons = Object.entries(counts).sort((a,b)=>b[1]-a[1]).slice(0,3);
  const plays = topReasons.map(([reasonId])=>{
    const r = REFUND_REASONS.find(x=>x.id===reasonId);
    return r ? { id:r.id, icon:r.icon, title:r.name, prevention:r.prevention, severity:r.severity, tips:r.tips, count:counts[reasonId] } : null;
  }).filter(Boolean);
  if(!plays.length) plays.push({ id:'none', icon:'✅', title:'No Major Issues', prevention:'Your refund rate is low. Keep monitoring and maintain quality standards.', severity:'low', tips:[], count:0 });
  el.innerHTML = `<div class="rs-play-grid">${plays.map((p,i)=>{
    const sevColor = p.severity==='critical'?'var(--accent-red)':p.severity==='high'?'var(--accent-orange)':p.severity==='medium'?'var(--accent-yellow)':'var(--accent-green)';
    return `<div class="rs-play-card" data-reason="${p.id}" tabindex="0" role="button">
      ...
      <div class="rs-play-info">
        ...
        ${p.tips.length?`<div class="rs-play-tips">${p.tips.slice(0,2).map(t=>`<span class="rs-play-tip">• ${t}</span>`).join('')}</div>`:''}
      </div>
...
function bindEvents(){
  ...
  UI.$('rsAddBtn')?.addEventListener('click', ()=>{
    ...
    refreshAll();
    ['rsNewOrder','rsNewProduct','rsNewCustomer','rsNewAmount','rsNewCost','rsNewShip','rsNewAd','rsNewSupplier','rsNewNotes'].forEach(id=>{ const el=UI.$(id); if(el) el.value=''; });
    UI.toast && UI.toast('Refund logged successfully','success');
  });

  UI.$('rsExportBtn')?.addEventListener('click', ()=>{
    const csv = ['Refund ID,Order ID,Product,Customer,Date,Amount,Cost,Shipping,Ad Cost,Reason,Status,Supplier,Platform,Notes'];
    _refunds.forEach(r=>{
      csv.push(`"${r.id}","${r.orderId}","${r.product}","${r.customer}","${r.date}",${r.amount},${r.cost},${r.shippingCost},${r.adCost},"${r.reason}","${r.status}","${r.supplier}","${r.platform}","${r.notes||''}"`);
    });
...
PluginRegistry.register('refund-shield', {
  ...
  mount(_ctx){
    ...
    section.innerHTML = `
      <div class="section-inner">
        ...
        ${window.HuntDrop.renderRelatedTools ? window.HuntDrop.renderRelatedTools([
          ...
          {section:'section-order-tracker', name:'Order Tracker', desc:'Track orders before they become refunds', icon:'📦', color:'var(--accent-cyan)'},
          {section:'section-supplier-intel', name:'Supplier Check', desc:'Verify supplier quality and reliability', icon:'🛡', color:'var(--accent-yellow)'},
          {section:'section-store-health', name:'Store Health', desc:'Refund rate affects store health score', icon:'❤', color:'var(--accent-red)'},
          {section:'section-business-sim', name:'Business Simulator', desc:'Model scenarios with real refund rates', icon:'🎯', color:'var(--accent-purple)'},
          {section:'section-cash-flow', name:'Cash Flow', desc:'Refund impact on cash position', icon:'💳', color:'var(--accent-orange)'}
        ]) : ''}
      </div>`;
    ...
    section.addEventListener('click', (e) => {
      ...
      if (!card) return;
      if (e.target.closest('button, a, select, input, .rs-cause-filter-btn, .rs-cause-detail-btn, .rs-sup-view-btn, .rs-ref-detail-btn')) return;
      e.preventDefault();
...
}catch(e){ console.error('[RefundShield] error:', e); }
})();
