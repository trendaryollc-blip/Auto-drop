...
const REFUND_REASONS = [
  { id:'quality', name:'Product Quality', icon:'🔍', severity:'high', prevention:'Improve product selection, order samples before listing' },
  { id:'damage', name:'Shipping Damage', icon:'📦', severity:'high', prevention:'Use better packaging, switch to tracked/insured shipping' },
  { id:'wrong_item', name:'Wrong Item Sent', icon:'❌', severity:'medium', prevention:'Verify SKU matching with supplier, improve order accuracy' },
  { id:'not_as_described', name:'Not As Described', icon:'📸', severity:'high', prevention:'Use accurate photos, honest descriptions, manage expectations' },
  { id:'late_delivery', name:'Late Delivery', icon:'⏰', severity:'medium', prevention:'Set realistic delivery estimates, use faster shipping methods' },
  { id:'buyer_remorse', name:'Buyer Remorse', icon:'🤷', severity:'low', prevention:'Improve product-market fit, better listing targeting' },
  { id:'fraud', name:'Suspected Fraud', icon:'🚨', severity:'critical', prevention:'Implement fraud checks, verify high-value orders' },
  { id:'duplicate', name:'Duplicate Order', icon:'🔄', severity:'low', prevention:'Add order confirmation step, clear cart UX' }
];

const SAMPLE_REFUNDS = [
  { id:'REF-001', orderId:'ORD-008', product:'Bluetooth Speaker Mini', customer:'Tom H.', date:'2026-07-14', amount:22.99, cost:7.50, shippingCost:3.50, adCost:4.00, reason:'damage', status:'completed', supplier:'AliExpress Supplier A', platform:'Shopify', notes:'Speaker arrived with cracked casing' },
  { id:'REF-002', orderId:'ORD-012', product:'LED Strip Lights 5m', customer:'Mike R.', date:'2026-07-13', amount:18.99, cost:6.50, shippingCost:2.80, adCost:3.00, reason:'quality', status:'completed', supplier:'CJ Dropshipping', platform:'Shopify', notes:'LEDs only half lit' },
  { id:'REF-003', orderId:'ORD-015', product:'Phone Case Set', customer:'Anna B.', date:'2026-07-12', amount:12.99, cost:3.20, shippingCost:1.50, adCost:2.50, reason:'wrong_item', status:'pending', supplier:'AliExpress Supplier B', platform:'Amazon', notes:'Ordered black, received blue' },
  { id:'REF-004', orderId:'ORD-018', product:'Smart Watch Band', customer:'Chris P.', date:'2026-07-11', amount:15.99, cost:4.50, shippingCost:2.00, adCost:2.80, reason:'not_as_described', status:'completed', supplier:'AliExpress Supplier A', platform:'Shopify', notes:'Buckle quality lower than photos' },
  { id:'REF-005', orderId:'ORD-020', product:'Yoga Mat Premium', customer:'Dana W.', date:'2026-07-10', amount:24.99, cost:8.50, shippingCost:5.00, adCost:4.50, reason:'late_delivery', status:'completed', supplier:'CJ Dropshipping', platform:'TikTok Shop', notes:'Took 28 days' },
  { id:'REF-006', orderId:'ORD-022', product:'Portable Charger 20000mAh', customer:'Sam L.', date:'2026-07-09', amount:34.99, cost:11.00, shippingCost:4.00, adCost:5.50, reason:'fraud', status:'disputed', supplier:'AliExpress Supplier C', platform:'Shopify', notes:'Chargeback filed' }
];
...
function calcTrueLoss(r){ return r.amount - (r.cost + r.shippingCost + r.adCost); }
...
function getSupplierData(supplierName){
  ...
  const byPlatform = {};
  refunds.forEach(r=>{ byPlatform[r.platform] = (byPlatform[r.platform]||0)+1; });
  ...
  return { name:supplierName, refunds, totalLost, totalAmount, avgLoss, byReason, byPlatform, riskScore, count:refunds.length };
}
...
PluginRegistry.register('refund-supplier-risk', {
  ...
  mount(_ctx){
    ...
    const riskColor = data.riskScore>=80?'var(--accent-red)':data.riskScore>=60?'var(--accent-orange)':data.riskScore>=40?'var(--accent-yellow)':'var(--accent-green)';
    const riskLevel = data.riskScore>=80?'Critical':data.riskScore>=60?'High':data.riskScore>=40?'Medium':'Low';
    ...
    const sortedOthers = Object.values(otherSuppliers).sort((a,b)=>b.lost-a.lost).slice(0,4);
      ...
      <div class="section-inner">
        ...
        <div class="rsr-section">
          ...
          <div class="rsr-reasons-chart">
            ${Object.entries(data.byReason).sort((a,b)=>b[1]-a[1]).map(([rid,count])=>{
              const reason = REFUND_REASONS.find(r=>r.id===rid);
              if(!reason) return '';
        ...
        <div class="rsr-section">
          ...
          <div class="rsr-platforms-grid">
            ${Object.entries(data.byPlatform).sort((a,b)=>b[1]-a[1]).map(([platform,count])=>{
              const pct = Math.round((count/data.count)*100);
              const colors = {Shopify:'#96bf48',Amazon:'#ff9900',eBay:'#e53238','TikTok Shop':'#000',Etsy:'#f1641e'};
        ...
        <div class="rsr-section">
          ...
          <div class="rsr-comparison-grid">
            ...
            ${sortedOthers.map(s=>{
              ...
              const sColor = sRisk>=60?'var(--accent-red)':sRisk>=40?'var(--accent-orange)':'var(--accent-green)';
        ...
        <div class="rsr-section">
          ...
          <div class="rsr-recommendations">
            ...
            ${Object.entries(data.byReason).sort((a,b)=>b[1]-a[1]).slice(0,2).map(([rid])=>{
              const reason = REFUND_REASONS.find(r=>r.id===rid);
              return reason ? `<div class="rsr-rec-card">
        ...
        ${window.HuntDrop.renderRelatedTools ? window.HuntDrop.renderRelatedTools([
          {section:'section-refund-shield', name:'Refund Shield', desc:'Overview of all refund data', icon:'🛡', color:'var(--accent-red)'},
          {section:'section-refund-root-cause', name:'Root Cause Analysis', desc:'Deep-dive into specific issues', icon:'🔍', color:'var(--accent-orange)'},
          {section:'section-order-tracker', name:'Order Tracker', desc:'Track orders from this supplier', icon:'📦', color:'var(--accent-cyan)'},
          {section:'section-supplier-intel', name:'Supplier Intelligence', desc:'Full supplier verification', icon:'🏭', color:'var(--accent-yellow)'},
          {section:'section-profit-lab', name:'Profit Calculator', desc:'Factor supplier costs into margins', icon:'💰', color:'var(--accent-green)'}
        ]) : ''}
      </div>`;
    container.appendChild(section);
    _section = section;
...
}catch(e){ console.error('[RefundSupplierRisk] error:', e); }
})();
