...
const ORIGINS = [
  { id:'cn', name:'China (Shenzhen)', flag:'🇨🇳', zone:1.0, region:'asia', carriers:['ePacket','CJ Packet','AliExpress Standard','Yanwen'] },
  { id:'cn_yw', name:'China (Yiwu)', flag:'🇨🇳', zone:1.0, region:'asia', carriers:['ePacket','CJ Packet','4PX','SunYou'] },
  { id:'us', name:'USA (LA)', flag:'🇺🇸', zone:0.6, region:'na', carriers:['USPS','UPS','FedEx','DHL'] },
  { id:'us_tx', name:'USA (Dallas)', flag:'🇺🇸', zone:0.65, region:'na', carriers:['USPS','UPS','FedEx','OnTrac'] },
  { id:'uk', name:'UK (London)', flag:'🇬🇧', zone:0.75, region:'eu', carriers:['Royal Mail','DPD','Hermes','Evri'] },
  { id:'de', name:'Germany', flag:'🇩🇪', zone:0.78, region:'eu', carriers:['DHL DE','Hermes','GLS','DPD DE'] },
  { id:'th', name:'Thailand', flag:'🇹🇭', zone:0.9, region:'asia', carriers:['Thai Post','Kerry Express','Flash Express'] },
  { id:'vn', name:'Vietnam', flag:'🇻🇳', zone:0.88, region:'asia', carriers:['Vietnam Post','Giao Hang','J&T Express'] },
  { id:'tr', name:'Turkey', flag:'🇹🇷', zone:0.85, region:'eu', carriers:['PTT','Yurtiçi','MNG Kargo'] },
  { id:'in', name:'India', flag:'🇮🇳', zone:0.7, region:'asia', carriers:['India Post','Delhivery','Blue Dart','Ecom Express'] }
];

const DESTINATIONS = [
  { id:'us', name:'United States', flag:'🇺🇸', baseRate:3.50, customsThreshold:800, avgDuty:0 },
  { id:'uk', name:'United Kingdom', flag:'🇬🇧', baseRate:4.20, customsThreshold:135, avgDuty:20 },
  { id:'ca', name:'Canada', flag:'🇨🇦', baseRate:4.00, customsThreshold:40, avgDuty:13 },
  { id:'au', name:'Australia', flag:'🇦🇺', baseRate:5.50, customsThreshold:1000, avgDuty:10 },
  { id:'de', name:'Germany', flag:'🇩🇪', baseRate:3.80, customsThreshold:150, avgDuty:19 },
  { id:'fr', name:'France', flag:'🇫🇷', baseRate:4.10, customsThreshold:150, avgDuty:20 },
  { id:'jp', name:'Japan', flag:'🇯🇵', baseRate:5.00, customsThreshold:130, avgDuty:10 },
  { id:'br', name:'Brazil', flag:'🇧🇷', baseRate:6.50, customsThreshold:50, avgDuty:60 },
  { id:'mx', name:'Mexico', flag:'🇲🇽', baseRate:4.50, customsThreshold:50, avgDuty:16 },
  { id:'in', name:'India', flag:'🇮🇳', baseRate:3.00, customsThreshold:0, avgDuty:18 },
  { id:'ae', name:'UAE', flag:'🇦🇪', baseRate:4.80, customsThreshold:0, avgDuty:5 },
  { id:'za', name:'South Africa', flag:'🇿🇦', baseRate:6.00, customsThreshold:0, avgDuty:20 }
];

const METHODS = [
  { id:'economy', name:'Economy', icon:'📦', speedLabel:'Budget', desc:'Cheapest option, no tracking', daysMult:1.5, costMult:0.55, tracking:false, insurance:false, color:'var(--text-muted)' },
  { id:'epacket', name:'ePacket', icon:'📧', speedLabel:'China-optimized', desc:'Best for CN→US under 2kg', daysMult:0.85, costMult:0.75, tracking:true, insurance:false, color:'var(--accent-cyan)' },
  { id:'cj_packet', name:'CJ Packet', icon:'⚡', speedLabel:'CJ Fast', desc:'20% faster than ePacket', daysMult:0.7, costMult:0.85, tracking:true, insurance:true, color:'var(--accent-green)' },
  { id:'standard', name:'Standard', icon:'🚚', speedLabel:'Balanced', desc:'Good balance of cost & speed', daysMult:1.0, costMult:1.0, tracking:true, insurance:true, color:'var(--accent-yellow)' },
  { id:'express', name:'Express', icon:'✈️', speedLabel:'Fastest', desc:'DHL/FedEx/UPS priority', daysMult:0.5, costMult:1.8, tracking:true, insurance:true, color:'var(--accent-orange)' }
];

const WEIGHT_BRACKETS = [
  { label:'Letter', emoji:'✉️', desc:'<0.1kg', weight:0.08, baseCost:1.50, examples:'Phone cases, screen protectors, jewelry' },
  { label:'Small', emoji:'📪', desc:'0.1–0.3kg', weight:0.2, baseCost:2.50, examples:'Earbuds, keychains, small gadgets' },
  { label:'Medium', emoji:'📦', desc:'0.3–1kg', weight:0.65, baseCost:4.00, examples:'Watches, sunglasses, small electronics' },
  { label:'Standard', emoji:'📬', desc:'1–2kg', weight:1.5, baseCost:6.50, examples:'Speakers, kitchen gadgets, toys' },
  { label:'Heavy', emoji:'📫', desc:'2–5kg', weight:3.5, baseCost:12.00, examples:'Small appliances, fitness gear' },
  { label:'Bulky', emoji:'📪', desc:'5–10kg', weight:7.5, baseCost:22.00, examples:'LED panels, larger electronics' },
  { label:'Oversize', emoji:'📮', desc:'10kg+', weight:12, baseCost:35.00, examples:'Furniture parts, large equipment' }
];
...
function calcShipping(originId, destId, methodId, bracketIdx, customWeight){
  ...
  return { cost, minDays, maxDays, method, origin, dest, weight, effectiveWeight:weight, hasTracking:method.tracking, hasInsurance:method.insurance };
}
...
function updateMethodTable(o,d,br,cw){ if(window._scUpdateMethodTable) window._scUpdateMethodTable(o,d,br,cw); }
function updateRoutes(m,br){ if(window._scUpdateRoutes) window._scUpdateRoutes(m,br); }
...
function bindEvents(s){
  ...
  UI.$('scResetBtn')?.addEventListener('click', function(){
    Config.set('shipcalc', { origin:'cn', destination:'us', method:'standard', bracketIdx:2, customWeight:0, productCost:5, sellPrice:29.99, platformFee:15, customs:0, adCost:3 });
    location.reload();
  });
  ...
  UI.$('scCalcBtn')?.addEventListener('click', function(){
    ...
    Config.set('shipcalc', {
      origin:originId, destination:destId, method, bracketIdx:bracket, customWeight:customW,
      productCost:parseFloat(UI.$('scProdCost').value)||5, sellPrice:parseFloat(UI.$('scSellPrice').value)||29.99,
      platformFee:parseFloat(UI.$('scPlatFee').value)||15, customs:parseFloat(UI.$('scCustoms').value)||0,
    ...
    UI.$('scCostSub').textContent = r.method.icon + ' ' + r.method.name + ' · ' + r.origin.flag + r.origin.name + ' → ' + r.dest.flag + r.dest.name;
    ...
    if(trackInfo){
      ...
      UI.$('scTrackText').textContent = r.hasInsurance ? 'Full insurance coverage included' : 'No insurance — consider upgrading for high-value items';
    }
    updateLanded();
  ...
  function updateLanded(){
    ...
    UI.$('scLMargin').style.color = marginPct >= 30 ? 'var(--accent-green)' : marginPct >= 15 ? 'var(--accent-yellow)' : 'var(--accent-red)';
    const alert = UI.$('scLandedAlert');
    if(alert){
      if(marginPct < 0) alert.innerHTML = '<div class="sc-alert sc-alert-danger">⚠️ Negative margin! You\'re losing money per sale. Reduce costs or increase price.</div>';
      else if(marginPct < 15) alert.innerHTML = '<div class="sc-alert sc-alert-warning">⚠️ Low margin ('+marginPct+'%). Consider <a data-section="section-price-elasticity" class="sc-link" style="cursor:pointer">optimizing your price</a> or finding cheaper suppliers on <a data-section="section-supplier-hub" class="sc-link" style="cursor:pointer">Supplier Hub</a>.</div>';
      else if(marginPct >= 30) alert.innerHTML = '<div class="sc-alert sc-alert-success">✅ Excellent margin! ('+marginPct+'%). Consider reinvesting in <a data-section="section-budget" class="sc-link" style="cursor:pointer">ad budget</a> to scale.</div>';
    ...
    if(chart){
      ...
      chart.innerHTML = '<div class="sc-bar-chart">'+items.map(i=>`<div class="sc-bar-seg" style="width:${Math.max(i.pct,2)}%;background:${i.color}" title="${i.label}: ${i.pct}%"></div>`).join('')+'</div><div class="sc-bar-legend">'+items.map(i=>`<span class="sc-bar-legend-item"><span class="sc-bar-dot" style="background:${i.color}"></span>${i.label} ${i.pct}%</span>`).join('')+'</div>';
    }
  }
...
function renderHeatmap(){
  ...
  METHODS.forEach(m=>{ html += `<div class="sc-hm-cell sc-hm-meth">${m.icon}<br>${m.name}</div>`; });
  html += '</div>';
  regions.forEach(reg=>{
    ...
    METHODS.forEach(m=>{
      ...
      const color = days<=3?'var(--accent-green)':days<=7?'var(--accent-cyan)':days<=14?'var(--accent-yellow)':'var(--accent-orange)';
...
function renderTips(){
  ...
  if(!grid) return;
  const tips = [
    {icon:'📦',title:'Bundle Orders',text:'Combine items from same supplier to cut per-item shipping 40-60%.',link:'section-bundles',linkText:'Find Bundles →'},
    {icon:'🏭',title:'Use Local Warehouses',text:'US/EU warehouses cut delivery from 12-18 days to 2-5 days.',link:'section-supplier-hub',linkText:'Find Suppliers →'},
    {icon:'📧',title:'ePacket for China',text:'Cheapest China→US option for items under 2kg. 7-15 day delivery.',link:'section-supplier-intel',linkText:'Check Carriers →'},
    {icon:'📐',title:'Dim Weight Matters',text:'Light but large items use L×W×H÷5000. Reduce packaging to save.',link:'section-shipping-calc',linkText:'Calculate Below →'},
    {icon:'💰',title:'Factor into Pricing',text:'Shipping is 20-40% of landed cost. Always include in profit math.',link:'section-profit-lab',linkText:'Calculate Profit →'},
    {icon:'⚡',title:'CJ Packet Advantage',text:'20% faster than ePacket with similar pricing. Best for CJ users.',link:'section-supplier-intel',linkText:'Check Suppliers →'},
    {icon:'🌍',title:'Avoid DDP for Small Orders',text:'DDP adds $2-5 per order. For low-value items, let customers handle customs.',link:'section-supplier-hub',linkText:'Find Options →'},
    {icon:'📊',title:'Track Refund Rates',text:'High shipping damage = high refunds. Use insured methods for fragile items.',link:'section-health',linkText:'Check Store Health →'},
    {icon:'🎯',title:'Scale with Budget',text:'Once profitable, reinvest in faster shipping for better reviews.',link:'section-budget',linkText:'Plan Budget →'}
  ];
...
PluginRegistry.register('shipping-calculator', {
  ...
  init(_ctx) {
    Config.defaults('shipcalc', {
      origin:'cn', destination:'us', method:'standard',
      bracketIdx:2, customWeight:0, productCost:5, sellPrice:29.99,
      platformFee:15, customs:0, adCost:3
    });
  },
    ...
    section.innerHTML = `
      <div class="section-inner">
        ...
        ${window.HuntDrop.renderRelatedTools ? window.HuntDrop.renderRelatedTools([
          { section:'section-profit-lab', name:'Profit Calculator', desc:'Calculate exact profit margins with shipping included', icon:'💰', color:'var(--accent-green)' },
          { section:'section-supplier-hub', name:'Supplier Hub', desc:'Find suppliers with best shipping terms', icon:'🏭', color:'var(--accent-cyan)' },
          { section:'section-supplier-intel', name:'Supplier Check', desc:'Verify supplier shipping reliability', icon:'🛡', color:'var(--accent-yellow)' },
          { section:'section-budget', name:'Budget Planner', desc:'Allocate shipping budget across orders', icon:'💳', color:'var(--accent-purple)' },
          { section:'section-bundles', name:'Bundle Ideas', desc:'Bundle items to reduce per-unit shipping', icon:'📦', color:'var(--accent-orange)' },
          { section:'section-simulator', name:'Business Simulator', desc:'Model scenarios with different shipping costs', icon:'🎯', color:'var(--accent-pink)' }
        ]) : ''}
      </div>`;
