...
function getPushHistory(){
  try{ return JSON.parse(localStorage.getItem('sc_push_history')) || []; }catch(e){ return []; }
}
function savePushHistory(h){
  try{ localStorage.setItem('sc_push_history', JSON.stringify(h.slice(0,50))); }catch(e){}
}
function getStats(){
...
function renderHistoryPanel(){
  ...
  return `
    <div class="sc-panel">
      ...
      <div class="sc-panel-body">
        ${recent.length === 0 ? '<div class="sc-empty">No products pushed yet. Go to Search Results and push your first product!</div>' :
          '<div class="sc-history-list">' + recent.map(h => `
  ...
  UI.$('scTestBtn')?.addEventListener('click', async function(){
    ...
    try{
      const resp = await fetch(TRENDARYO_API, {
        ...
        body: JSON.stringify({ products: [{ name: 'Connection Test', price: 0, category: 'test', source: 'huntkit', status: 'draft', huntkitProductId: 'hd-test-' + Date.now() }] })
      });
      const data = await resp.json();
      if(resultEl){
        if(data.success){
          ...
          resultEl.innerHTML = '<div class="sc-test-error">❌ API returned: ' + esc(JSON.stringify(data.error || data)) + '</div>';
        }
      }
    }catch(e){
      if(resultEl) resultEl.innerHTML = '<div class="sc-test-error">❌ Network error: ' + esc(e.message) + '</div>';
    }
    btn.disabled = false;
...
window.HuntDrop.StoreConnect = {
  ...
  getSelectedProducts: function(){ return Array.from(_selectedProducts); },
  clearSelection: function(){ _selectedProducts.clear(); },
...
}catch(e){ console.error('[StoreConnect] error:', e); }
})();
