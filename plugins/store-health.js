  ...
  const StoreHealthPlugin = {
    ...
    analyze() {
      ...
      const metricsList = [
        {
          ...
          icon: '📦',
          desc: 'Quality and diversity of your product catalog',
        },
      ...
      el.innerHTML =
        ...
        '<div class="sh-overall-grade" style="background:' +
        gradeColor +
        ';color:#000">' +
        grade +
        '</div>' +
        '<div>' +
        '<div class="sh-overall-status" style="color:' +
        gradeColor +
        '">' +
        gradeLabel +
        '</div>' +
        '<div class="sh-overall-sub">Your store is performing <strong>' +
        (overall >= 70 ? 'above' : 'below') +
        '</strong> industry average</div>' +
        '</div>' +
        '</div>' +
        '<div class="sh-overall-meta">' +
        '<div class="sh-overall-meta-item"><span class="sh-overall-meta-label">Scanned</span><span class="sh-overall-meta-val">' +
        products.length +
        ' products</span></div>' +
        ...
        '<circle cx="60" cy="60" r="52" fill="none" stroke="url(#shRingGrad)" stroke-width="8" stroke-linecap="round" stroke-dasharray="' +
        overall * 3.27 +
        ' 327" transform="rotate(-90 60 60)" class="sh-ring-progress"/>' +
        '</svg>' +
        '<div class="sh-score-center">' +
        '<div class="sh-score-number" style="color:' +
        gradeColor +
        '">' +
        overall +
        '</div>' +
        ...
        metricsList
          .map(function (m) {
            ...
            return (
              ...
              '<div class="sh-metric-icon" style="background:' +
              color +
              '22;color:' +
              color +
              '">' +
              ...
              m.label +
              '</div>' +
              '<div class="sh-metric-desc">' +
              m.desc +
              '</div>' +
              '<div class="sh-metric-bar-wrap"><div class="sh-metric-bar" style="width:' +
              val +
              '%;background:' +
              color +
              '"></div></div>' +
              ...
              '<div class="sh-metric-score" style="color:' +
              color +
              '">' +
              val +
              '</div>' +
              '<div class="sh-metric-status" style="color:' +
              color +
              '">' +
              status +
              '</div>' +
        ...
        '<div class="sh-tabs">' +
        '<button class="sh-tab-btn active" data-tab="alerts"><span class="sh-tab-icon">⚠️</span> Alerts & Warnings <span class="sh-tab-badge">' +
        alerts.length +
        '</span></button>' +
        '<button class="sh-tab-btn" data-tab="actions"><span class="sh-tab-icon">🎯</span> Action Plan <span class="sh-tab-badge">' +
        actions.length +
        '</span></button>' +
              ...
              a.type === 'danger'
                ...
                : a.type === 'warning'
                  ...
                  : a.type === 'success'
                    ...
                    : 'Info';
            return (
              '<div class="sh-alert sh-alert-' +
              a.type +
              '">' +
              '<div class="sh-alert-icon-wrap">' +
              icon +
              '</div>' +
              '<div class="sh-alert-body">' +
              '<div class="sh-alert-meta">' +
              '<span class="sh-alert-type sh-type-' +
              a.type +
              '">' +
              typeLabel +
              '</span>' +
              '</div>' +
              '<div class="sh-alert-title">' +
              a.title +
              '</div>' +
        ...
        actions
          .map(function (a, i) {
            const clickAttr = a.section
              ? ' onclick="window.HuntDrop.navigateTo(\'' + a.section + '\')" style="cursor:pointer"'
              : '';
            return (
              '<div class="sh-action"' +
              clickAttr +
              '>' +
              '<div class="sh-action-num">' +
              (i + 1) +
              '</div>' +
              ...
              a.title +
              '</div>' +
              '<div class="sh-action-desc">' +
              a.desc +
              '</div>' +
              '</div>' +
              '<div class="sh-action-impact-wrap">' +
              '<div class="sh-action-impact" style="color:' +
              (a.impact === 'High' ? 'var(--accent-green)' : 'var(--accent-orange)') +
              '">' +
              a.impact +
              ' Impact</div>' +
      ...
      section.querySelectorAll('.sh-tab-btn').forEach(function (btn) {
        btn.addEventListener('click', function () {
          section.querySelectorAll('.sh-tab-btn').forEach(function (b) {
            b.classList.remove('active');
          });
          section.querySelectorAll('.sh-tab-panel').forEach(function (p) {
          ...
          if (tab === 'charts')
            setTimeout(function () {
              self.renderCharts(scores, section);
            }, 50);
          if (tab === 'history')
            setTimeout(function () {
              self.renderHistoryChart(scores, section);
            }, 50);
        });
      });
      ...
      section.querySelectorAll('.sh-view-btn').forEach(function (btn) {
        btn.addEventListener('click', function () {
          section.querySelectorAll('.sh-view-btn').forEach(function (b) {
            b.classList.remove('sh-view-active');
          });
      ...
      metricsList.forEach(function (m) {
        ...
        insights.push(
          '<div class="sh-insight-item">' +
            '<span class="sh-insight-icon">' +
            m.icon +
            '</span>' +
            '<div class="sh-insight-content">' +
            '<strong>' +
            m.label +
            '</strong> is <span style="color:' +
            color +
            '">' +
            status +
            '</span> with a score of ' +
            val +
            '/100.' +
            '</div>' +
            '</div>'
    ...
    scoreProductSelection(products) {
      ...
      const trendingCount = products.filter(function (p) {
        return p.badges.indexOf('trending') > -1;
      }).length;
    ...
    scorePricing(products) {
      ...
      const priceRange = products.map(function (p) {
        return p.price;
      });
    ...
    scoreFinancialHealth(products) {
      const avgRisk =
        products.reduce(function (s, p) {
          return s + p.riskScore;
        }, 0) / products.length;
      const riskScore = 100 - avgRisk;
      const avgSaturation =
        products.reduce(function (s, p) {
          return s + p.marketSaturation;
        }, 0) / products.length;
    ...
    generateAlerts(products, scores) {
      ...
      if (scores.pricing < 60) {
        ...
        alerts.push({
          type: 'warning',
          title: 'Pricing Below Optimal',
          text:
            'Your average margin is ' +
            avgMargin.toFixed(0) +
            '%. Top stores average 65-75%. Consider raising prices or sourcing cheaper suppliers.',
        });
      }
      if (scores.supplierRel < 70) {
        alerts.push({
          ...
          text: 'Some suppliers have response times over 3 hours. Late shipments can increase refund rate by 15-25%. Prioritize verified suppliers.',
        });
      }
      if (scores.adStrategy < 60) {
        ...
        alerts.push({
          ...
          text:
            'Average CPA is $' +
            avgCPA.toFixed(2) +
            '. Industry benchmark is $3-5. Optimize ad targeting or test new creatives.',
        });
      }
      const highSatProducts = products.filter(function (p) {
        return p.marketSaturation > 60;
      });
      if (highSatProducts.length > 0) {
        alerts.push({
          type: 'danger',
          title: 'Saturated Products Detected',
          text:
            highSatProducts.length +
            ' product(s) have over 60% market saturation. These are at risk of declining returns. Consider diversifying.',
        });
      }
      const bestProduct = products.reduce(function (best, p) {
        return p.score > best.score ? p : best;
      }, products[0]);
      if (bestProduct && scores.adStrategy < 70) {
        alerts.push({
          type: 'info',
          title: 'Best Product Underperforming in Ads',
          text:
            '"' +
            bestProduct.title.split('—')[0].trim() +
            '" has an AI score of ' +
            bestProduct.score +
            ' but average ad performance. Increase budget allocation for this product.',
        });
      }
      if (alerts.length === 0) {
        alerts.push({
          type: 'success',
          title: 'Store Looking Healthy!',
          text: 'All metrics are within optimal ranges. Keep monitoring and scaling what works.',
        });
      }
      return alerts;
    },
    ...
    generateActions(scores, _alerts) {
      ...
      if (scores.adStrategy < 70)
        actions.push({
          ...
          desc: 'Use the Ad Budget AI Allocator to shift spend toward high-ROI products and cut underperformers.',
          impact: 'High',
          section: 'section-budget',
        });
      if (scores.supplierRel < 80)
        actions.push({
          title: 'Upgrade Supplier Network',
          desc: 'Switch to suppliers with <2h response time and verified status. Check Supplier Hub for alternatives.',
          impact: 'Medium',
          section: 'section-supplier-hub',
        });
      if (scores.productSelection < 70)
        actions.push({
          title: 'Diversify Product Selection',
          desc: 'Add 2-3 products from rising niches. Use Niche Radar to find untapped opportunities.',
          impact: 'High',
          section: 'section-niche-radar',
        });
      if (scores.customerExp < 75)
        actions.push({
          title: 'Improve Social Proof',
          desc: 'Focus on products with 4.5+ ratings. Encourage reviews and use UGC in ad creatives.',
          impact: 'Medium',
          section: 'section-ad-studio',
        });
      if (scores.financial < 65)
        actions.push({
          title: 'Reduce Risk Exposure',
          desc: 'Drop products with risk score >60. Focus on proven winners with stable demand curves.',
          impact: 'High',
      ...
      if (actions.length === 0)
        actions.push({
          ...
          desc: 'Your store is performing well. Focus on scaling winning products and testing new audiences.',
          impact: 'Low',
          section: '',
        });
      return actions;
    ...
    renderCharts(scores, section) {
      ...
      if (ctx) {
        ...
        const labels = Object.keys(scores).map(function (k) {
          return k.replace(/([A-Z])/g, ' $1').replace(/^./, function (s) {
      ...
      if (barCtx) {
        ...
        const blabels = Object.keys(scores).map(function (k) {
          return k.replace(/([A-Z])/g, ' $1').replace(/^./, function (s) {
            return s.toUpperCase();
          });
        });
        const bvalues = Object.values(scores);
        const bcolors = bvalues.map(function (v) {
          return v >= 80 ? '#00ff88' : v >= 60 ? '#ff8a00' : '#ff3366';
        });
    ...
    renderHistoryChart(scores, section) {
      ...
      const current = Math.round(
        Object.values(scores).reduce(function (s, v) {
          return s + v;
        }, 0) / 6
      );
