...
(function () {
  ...
  function calculateProfitPotential(supplier, productName) {
    ...
    return {
      supplierCost: cost, marketPrice, lowestCompetitor,
      profitAtAvg: Math.max(profitAtAvg, 0), profitAtLowest: Math.max(profitAtLowest, 0),
      marginAtAvg: Math.max(marginAtAvg, 0), netProfit: Math.max(netProfit, 0),
  ...
  function getWatchlist() { return _lsGet(LS_WATCHLIST, []); }
  function setWatchlist(wl) { _lsSet(LS_WATCHLIST, wl); }
  function isWatched(id) { return getWatchlist().some(function (w) { return w.supplierId === id; }); }

  function toggleWatch(supplier) {
    const wl = getWatchlist();
    const idx = wl.findIndex(function (w) { return w.supplierId === supplier.name; });
    if (idx >= 0) {
      wl.splice(idx, 1);
      _toast('Removed from watchlist', 'info');
    } else {
      wl.push({
        supplierId: supplier.name, platform: supplier.platform,
        addedAt: Date.now(), lastData: { price: supplier.price, rating: supplier.rating, orders: supplier.orders },
      });
  ...
  var NEGO_STATUSES = { PENDING: 'pending', REPLIED: 'replied', COUNTERED: 'countered', ACCEPTED: 'accepted', REJECTED: 'rejected' };
  var NEGO_STATUS_LABELS = { pending: 'Awaiting Reply', replied: 'Supplier Replied', countered: 'Counter-Offer Sent', accepted: 'Deal Accepted', rejected: 'Declined' };
  var NEGO_STATUS_COLORS = { pending: '#FFD700', replied: 'var(--sh-nebula)', countered: 'var(--sh-cosmic)', accepted: 'var(--sh-aurora)', rejected: 'var(--sh-solar)' };

  function getNegotiations() { return _lsGet(LS_NEGOTIATIONS, []); }
  function setNegotiations(n) { _lsSet(LS_NEGOTIATIONS, n); }

  function getActiveNego(supplierName) {
    return getNegotiations().find(function (n) { return n.supplierId === supplierName; }) || null;
  }
  ...
  function extractPriceOffers(text) {
    ...
    return matches ? matches.map(function (m) { return parseFloat(m.replace(/[$,]/g, '')); }).filter(Boolean) : [];
  }
  ...
  function getBestOffer(nego) {
    ...
    nego.messages.forEach(function (m) {
      if (m.from === 'supplier') {
        ...
        prices.forEach(function (p) { offers.push({ price: p, ts: m.ts }); });
      }
    });
  ...
  function simulateSupplierReply(userText, supplier) {
    ...
    var isBulk = lowerText.includes('bulk') || lowerText.includes('quantity') || lowerText.includes('100') || lowerText.includes('250');
    var isShipping = lowerText.includes('shipping') || lowerText.includes('delivery');
    var isPrivateLabel = lowerText.includes('private label') || lowerText.includes('brand') || lowerText.includes('logo');
    ...
    if (isCounter) {
      ...
      return 'I appreciate your offer. The best I can do is $' + counterPrice + '/unit. This includes:\n- Quality inspection\n- Individual packaging\n- Tracking number\n\nThis is a special price for committed partners. Shall we proceed?';
    }
  ...
  function generateSupplierDNA(supplier) {
    ...
    return {
      identity: {
        name: supplier.name, platform: supplier.platform, location: supplier.location,
        specialty: supplier.specialty || 'General', yearsActive: supplier.yearsActive || 0,
        verified: supplier.verified, color: supplier.color || '#667eea',
      },
      performance: {
        score: score, grade: grade, risk: risk, velocity: velocity,
        quality: supplier.quality || 0, communication: supplier.communication || 0, value: supplier.value || 0,
        responseRate: supplier.responseRate || 0, fulfillmentRate: supplier.fulfillmentRate || 0,
        disputeRate: supplier.disputeRate || 0, rating: supplier.rating || 0,
        velocityData: supplier.velocityData || {},
      },
      products: {
        topProducts: supplier.topProducts || [], count: (supplier.topProducts || []).length,
        categories: supplier.specialty ? [supplier.specialty] : [],
      },
      shipping: {
        shipTime: supplier.shipTime || 'N/A', shipCost: supplier.shipCost || 'N/A',
        minOrder: supplier.minOrder || 'N/A', paymentTerms: supplier.paymentTerms || 'N/A',
      },
      market: {
        priceTier: (supplier.value || 50) >= 70 ? 'budget' : (supplier.value || 50) >= 40 ? 'mid' : 'premium',
        orders: _parseNum(supplier.orders), yearsActive: supplier.yearsActive || 0,
      },
      risks: {
        verified: supplier.verified, disputeRate: supplier.disputeRate || 0,
        responseRate: supplier.responseRate || 100, refundRate: supplier.refundRate || 0,
      },
      capabilities: {
  ...
  function getCheckedItems() { return _lsGet(LS_CHECKLIST, {}); }
  function setCheckedItem(id, checked) {
    const items = getCheckedItems();
    if (checked) items[id] = true; else delete items[id];
  ...
  function renderCommandCenter(suppliers) {
    ...
    var verified = suppliers.filter(function (s) { return s.verified; }).length;
    var avgRating = (suppliers.reduce(function (a, s) { return a + (s.rating || 0); }, 0) / suppliers.length).toFixed(1);
    var totalOrders = suppliers.reduce(function (a, s) { return a + _parseNum(s.orders); }, 0);
    var avgProducts = Math.round(suppliers.reduce(function (a, s) { return a + (s.topProducts || []).length; }, 0) / suppliers.length);
    var risingStars = suppliers.filter(function (s) { return computeVelocityScore(s) >= 45; }).length;

    return '<div class="sh-command-center">' +
      '<div class="sh-stat-card"><div class="sh-stat-icon" style="background:rgba(79,140,255,0.12);color:var(--sh-nebula)">\uD83C\uDFED</div><div class="sh-stat-info"><div class="sh-stat-value">' + suppliers.length + '</div><div class="sh-stat-label">Suppliers Found</div></div></div>' +
      '<div class="sh-stat-card"><div class="sh-stat-icon" style="background:rgba(0,245,160,0.12);color:var(--sh-aurora)">\u2705</div><div class="sh-stat-info"><div class="sh-stat-value">' + verified + '</div><div class="sh-stat-label">Verified</div></div></div>' +
      '<div class="sh-stat-card"><div class="sh-stat-icon" style="background:rgba(255,215,0,0.12);color:#FFD700">\u2B50</div><div class="sh-stat-info"><div class="sh-stat-value">' + avgRating + '\u2605</div><div class="sh-stat-label">Avg Rating</div></div></div>' +
      '<div class="sh-stat-card"><div class="sh-stat-icon" style="background:rgba(168,85,247,0.12);color:var(--sh-cosmic)">\uD83D\uDCE6</div><div class="sh-stat-info"><div class="sh-stat-value">' + (totalOrders / 1000).toFixed(0) + 'K</div><div class="sh-stat-label">Total Orders</div></div></div>' +
      '<div class="sh-stat-card"><div class="sh-stat-icon" style="background:rgba(0,245,160,0.12);color:var(--sh-aurora)">\uD83D\uDE80</div><div class="sh-stat-info"><div class="sh-stat-value">' + risingStars + '</div><div class="sh-stat-label">Rising Stars</div></div></div>' +
      '</div>';
  }

  // --- Filters ---
  function renderFilters() {
    return '<div class="sh-filters" id="shFilters">' +
  ...
  function bindFilters() {
    ...
    _section.querySelectorAll('.sh-filter-pill').forEach(function (btn) {
      btn.addEventListener('click', function () {
        _section.querySelectorAll('.sh-filter-pill').forEach(function (b) { b.classList.remove('active'); });
  ...
  function renderCards(suppliers) {
    ...
    if (!grid) return;
    if (!suppliers.length) {
      grid.innerHTML = '<div class="sh-empty"><div class="sh-empty-icon">\uD83D\uDD0D</div>Use the search bar above to find suppliers</div>';
      return;
    }
    grid.innerHTML = suppliers.map(function (s, i) {
      try {
        ...
        return '<div class="sh-supplier-card" tabindex="0" role="button" aria-label="View ' + esc(name) + ' details" data-idx="' + i + '" data-verified="' + s.verified + '" data-response="' + esc(s.responseTime || '') + '" data-rating="' + (s.rating || 0) + '" data-platform="' + esc(platform) + '" data-velocity="' + computeVelocityScore(s) + '" data-viable="' + profit.isViable + '">' +
          (vel ? '<div class="sh-velocity-badge ' + vel.cls + '">' + vel.icon + ' ' + vel.text + '</div>' : '') +
          '<div class="sh-card-header">' +
          '<div class="sh-card-avatar" style="background:' + esc(color) + '22;color:' + esc(color) + '">' + esc(name.charAt(0)) + '</div>' +
          '<div><div class="sh-card-name">' + esc(name) + '</div><div class="sh-card-platform">' + esc(platform) + ' \u2022 ' + esc(s.location || '') + '</div></div>' +
          ...
          '<div class="sh-card-stat"><span class="sh-card-stat-val" style="color:#FFD700">' + esc(s.rating || 0) + '\u2605</span><span class="sh-card-stat-lbl">Rating</span></div>' +
          '<div class="sh-card-stat"><span class="sh-card-stat-val">' + esc(s.orders || 0) + '</span><span class="sh-card-stat-lbl">Orders</span></div>' +
          '<div class="sh-card-stat"><span class="sh-card-stat-val">' + esc(s.responseTime || 'N/A') + '</span><span class="sh-card-stat-lbl">Response</span></div>' +
          '<div class="sh-card-stat"><span class="sh-card-stat-val">' + (s.topProducts ? s.topProducts.length : 0) + '</span><span class="sh-card-stat-lbl">Products</span></div>' +
          '</div>' +
          (profit.netProfit > 0 ? '<div class="sh-card-profit"><span class="sh-card-profit-val">' + _formatMoney(profit.netProfit) + '</span><span class="sh-card-profit-margin">' + Math.round(profit.marginAtAvg) + '% margin</span><span class="sh-card-profit-viable ' + (profit.isViable ? 'yes' : 'no') + '">' + (profit.isViable ? 'Viable' : 'Low') + '</span></div>' : '') +
          '<div class="sh-card-score-bar"><div class="sh-card-score-fill" style="width:' + score + '%;background:' + (score >= 90 ? 'var(--sh-aurora)' : score >= 80 ? 'var(--sh-nebula)' : '#FFD700') + '"></div></div>' +
          '<div class="sh-card-score-text">' + score + '/100</div>' +
          ...
          '<button class="sh-card-action sh-card-watch-btn" data-supplier="' + esc(name) + '">' + (watched ? 'Watching' : 'Watch') + '</button>' +
          '<button class="sh-card-action sh-card-detail-btn" data-idx="' + i + '">Details</button>' +
          '</div>' +
          '</div>' +
          '</div>';
      } catch (err) {
    ...
    grid.querySelectorAll('.sh-card-watch-btn').forEach(function (btn) {
      btn.addEventListener('click', function (e) {
        e.stopPropagation();
        var s = _suppliers.find(function (x) { return x.name === btn.dataset.supplier; });
        if (s) toggleWatch(s);
      });
    });
  ...
  function renderComparison(suppliers) {
    ...
    if (!wrap || !suppliers.length) { if (wrap) wrap.innerHTML = ''; return; }
    var sorted = suppliers.slice().sort(function (a, b) { return computeScore(b) - computeScore(a); }).slice(0, 8);
    var bestProfit = 0;
    sorted.forEach(function (s) { var p = calculateProfitPotential(s); if (p.netProfit > bestProfit) bestProfit = p.netProfit; });
    var bestScore = computeScore(sorted[0] || {});

    var html = '<table class="sh-table"><thead><tr>' +
    ...
    sorted.forEach(function (s) {
      ...
      var trendColor = vd.ratingTrend === 'up' ? 'var(--sh-aurora)' : vd.ratingTrend === 'down' ? 'var(--sh-solar)' : '#888';

      html += '<tr class="' + (isWinner ? 'sh-winner' : '') + '" data-name="' + esc(name) + '">' +
        '<td><div class="sh-tbl-name"><div class="sh-tbl-avatar" style="background:' + esc(color) + '22;color:' + esc(color) + '">' + esc(name.charAt(0)) + '</div>' + esc(name) + '</div></td>' +
        '<td>' + esc(s.platform || '') + '</td>' +
        '<td><span class="sh-tbl-grade" style="background:' + esc(grade.color) + '18;color:' + esc(grade.color) + '">' + esc(grade.grade) + '</span></td>' +
        '<td style="color:' + esc(grade.color) + ';font-weight:700">' + score + '</td>' +
        '<td>\u2605 ' + esc(s.rating || 0) + '</td>' +
        '<td>' + esc(s.shipTime || 'N/A') + '</td>' +
        '<td>' + esc(s.responseTime || 'N/A') + '</td>' +
        '<td style="color:' + trendColor + ';font-weight:700">' + trendArrow + ' ' + (vd.salesGrowth30d || 0) + '%</td>' +
        '<td><span class="sh-tbl-risk" style="background:' + esc(risk.color) + '18;color:' + esc(risk.color) + '">' + esc(risk.level) + '</span></td>' +
        '<td class="sh-tbl-profit" style="color:' + (profit.isViable ? 'var(--sh-aurora)' : 'var(--sh-text-dim)') + '">' + _formatMoney(profit.netProfit) + '</td>' +
        '</tr>';
    });

    html += '</tbody></table>';
    html += '<div class="sh-winner-banner"><span class="sh-winner-icon">\uD83C\uDFC6</span><strong>' + esc(sorted[0] ? sorted[0].name : '') + '</strong> is your best match! (' + bestScore + '/100)</div>';
    wrap.innerHTML = html;

    wrap.querySelectorAll('tr[data-name]').forEach(function (row) {
      row.addEventListener('click', function () {
        var s = _suppliers.find(function (x) { return x.name === row.dataset.name; });
        if (s) showDetail(s);
      });
    });
  }
  ...
  function renderScores(suppliers) {
    ...
    if (!grid || !suppliers.length) { if (grid) grid.innerHTML = ''; return; }
    var top6 = suppliers.slice().sort(function (a, b) { return computeScore(b) - computeScore(a); }).slice(0, 6);
    grid.innerHTML = top6.map(function (s) {
      var score = computeScore(s);
      var name = s.name || 'Unknown';
      var color = s.color || '#667eea';
      return '<div class="sh-score-card" data-name="' + esc(name) + '">' +
        '<div class="sh-score-header">' +
        '<div class="sh-score-avatar" style="background:' + esc(color) + '22;color:' + esc(color) + '">' + esc(name.charAt(0)) + '</div>' +
        '<div><div class="sh-score-name">' + esc(name) + '</div><div class="sh-score-platform">' + esc(s.platform || '') + '</div></div>' +
        '<div class="sh-score-total">' + score + '</div></div>' +
        '<div class="sh-score-bars">' +
        '<div class="sh-score-row"><span>Quality</span><div class="sh-bar"><div class="sh-bar-fill" style="width:' + (s.quality || 0) + '%;background:var(--sh-aurora)"></div></div><span>' + (s.quality || 0) + '</span></div>' +
        '<div class="sh-score-row"><span>Communication</span><div class="sh-bar"><div class="sh-bar-fill" style="width:' + (s.communication || 0) + '%;background:var(--sh-nebula)"></div></div><span>' + (s.communication || 0) + '</span></div>' +
        '<div class="sh-score-row"><span>Value</span><div class="sh-bar"><div class="sh-bar-fill" style="width:' + (s.value || 0) + '%;background:var(--sh-cosmic)"></div></div><span>' + (s.value || 0) + '</span></div>' +
        '</div></div>';
    }).join('');

    grid.querySelectorAll('.sh-score-card').forEach(function (card) {
      card.addEventListener('click', function () {
        var s = _suppliers.find(function (x) { return x.name === card.dataset.name; });
        if (s) showDetail(s);
      });
    });
  }
  ...
  function renderShipping(suppliers) {
    ...
    var icons = { AliExpress: '\uD83C\uDF10', Amazon: '\uD83D\uDCE6', 'CJ Dropshipping': '\uD83D\uDE9A', Alibaba: '\uD83C\uDFEA', Temu: '\uD83D\uDCB0', DHgate: '\uD83C\uDFEA', Etsy: '\uD83C\uDFA8' };

    grid.innerHTML = Object.keys(platforms).map(function (name) {
      var p = platforms[name];
      var avgTime = p.times.length ? Math.round(p.times.reduce(function (a, b) { return a + b; }, 0) / p.times.length) : '?';
      var avgCost = p.costs.length ? '$' + (p.costs.reduce(function (a, b) { return a + b; }, 0) / p.costs.length).toFixed(2) : 'Free';
      return '<div class="sh-ship-card" data-platform="' + esc(name) + '">' +
        '<div class="sh-ship-icon">' + (icons[name] || '\uD83C\uDFE2') + '</div>' +
        '<div class="sh-ship-name">' + esc(name) + '</div>' +
        '<div class="sh-ship-stats">' +
        '<div class="sh-ship-stat"><span class="sh-ship-stat-label">Avg Ship</span><span class="sh-ship-stat-value">' + avgTime + ' days</span></div>' +
        '<div class="sh-ship-stat"><span class="sh-ship-stat-label">Avg Cost</span><span class="sh-ship-stat-value">' + avgCost + '</span></div>' +
  ...
  function renderChecklist() {
    ...
    var items = [
      { id: 'verify-license', icon: '\uD83D\uDD0D', title: 'Verify Business License', desc: 'Confirm valid business registration and import/export licenses', priority: 'Critical' },
      { id: 'request-samples', icon: '\uD83D\uDCCB', title: 'Request Product Samples', desc: 'Always order 2-3 samples before bulk orders', priority: 'Critical' },
      { id: 'test-response', icon: '\uD83D\uDCAC', title: 'Test Response Time', desc: 'Send inquiries at different hours to verify', priority: 'High' },
      { id: 'check-orders', icon: '\uD83D\uDCCA', title: 'Check Order History', desc: 'Look for consistent volume over 6+ months', priority: 'High' },
      { id: 'review-return', icon: '\uD83D\uDD04', title: 'Review Return Policy', desc: 'Understand refund terms and dispute process', priority: 'High' },
      { id: 'verify-photos', icon: '\uD83D\uDCF7', title: 'Verify Product Photos', desc: 'Request actual photos, not stock images', priority: 'Medium' },
      { id: 'compare-pricing', icon: '\uD83C\uDFF7\uFE0F', title: 'Compare Unit Pricing', desc: 'Get quotes for different quantities', priority: 'Medium' },
      { id: 'confirm-shipping', icon: '\uD83D\uDE9A', title: 'Confirm Shipping Methods', desc: 'Verify carriers, tracking, and insurance', priority: 'Medium' },
      { id: 'read-reviews', icon: '\uD83D\uDCDD', title: 'Read Sample Reviews', desc: 'Check reviews from other dropshippers', priority: 'Low' },
      { id: 'negotiate-terms', icon: '\uD83E\uDD1D', title: 'Negotiate Terms', desc: 'Discuss payment terms and exclusivity', priority: 'Low' },
    ];
    var done = Object.keys(checked).length;
    var total = items.length;
    var pct = Math.round((done / total) * 100);
    var priColors = { Critical: 'var(--sh-solar)', High: '#FFD700', Medium: 'var(--sh-nebula)', Low: 'var(--sh-text-dim)' };

    el.innerHTML = '<div class="sh-check-progress"><div class="sh-check-progress-bar"><div class="sh-check-progress-fill" id="shCheckProgress" style="width:' + pct + '%"></div></div><span class="sh-check-progress-label">Verification</span><span class="sh-check-progress-count" id="shCheckCount">' + done + '/' + total + '</span></div>' +
      items.map(function (i) {
        return '<div class="sh-check-item' + (checked[i.id] ? ' done' : '') + '" data-id="' + i.id + '">' +
          '<div class="sh-check-box">' + (checked[i.id] ? '\u2713' : '') + '</div>' +
          '<div class="sh-check-icon">' + i.icon + '</div>' +
          '<div class="sh-check-info"><div class="sh-check-title">' + i.title + '</div><div class="sh-check-desc">' + i.desc + '</div></div>' +
          '<div class="sh-check-priority" style="color:' + priColors[i.priority] + '">' + i.priority + '</div></div>';
      }).join('');
  ...
  function renderPicks(suppliers) {
    ...
    var bestValue = suppliers.slice().sort(function (a, b) { return (b.value || 0) - (a.value || 0); })[0];
    var topRated = suppliers.slice().sort(function (a, b) { return (b.rating || 0) - (a.rating || 0); })[0];
    var mostOrders = suppliers.slice().sort(function (a, b) { return _parseNum(b.orders) - _parseNum(a.orders); })[0];
    var lowestRisk = suppliers.slice().sort(function (a, b) { return (a.disputeRate || 5) - (b.disputeRate || 5); })[0];
    var bestBeginner = suppliers.find(function (s) { return s.verified && String(s.minOrder || '').indexOf('1') >= 0; }) || topRated;
    ...
    var picks = [
      { use: '\uD83D\uDE80 Fastest Shipping', s: fastest, reason: (fastest ? fastest.shipTime : '?') + ' day delivery', color: 'var(--sh-aurora)' },
      { use: '\uD83D\uDCB0 Best Value', s: bestValue, reason: 'Score: ' + (bestValue ? bestValue.value : 0) + '/100', color: 'var(--sh-nebula)' },
      { use: '\u2B50 Highest Rated', s: topRated, reason: (topRated ? topRated.rating : 0) + '\u2605 with ' + (topRated ? topRated.orders : 0) + ' orders', color: '#FFD700' },
      { use: '\uD83D\uDCE6 Most Orders', s: mostOrders, reason: (mostOrders ? mostOrders.orders : 0) + ' total orders', color: 'var(--sh-cosmic)' },
      { use: '\uD83D\uDEE1\uFE0F Lowest Risk', s: lowestRisk, reason: (lowestRisk ? lowestRisk.disputeRate : 0) + '% dispute rate', color: 'var(--sh-aurora)' },
      { use: '\uD83C\uDFAF Best Profit', s: bestProfit, reason: _formatMoney(bestProfit ? calculateProfitPotential(bestProfit).netProfit : 0) + '/unit net', color: 'var(--sh-aurora)' },
    ].filter(function (p) { return p.s; });

    grid.innerHTML = picks.map(function (p) {
      return '<div class="sh-pick-card" style="border-left-color:' + p.color + '" data-name="' + esc(p.s.name) + '">' +
        ...
        '<div class="sh-pick-reason">' + esc(p.reason) + '</div></div>';
    }).join('');
  ...
  function renderRiskRadar(suppliers) {
    ...
    if (!el) return;
    var risks = suppliers.filter(function (s) { return getRiskLevel(s).level !== 'LOW'; });
    if (!risks.length) {
      el.innerHTML = '<div class="sh-no-risk"><span style="font-size:28px">\uD83C\uDF89</span><div>All suppliers are healthy \u2014 no alerts</div></div>';
      return;
    }
    el.innerHTML = risks.map(function (s) {
      var risk = getRiskLevel(s);
      var reasons = [];
      ...
      var name = s.name || 'Unknown';
      var color = s.color || '#667eea';
      return '<div class="sh-risk-card" style="border-left-color:' + esc(risk.color) + '" data-name="' + esc(name) + '">' +
        '<div class="sh-risk-header"><div class="sh-risk-avatar" style="background:' + esc(risk.color) + '22;color:' + esc(risk.color) + '">' + esc(name.charAt(0)) + '</div>' +
        '<div><div class="sh-risk-name">' + esc(name) + '</div><div class="sh-risk-platform">' + esc(s.platform || '') + '</div></div>' +
        '<div class="sh-risk-badge" style="background:' + esc(risk.color) + '18;color:' + esc(risk.color) + ';border-color:' + esc(risk.color) + '44">' + esc(risk.level) + ' RISK</div></div>' +
        '<div class="sh-risk-reasons">' + reasons.map(function (r) { return '<span class="sh-risk-reason">\u26A0 ' + esc(r) + '</span>'; }).join('') + '</div>' +
  ...
  function renderBackups(suppliers) {
    ...
    if (!el || !suppliers.length) { if (el) el.innerHTML = ''; return; }
    var sorted = suppliers.slice().sort(function (a, b) { return computeScore(b) - computeScore(a); });
    var pairs = [];
    for (var i = 0; i < Math.min(sorted.length - 1, 5); i++) {
      pairs.push({
        primary: sorted[i], backup: sorted[i + 1],
    ...
    el.innerHTML = pairs.map(function (p) {
      return '<div class="sh-backup-card" data-name="' + esc(p.primary.name) + '">' +
        '<div class="sh-backup-category">' + esc(p.primary.specialty || p.primary.platform || 'Category') + '</div>' +
        '<div class="sh-backup-flow">' +
        '<div class="sh-backup-box primary"><div class="sh-backup-label">Primary</div><div class="sh-backup-name">' + esc(p.primary.name) + '</div></div>' +
        '<div class="sh-backup-arrow">\u2192</div>' +
        '<div class="sh-backup-box alt"><div class="sh-backup-label">Backup</div><div class="sh-backup-name">' + esc(p.backup ? p.backup.name : 'N/A') + '</div></div>' +
        '</div>' +
        '<div class="sh-backup-reason">' + esc(p.reason) + '</div>' +
        '<div class="sh-backup-score">Match: <strong>' + p.score + '%</strong></div></div>';
    }).join('');
  ...
  function renderWatchlist() {
    ...
    var wl = getWatchlist();
    if (!wl.length) {
      el.innerHTML = '<div class="sh-watchlist-panel"><div class="sh-watchlist-title">Your Watchlist <span class="sh-watchlist-count">0</span></div><div class="sh-watch-empty">Click "Watch" on any supplier to track price & stock changes</div></div>';
      return;
    }
    el.innerHTML = '<div class="sh-watchlist-panel"><div class="sh-watchlist-title">Your Watchlist <span class="sh-watchlist-count">' + wl.length + '</span></div>' +
      '<div class="sh-watchlist-grid">' + wl.map(function (w) {
        return '<div class="sh-watch-card" data-id="' + esc(w.supplierId) + '">' +
          '<div class="sh-watch-header">' +
          '<div class="sh-watch-avatar" style="background:rgba(79,140,255,0.12);color:var(--sh-nebula)">' + esc(w.supplierId.charAt(0)) + '</div>' +
          '<div class="sh-watch-name">' + esc(w.supplierId) + '</div>' +
          '<button class="sh-watch-remove" data-id="' + esc(w.supplierId) + '">\u2715</button></div>' +
          '<div class="sh-watch-changes">' +
          '<div class="sh-watch-change"><span class="stable">\u2014</span> Price: ' + (w.lastData ? _formatMoney(parseFloat(w.lastData.price)) : 'N/A') + '</div>' +
          '<div class="sh-watch-change"><span class="stable">\u2014</span> Rating: ' + (w.lastData ? (w.lastData.rating || 0) + '\u2605' : 'N/A') + '</div>' +
          '</div>' +
          '<div class="sh-watch-time">Added ' + new Date(w.addedAt).toLocaleDateString() + '</div></div>';
      }).join('') + '</div></div>';
    ...
    el.querySelectorAll('.sh-watch-remove').forEach(function (btn) {
      btn.addEventListener('click', function (e) {
        ...
        var wl2 = getWatchlist().filter(function (w) { return w.supplierId !== id; });
        setWatchlist(wl2);
        renderWatchlist();
        updateCardsWatchState();
        _toast('Removed from watchlist', 'info');
  ...
  function showDetail(supplier) {
    ...
    var html = '<div class="sh-detail-overlay" id="shDetailOverlay"></div>' +
      ...
      '<div class="sh-detail-meta">' + esc(supplier.platform || '') + ' \u2022 ' + esc(supplier.location || '') + ' \u2022 ' + esc(supplier.specialty || 'General') + '</div>' +
      ...
      '<span class="sh-detail-badge" style="background:rgba(255,215,0,0.12);color:#FFD700">' + esc(supplier.rating || 0) + '\u2605</span>' +
      '<span class="sh-detail-badge" style="background:' + esc(risk.color) + '18;color:' + esc(risk.color) + '">' + esc(risk.level) + ' RISK</span>' +
      (supplier.verified ? '<span class="sh-detail-badge" style="background:rgba(0,245,160,0.12);color:var(--sh-aurora)">\u2713 Verified</span>' : '') +
      (velBadge ? '<span class="sh-detail-badge" style="background:rgba(0,245,160,0.12);color:var(--sh-aurora)">' + velBadge.icon + ' ' + velBadge.text + '</span>' : '') +
      '</div></div>' +
      '<div class="sh-detail-score-ring"><svg viewBox="0 0 100 100"><circle cx="50" cy="50" r="42" fill="none" stroke="rgba(255,255,255,0.08)" stroke-width="6"/><circle cx="50" cy="50" r="42" fill="none" stroke="' + (score >= 90 ? 'var(--sh-aurora)' : score >= 80 ? 'var(--sh-nebula)' : '#FFD700') + '" stroke-width="6" stroke-dasharray="264" stroke-dashoffset="' + (264 - (264 * score) / 100) + '" stroke-linecap="round" transform="rotate(-90 50 50)" style="transition:stroke-dashoffset 0.8s cubic-bezier(0.4,0,0.2,1)"/></svg><div class="sh-detail-score-val">' + score + '</div></div>' +
      '</div>' +
    ...
    panel.querySelectorAll('.sh-detail-tab').forEach(function (tab) {
      tab.addEventListener('click', function () {
        panel.querySelectorAll('.sh-detail-tab').forEach(function (t) { t.classList.remove('active'); });
    ...
    var onEsc = function (e) { if (e.key === 'Escape') { closeDetail(); document.removeEventListener('keydown', onEsc); } };
    document.addEventListener('keydown', onEsc);

    // Negotiate
    var negoBtn = panel.querySelector('#shNegoBtn');
    if (negoBtn) negoBtn.addEventListener('click', function () {
  ...
  function renderDetailTab(tab, supplier, dna, profit, vel) {
    if (tab === 'overview') {
      return '<div class="sh-detail-card"><h4>\uD83D\uDCCA Key Metrics</h4><div class="sh-detail-metrics">' +
        ...
        '<div class="sh-detail-m"><span class="sh-detail-m-label">Products</span><span class="sh-detail-m-val">' + (supplier.topProducts ? supplier.topProducts.length : 0) + '</span></div>' +
        ...
        '<div class="sh-detail-m"><span class="sh-detail-m-label">Response Rate</span><span class="sh-detail-m-val">' + esc(supplier.responseRate || 0) + '%</span></div>' +
        '<div class="sh-detail-m"><span class="sh-detail-m-label">Fulfillment</span><span class="sh-detail-m-val">' + esc(supplier.fulfillmentRate || 0) + '%</span></div>' +
        '<div class="sh-detail-m"><span class="sh-detail-m-label">Dispute Rate</span><span class="sh-detail-m-val" style="color:' + ((supplier.disputeRate || 0) < 1 ? 'var(--sh-aurora)' : 'var(--sh-solar)') + '">' + esc(supplier.disputeRate || 0) + '%</span></div>' +
        ...
        '<div class="sh-detail-m"><span class="sh-detail-m-label">Velocity Score</span><span class="sh-detail-m-val">' + vel + '/60</span></div>' +
        '</div></div>' +
        '<div class="sh-detail-card"><h4>\u2705 Capabilities</h4><div class="sh-detail-metrics">' +
        '<div class="sh-detail-m"><span class="sh-detail-m-label">Verified</span><span class="sh-detail-m-val">' + (supplier.verified ? '\u2713 Yes' : '\u2717 No') + '</span></div>' +
        '<div class="sh-detail-m"><span class="sh-detail-m-label">Samples</span><span class="sh-detail-m-val">' + (supplier.sampleAvailable ? '\u2713 Available' : '\u2717 No') + '</span></div>' +
        '<div class="sh-detail-m"><span class="sh-detail-m-label">Custom Packaging</span><span class="sh-detail-m-val">' + (supplier.customPackaging ? '\u2713 Yes' : '\u2717 No') + '</span></div>' +
        '<div class="sh-detail-m"><span class="sh-detail-m-label">Dropship Support</span><span class="sh-detail-m-val">' + (supplier.dropshipSupport ? '\u2713 Yes' : '\u2717 No') + '</span></div>' +
        '</div></div>' +
        (function () {
          ...
          return '<div class="sh-detail-card sh-nego-summary-card"><h4>\uD83E\uDD1D Negotiation Status</h4>' +
            '<div class="sh-nego-summary-row">' +
            '<span class="sh-nego-status-badge" style="background:' + color + '22;color:' + color + ';border:1px solid ' + color + '44">' + label + '</span>' +
            '<span class="sh-nego-summary-msgs">' + msgCount + ' messages</span>' +
            '</div>' +
            (bo ? '<div class="sh-nego-summary-offer">Best offer: <strong>$' + bo.price.toFixed(2) + '</strong>/unit</div>' : '') +
    ...
    if (tab === 'performance') {
      ...
      return '<div class="sh-detail-card"><h4>\uD83C\uDFAF Score Breakdown</h4><div class="sh-detail-bars">' +
        '<div class="sh-detail-bar-row"><span class="sh-detail-bar-label">Quality</span><div class="sh-detail-bar-track"><div class="sh-detail-bar-fill" style="width:' + (supplier.quality || 0) + '%;background:var(--sh-aurora)"></div></div><span class="sh-detail-bar-val">' + (supplier.quality || 0) + '</span></div>' +
        '<div class="sh-detail-bar-row"><span class="sh-detail-bar-label">Communication</span><div class="sh-detail-bar-track"><div class="sh-detail-bar-fill" style="width:' + (supplier.communication || 0) + '%;background:var(--sh-nebula)"></div></div><span class="sh-detail-bar-val">' + (supplier.communication || 0) + '</span></div>' +
        '<div class="sh-detail-bar-row"><span class="sh-detail-bar-label">Value</span><div class="sh-detail-bar-track"><div class="sh-detail-bar-fill" style="width:' + (supplier.value || 0) + '%;background:var(--sh-cosmic)"></div></div><span class="sh-detail-bar-val">' + (supplier.value || 0) + '</span></div>' +
        '<div class="sh-detail-bar-row"><span class="sh-detail-bar-label">Response Rate</span><div class="sh-detail-bar-track"><div class="sh-detail-bar-fill" style="width:' + (supplier.responseRate || 0) + '%;background:#FFD700"></div></div><span class="sh-detail-bar-val">' + (supplier.responseRate || 0) + '%</span></div>' +
        '<div class="sh-detail-bar-row"><span class="sh-detail-bar-label">Fulfillment</span><div class="sh-detail-bar-track"><div class="sh-detail-bar-fill" style="width:' + (supplier.fulfillmentRate || 0) + '%;background:var(--sh-aurora)"></div></div><span class="sh-detail-bar-val">' + (supplier.fulfillmentRate || 0) + '%</span></div>' +
        '</div></div>' +
        '<div class="sh-detail-card"><h4>\uD83D\uDCC8 Velocity Trends</h4><div class="sh-trend-list">' +
        '<div class="sh-trend-item"><span class="sh-trend-arrow ' + ((vd.salesGrowth30d || 0) > 0 ? 'up' : 'down') + '">' + ((vd.salesGrowth30d || 0) > 0 ? '\u2191' : '\u2193') + '</span>Sales Growth (30d): ' + (vd.salesGrowth30d || 0) + '%</div>' +
        '<div class="sh-trend-item"><span class="sh-trend-arrow ' + (vd.ratingTrend === 'up' ? 'up' : vd.ratingTrend === 'down' ? 'down' : 'stable') + '">' + (vd.ratingTrend === 'up' ? '\u2191' : vd.ratingTrend === 'down' ? '\u2193' : '\u2192') + '</span>Rating Trend: ' + (vd.ratingTrend || 'stable') + '</div>' +
        '<div class="sh-trend-item"><span class="sh-trend-arrow up">\u2191</span>New Products (30d): ' + (vd.newProducts30d || 0) + '</div>' +
        '<div class="sh-trend-item"><span class="sh-trend-arrow ' + (vd.responseTimeTrend === 'faster' ? 'up' : 'stable') + '">' + (vd.responseTimeTrend === 'faster' ? '\u2191' : '\u2192') + '</span>Response Time: ' + (vd.responseTimeTrend || 'stable') + '</div>' +
        '</div></div>';
    }
    if (tab === 'shipping') {
      ...
      var carrierHtml = carriers.length ? '<div class="sh-ship-compare" style="margin-top:10px"><div class="sh-ship-compare-row sh-ship-compare-header"><span class="sh-ship-compare-label"><strong>Carrier</strong></span><span class="sh-ship-compare-val"><strong>Days</strong></span><span class="sh-ship-compare-val"><strong>Cost</strong></span></div>' +
        carriers.map(function (c) {
          return '<div class="sh-ship-compare-row"><span class="sh-ship-compare-label">' + esc(c.name) + '</span><span class="sh-ship-compare-val">' + esc(c.days) + '</span><span class="sh-ship-compare-val">' + esc(c.cost) + '</span></div>';
        }).join('') + '</div>' : '';
      var confBadge = confidence !== null ? '<span class="sh-badge" style="background:' + (confidence >= 85 ? 'var(--sh-aurora)' : confidence >= 70 ? '#FFD700' : 'var(--sh-solar)') + ';color:#000;margin-left:8px">' + confidence + '% confidence</span>' : '';
      var recBadge = recommendation ? '<span class="sh-badge" style="background:' + (recommendation === 'Reliable' ? 'var(--sh-aurora)' : recommendation === 'Moderate' ? '#FFD700' : 'var(--sh-solar)') + ';color:#000;margin-left:8px">' + esc(recommendation) + '</span>' : '';
      return '<div class="sh-detail-card"><h4>\uD83D\uDE9A Shipping Info</h4><div class="sh-detail-metrics">' +
        ...
        '</div></div>' +
        '<div class="sh-detail-card"><h4>\uD83D\uDCCA Estimated vs Reality (to ' + esc(country) + ')' + confBadge + recBadge + '</h4><div class="sh-ship-compare">' +
        '<div class="sh-ship-compare-row"><span class="sh-ship-compare-label">Supplier Claims</span><span class="sh-ship-compare-val">' + esc(supplier.shipTime || 'N/A') + ' days</span></div>' +
        '<div class="sh-ship-compare-row"><span class="sh-ship-compare-label">Realistic Best</span><span class="sh-ship-compare-val" style="color:var(--sh-aurora)">~' + bestCase + ' days</span></div>' +
        '<div class="sh-ship-compare-row"><span class="sh-ship-compare-label">Realistic Worst</span><span class="sh-ship-compare-val" style="color:var(--sh-solar)">~' + worstCase + ' days</span></div>' +
        '<div class="sh-ship-compare-row"><span class="sh-ship-compare-label">Customs Delay</span><span class="sh-ship-compare-val">' + esc(customsInfo) + '</span></div>' +
        '</div>' + carrierHtml + '</div>';
    }
    if (tab === 'profit') {
      return '<div class="sh-detail-card"><h4>\uD83D\uDCB0 Profit Potential</h4><div class="sh-detail-metrics">' +
        '<div class="sh-detail-m"><span class="sh-detail-m-label">Supplier Cost</span><span class="sh-detail-m-val">' + _formatMoney(profit.supplierCost) + '</span></div>' +
        '<div class="sh-detail-m"><span class="sh-detail-m-label">Market Price</span><span class="sh-detail-m-val">' + _formatMoney(profit.marketPrice) + '</span></div>' +
        '<div class="sh-detail-m"><span class="sh-detail-m-label">Net Profit</span><span class="sh-detail-m-val" style="color:var(--sh-aurora)">' + _formatMoney(profit.netProfit) + '/unit</span></div>' +
        '<div class="sh-detail-m"><span class="sh-detail-m-label">Margin</span><span class="sh-detail-m-val">' + Math.round(profit.marginAtAvg) + '%</span></div>' +
        '<div class="sh-detail-m"><span class="sh-detail-m-label">Viability</span><span class="sh-detail-m-val" style="color:' + (profit.isViable ? 'var(--sh-aurora)' : 'var(--sh-solar)') + '">' + (profit.isViable ? '\u2705 Viable' : '\u26A0\uFE0F Low Margin') + '</span></div>' +
        '</div></div>';
    }
    if (tab === 'products') {
      var products = supplier.topProducts || [];
      return '<div class="sh-detail-card"><h4>\uD83C\uDFC6 Top Products</h4><div class="sh-detail-products">' +
        (products.length ? products.map(function (p) { return '<span class="sh-detail-product-chip" data-product="' + esc(p) + '">' + esc(p) + '</span>'; }).join('') : '<span style="color:var(--sh-text-dim)">No products listed</span>') +
        '</div></div>';
    }
    return '';
  }
  ...
  function renderNegoStatusBadge(status) {
    ...
    return '<span class="sh-nego-status-badge" style="background:' + color + '22;color:' + color + ';border:1px solid ' + color + '44">' + label + '</span>';
  }
  ...
  function renderNegoTimeline(nego) {
    ...
    nego.messages.forEach(function (m, i) {
      ...
      var offerTag = offers.length ? '<span class="sh-nego-offer-tag">$' + offers[offers.length - 1].toFixed(2) + '</span>' : '';
      html += '<div class="sh-nego-timeline-item ' + (isUser ? 'user' : 'supplier') + '">' +
        '<div class="sh-nego-timeline-dot"></div>' +
        '<div class="sh-nego-timeline-content">' +
        '<div class="sh-nego-timeline-header">' +
        '<span class="sh-nego-timeline-sender">' + (isUser ? 'You' : (nego.supplierId || 'Supplier')) + '</span>' +
        '<span class="sh-nego-timeline-time">' + timeStr + '</span>' +
        '</div>' +
        '<div class="sh-nego-timeline-text">' + esc(m.text) + '</div>' +
        offerTag +
        (isUser && m.text.toLowerCase().includes('counter') ? '<span class="sh-nego-offer-tag" style="background:var(--sh-cosmic);color:#fff">Counter-Offer</span>' : '') +
        '</div></div>';
    });
    html += '</div>';
    return html;
  ...
  function showNegotiationModal(supplier) {
    ...
    var negoIdx = negos.findIndex(function (n) { return n.supplierId === supplier.name; });
    ...
    var lastOfferHtml = lastOffer
      ? '<div class="sh-nego-last-offer">Last offer: <strong>$' + lastOffer.price.toFixed(2) + '</strong>' +
        (currentPrice > 0 ? ' <span class="sh-nego-savings">(' + Math.round((1 - lastOffer.price / currentPrice) * 100) + '% off original)</span>' : '') +
        '</div>'
      : '';
    ...
    var tplHtml = templates.map(function (t) {
      return '<button class="sh-nego-tpl-btn" id="' + t.id + '">' + t.label + '</button>';
    }).join('');

    var counterOfferHtml = '<div class="sh-nego-counter-wrap" id="shNegoCounterWrap" style="display:none">' +
    ...
    var dealActionsHtml = '<div class="sh-nego-deal-actions" id="shNegoDealActions" style="display:none">' +
    ...
    modal.innerHTML = '<div class="sh-nego-content">' +
      '<div class="sh-nego-header">' +
      '<div class="sh-nego-title">\uD83E\uDD1D Negotiate with ' + esc(supplier.name) + '</div>' +
      '<div class="sh-nego-header-meta">' + statusHtml + lastOfferHtml + '</div>' +
      '<button class="sh-nego-close" id="shNegoCloseBtn">\u2715</button>' +
      '</div>' +

      '<div class="sh-nego-templates" id="shNegoTemplates">' + tplHtml + '</div>' +
    ...
    function refreshMessages() {
      ...
      var r = refreshed.find(function (n) { return n.supplierId === supplier.name; });
      if (r) {
        msgArea.innerHTML = renderNegoTimeline(r);
        var bo = getBestOffer(r);
        var loEl = modal.querySelector('.sh-nego-last-offer');
        if (bo && loEl) {
          loEl.innerHTML = 'Last offer: <strong>$' + bo.price.toFixed(2) + '</strong>' +
            (currentPrice > 0 ? ' <span class="sh-nego-savings">(' + Math.round((1 - bo.price / currentPrice) * 100) + '% off original)</span>' : '');
        }
    ...
    function sendMsg(text) {
      ...
      if (!nego) {
        nego = {
          supplierId: supplier.name, product: (supplier.topProducts && supplier.topProducts[0]) || '',
          platform: supplier.platform, status: NEGO_STATUSES.PENDING, messages: [],
      ...
      nego.updatedAt = Date.now();
      setNegotiations(negos);
      negoIdx = negos.findIndex(function (n) { return n.supplierId === supplier.name; });
    ...
    function sendCounterOffer() {
      ...
      if (!val || val <= 0) { _toast('Enter a valid price', 'error'); return; }
    ...
    function acceptDeal() {
      ...
      nego.messages.push({ from: 'system', text: '\u2705 Deal accepted at $' + (lastOffer ? lastOffer.price.toFixed(2) : 'N/A') + '/unit', ts: Date.now() });
      setNegotiations(negos);
    ...
    modal.querySelector('#shNegoSend').addEventListener('click', function () { sendMsg(negoInput.value); });
    negoInput.addEventListener('keydown', function (e) { if (e.key === 'Enter') sendMsg(negoInput.value); });

    templates.forEach(function (t) {
      var btn = modal.querySelector('#' + t.id);
      if (btn) btn.addEventListener('click', function () { sendMsg(getNegoTemplate(t.type, supplier)); });
    });
    ...
    modal.querySelector('#shNegoCounterInput').addEventListener('keydown', function (e) { if (e.key === 'Enter') sendCounterOffer(); });
    ...
    modal.querySelector('#shNegoClear').addEventListener('click', function () {
      ...
      msgArea.innerHTML = '<div class="sh-nego-empty">Choose a template above or type a message to start negotiating</div>';
    ...
    modal.querySelector('#shNegoCloseBtn').addEventListener('click', function () { modal.remove(); });
  ...
  async function searchSuppliers(query) {
    ...
    try {
      ...
      if (!resp.ok) {
        var err = await resp.json().catch(function () { return { error: 'Search failed' }; });
      ...
      if (statusEl) {
        statusEl.innerHTML = '<span class="sh-search-spinner"></span> Found ' + _suppliers.length + ' suppliers. Enriching with velocity & shipping data...';
      }
      ...
      if (doDeep) {
        ...
        if (deepResults.length) {
          deepResults.forEach(function (ds) {
            var existing = _suppliers.find(function (s) { return s.name === ds.name; });
            if (!existing) {
              ds._isDeep = true;
              _suppliers.push(ds);
            }
      ...
      if (statusEl) {
        statusEl.innerHTML = 'Found <strong>' + _suppliers.length + '</strong> suppliers for "' + esc(query) + '" \u2014 ' + webLabel + ', ' + aiLabel;
      }
    } catch (e) {
      console.error('[SupplierHub] Search error:', e);
      if (statusEl) statusEl.innerHTML = '<span class="sh-search-error">Error: ' + esc(String(e.message || e)) + '</span>';
    } finally {
      _searching = false;
  ...
  async function fetchVelocityData(suppliers) {
    ...
    try {
      var resp = await fetch(BACKEND_URL + '/suppliers/velocity', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ suppliers: suppliers.map(function (s) {
          return { name: s.name, platform: s.platform, rating: s.rating, orders: s.orders, responseTime: s.responseTime };
        }) }),
      });
      if (!resp.ok) return;
      var data = await resp.json();
      ...
      var resp = await fetch(BACKEND_URL + '/suppliers/shipping-intel', {
        ...
        body: JSON.stringify({
          suppliers: suppliers.map(function (s) {
            return { name: s.name, shipTime: s.shipTime, shipCost: s.shipCost, location: s.location, platform: s.platform, fulfillmentRate: s.fulfillmentRate, disputeRate: s.disputeRate, verified: s.verified };
          }),
  ...
  function bindSearch() {
    ...
    if (linkGoBtn && linkInput) {
      ...
      linkInput.addEventListener('keydown', function (e) { if (e.key === 'Enter') doLinkSearch(); });
    }
    ...
    if (dropzoneUpload && fileInput) {
      dropzoneUpload.addEventListener('click', function () { fileInput.click(); });
    }
    ...
    function handleImageSearch(file) {
      var statusEl = _section ? _section.querySelector('#shSearchStatus') : null;
      if (statusEl) statusEl.innerHTML = '<span class="sh-search-spinner"></span> Analyzing image: ' + esc(file.name) + '...';
  ...
  var SupplierHubPlugin = {
    ...
    name: 'Find Suppliers',
    version: '3.0.0',
    description: 'Complete supplier intelligence — velocity scoring, deep sourcing, profit calculation, negotiation, watchlist & DNA profiles',
