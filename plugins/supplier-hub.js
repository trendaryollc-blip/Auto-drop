...
(function () {
    ...
    const picks = [
      {
        ...
        supplier: 'PawWalk USA',
        reason: '1-4 day delivery with Free Prime shipping',
        platform: 'Amazon',
        color: 'var(--accent-green)',
      },
      {
        ...
        reason: 'Lowest cost with high order volume (320K+)',
        platform: 'Temu',
        color: 'var(--accent-cyan)',
      },
      {
        use: '\u2B50 Highest Rated',
        supplier: 'TechGear Direct',
        reason: '4.9 rating with 120K orders and <2h response',
        platform: 'AliExpress',
        color: 'var(--accent-yellow)',
      },
      {
        use: '\uD83D\uDCE6 Largest Catalog',
        supplier: 'BeautyGlow Co',
        reason: '567 products with fastest response time',
        platform: 'CJ Dropshipping',
        color: 'var(--accent-pink)',
      },
      {
        use: '\uD83D\uDEE1\uFE0F Lowest Risk',
        supplier: 'StarLight Tech',
        reason: '0.9% refund rate with premium quality score',
        platform: 'DHgate',
        color: 'var(--accent-purple)',
      },
      {
        use: '\uD83C\uDFAF Best for Beginners',
        supplier: 'SmartHome US',
        reason: 'No minimum order, free shipping, US-based',
        platform: 'Amazon',
        color: 'var(--accent-orange)',
      },
    ];
  ...
  `
    ...
    picksGrid.querySelectorAll('.sh-pick-card').forEach((card) => {
      const handler = () => {
        const s = SUPPLIERS.find((x) => x.name === card.dataset.supplier);
        if (s) showDetail(s);
      };
      card.addEventListener('click', handler);
      card.addEventListener('keydown', (e) => {
